...
def llinput(*prompt, sep=" ", end='\n', wend='', max_length=None, forceint=False, forceinput=False, forcemininputlen=0, forceinputlen=0, otodt=False, inputcolor=None, promptcolor=None, endcolor=None, wendcolor=None):
    ...
    update_display()  # Initial display with prompt and wend
    if os.name == "nt":
        while True:
            ch = msvcrt.getwch()  # Unicode karakterleri destekleyen versiyon
            if ch in {'\r', '\n'}:  # Enter key
                if forceinput:
                    if input_str == "":
                        continue
