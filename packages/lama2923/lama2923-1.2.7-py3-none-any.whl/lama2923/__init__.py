...
def llinput(*prompt, sep=" ", end='\n', wend='', max_length=None, min_lenght=None, forceint=False, forceinput=False, startswith = ("", False), forcestartswith="", blockedchars = r"", forcemininputlen=0, forceinputlen=0, otodt=False, inputcolor=None, promptcolor=None, endcolor=None, wendcolor=None):
    ...
    if os.name == "nt":
        while True:
            ...
            if forcestartswith:
                lenx = len(input_str)
                if lenx < len(forcestartswith):
                    if ch != forcestartswith[lenx]:
                        if ch in {'\b', '\x7f'}:
            ...
            if ch in {'\r', '\n'}:  # Enter key
                ...
                if forcemininputlen > 0:
                    if len(input_str) < forcemininputlen:
                        continue
        ...
        raise NotImplementedError("This function is not implemented for Unix-like systems yet.")
