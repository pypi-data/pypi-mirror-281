    ...
    def ReliabilityTS(self):
        ...
        if self.state=="ts":
            ...
            if self.state=="yearly_total":
                if self.ignore_leap_years == False:
                    year_month=utils.get_year_and_month(demand_ts.index)
                    demand_ts[:]=[self.demand/(365+calendar.isleap(int(x[0:4]))) for x in year_month] # If using leap years, divide by 365 or 366 days depending on year
                else:
                    demand_ts[:]=self.demand/365 # If not using leap years, only ever divide by 365                    
                return demand_ts


    def MonthlyReliability(self, tol=1, allow_part_months=False):
