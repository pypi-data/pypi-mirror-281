...
class Rows(ChildObject):
    ...
    def insert(self, columns=None, values=None) -> None:
        if (values or columns) is None:
            raise ValueError('The "values" must be specified')
        if columns and values is None:
            raise ValueError('If "columns" are specified, then "values" must also be specified.')

        if columns is None and values:
            # inserting a "tuple" single row
            if isinstance(values, tuple):
                columns = self.find_parent(Table).columns.names[:len(values)]
                return self.insert(columns=columns, values=values)
            if isinstance(values, Row):
                return self.insert(values=tuple(values))

            # inserting a "dict" single row
            if isinstance(values, dict):
                columns = tuple(values.keys())
                values = tuple(values.values())
                return self.insert(columns=columns, values=values)
            if isinstance(values, zip):
                return self.insert(values=dict(values))

            # inserting a collection of rows
            if isinstance(values, list):
                for item in values:
                    self.insert(values=item)
                return None
            if isinstance(values, Rows):
                for item in values:
                    self.insert(values=tuple(item))
                return None
        ...
        if isinstance(columns, tuple) and values:
            ...
            # inserting a "tuple" single row
            if isinstance(values, tuple):
                plug = ','.join(['?'] * len(values))
                cmd = f"INSERT INTO {self.find_parent(Table).name} ({plug}) VALUES({plug});"
                self._crsr.execute(cmd, columns + values)
                return self._conn.commit()
            if isinstance(values, Row):
                values = tuple(values.get(item) for item in columns)
                if None in values:
                    raise KeyError("The columns do not match the values.")
                return self.insert(columns=columns, values=values)
            ...
            if isinstance(values, dict):
                ...
                return self.insert(columns=columns, values=values)
            if isinstance(values, zip):
                return self.insert(columns=columns, values=dict(values))

            # inserting a collection of rows
            if isinstance(values, list):
                for item in values:
                    self.insert(columns=columns, values=item)
                return None
            if isinstance(values, Rows):
                for item in values:
                    self.insert(columns=columns, values=tuple(item))
                return None
