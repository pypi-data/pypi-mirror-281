...
class Gitea:
    ...
    def ins(self):
        return get_ins(self.url, **self.auth)
...
class GiteaManager:
    ...
    def gitea(self):
        ...
        gitea_kwargs = {}
        if auth['username']:
            gitea_kwargs = dict(auth=(auth['username'], auth['password']))
        if auth['token']:
            gitea_kwargs = dict(token_text=data['token'])
        return self.gitea_cls(data['url'], gitea_kwargs)
