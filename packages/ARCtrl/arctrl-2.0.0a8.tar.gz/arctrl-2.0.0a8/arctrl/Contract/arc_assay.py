...
def ARCtrl_ArcAssay__ArcAssay_ToCreateContract_6FCE9E49(this: ArcAssay, WithFolder: bool | None=None) -> Array[Contract]:
    ...
    def _arrow1927(__unit: None=None, this: Any=this, WithFolder: Any=WithFolder) -> IEnumerable_1[Contract]:
        def _arrow1924(__unit: None=None) -> IEnumerable_1[Contract]:
            ...
            def _arrow1922(p: str) -> IEnumerable_1[Contract]:
                return singleton(Contract.create_create(p, DTOType(7))) if (p != path) else empty()

            return collect(_arrow1922, folder_fs.ToFilePaths(False))

        def _arrow1926(__unit: None=None) -> IEnumerable_1[Contract]:
            return singleton(c)

        return append(_arrow1924() if with_folder else empty(), delay(_arrow1926))

    return to_array(delay(_arrow1927))
