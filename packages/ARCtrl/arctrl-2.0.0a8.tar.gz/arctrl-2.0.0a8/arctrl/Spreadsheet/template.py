...
TemplateReadError_reflection = _expr1059
...
Metadata_Template_TemplateInfo_reflection = _expr1061
...
def Metadata_Template_TemplateInfo_ToSparseTable_48C39BE1(template: Template) -> SparseTable:
    ...
    def _arrow1062(__unit: None=None, template: Any=template) -> str:
        copy_of_struct: str = template.Id
        return str(copy_of_struct)

    def _arrow1063(__unit: None=None, template: Any=template) -> str:
        copy_of_struct_1: str = template.Id
        return str(copy_of_struct_1)

    add_to_dict(matrix.Matrix, ("Id", 1), "" if (_arrow1062().find("MISSING_IDENTIFIER_") == 0) else _arrow1063())
...
def Metadata_Template_toRows(template: Template) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
    def _arrow1073(__unit: None=None, template: Any=template) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
                    ...
                    def _arrow1069(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
                        def _arrow1068(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
                            def _arrow1067(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
                                ...
                                return append(singleton(SparseRowModule_fromValues(to_enumerable(["AUTHORS"]))), delay(_arrow1066))

                            return append(Metadata_Tags_toRows(None, to_list(template.Tags)), delay(_arrow1067))

                        return append(singleton(SparseRowModule_fromValues(to_enumerable(["TAGS"]))), delay(_arrow1068))

                    return append(Metadata_ER_toRows(None, to_list(template.EndpointRepositories)), delay(_arrow1069))

                return append(singleton(SparseRowModule_fromValues(to_enumerable(["ERS"]))), delay(_arrow1070))

            return append(Metadata_Template_TemplateInfo_toRows_48C39BE1(template), delay(_arrow1071))

        return append(singleton(SparseRowModule_fromValues(to_enumerable(["TEMPLATE"]))), delay(_arrow1072))

    return delay(_arrow1073)
...
def Template_fromFsWorkbook(doc: FsWorkbook) -> Template:
    ...
    def _arrow1075(__unit: None=None, doc: Any=doc) -> ArcTable:
    ...
    return Template_fromParts(template_info, pattern_input[1], pattern_input[2], pattern_input[3], _arrow1075(), now())
