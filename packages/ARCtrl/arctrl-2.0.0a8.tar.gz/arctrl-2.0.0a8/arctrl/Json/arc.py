...
def encoder(isa: ArcInvestigation) -> Json:
    ...
    def _arrow1799(value: str, isa: Any=isa) -> Json:
        return Json(0, value)

    def _arrow1800(value_2: str, isa: Any=isa) -> Json:
        return Json(0, value_2)

    def _arrow1801(oa: ArcInvestigation, isa: Any=isa) -> Json:
        return Investigation_ROCrate_encoder(oa)

    return Json(5, choose(chooser, of_array([try_include("@type", _arrow1799, "CreativeWork"), try_include("@id", _arrow1800, "ro-crate-metadata.json"), try_include("about", _arrow1801, isa), ("conformsTo", conforms_to_jsonvalue), ("@context", context_jsonvalue)])))


def _arrow1802(get: IGetters) -> ArcInvestigation | None:
    object_arg: IOptionalGetter = get.Optional
    return object_arg.Field("about", Investigation_ROCrate_decoder)


decoder: Decoder_1[ArcInvestigation | None] = object(_arrow1802)
