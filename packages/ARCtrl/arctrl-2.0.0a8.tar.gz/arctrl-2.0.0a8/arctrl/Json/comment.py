...
def Comment_ISAJson_encoder(id_map: Any | None, comment: Comment) -> Json:
    def f(comment_1: Comment, id_map: Any=id_map, comment: Any=comment) -> Json:
        ...
        def _arrow1109(value: str, comment_1: Any=comment_1) -> Json:
            return Json(0, value)

        def _arrow1110(value_2: str, comment_1: Any=comment_1) -> Json:
            return Json(0, value_2)

        def _arrow1111(value_4: str, comment_1: Any=comment_1) -> Json:
            return Json(0, value_4)

        return Json(5, choose(chooser, of_array([try_include("@id", _arrow1109, Comment_ROCrate_genID(comment_1)), try_include("name", _arrow1110, comment_1.Name), try_include("value", _arrow1111, comment_1.Value)])))

    if id_map is None:
        return f(comment)

    else: 
        def _arrow1112(c: Comment, id_map: Any=id_map, comment: Any=comment) -> str:
            return Comment_ROCrate_genID(c)

        return encode(_arrow1112, f, comment, id_map)
...
def ARCtrl_Comment__Comment_toJsonString_Static_71136F3F(spaces: int | None=None) -> Callable[[Comment], str]:
    def _arrow1113(c: Comment, spaces: Any=spaces) -> str:
...
def ARCtrl_Comment__Comment_toROCrateJsonString_Static_71136F3F(spaces: int | None=None) -> Callable[[Comment], str]:
    def _arrow1114(c: Comment, spaces: Any=spaces) -> str:
...
def ARCtrl_Comment__Comment_toISAJsonString_Static_71136F3F(spaces: int | None=None) -> Callable[[Comment], str]:
    def _arrow1115(c: Comment, spaces: Any=spaces) -> str:
