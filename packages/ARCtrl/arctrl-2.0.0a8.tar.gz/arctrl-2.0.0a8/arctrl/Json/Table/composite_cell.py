...
def _arrow1636(get: IGetters) -> CompositeCell:
    ...
    def _arrow1635(__unit: None=None) -> Data:
    ...
    return CompositeCell(1, _arrow1631()) if (match_value == "FreeText") else (CompositeCell(0, _arrow1632()) if (match_value == "Term") else (CompositeCell(2, _arrow1633(), _arrow1634()) if (match_value == "Unitized") else (CompositeCell(3, _arrow1635()) if (match_value == "Data") else to_fail(printf("Error reading CompositeCell from json string: %A"))(match_value))))


CompositeCell_decoder: Decoder_1[CompositeCell] = object(_arrow1636)
...
def CompositeCell_decoderCompressed(string_table: Array[str], oa_table: Array[OntologyAnnotation]) -> Decoder_1[CompositeCell]:
    def _arrow1642(get: IGetters, string_table: Any=string_table, oa_table: Any=oa_table) -> CompositeCell:
        match_value: str
        ...
        def _arrow1638(__unit: None=None) -> OntologyAnnotation:
        ...
        def _arrow1640(__unit: None=None) -> OntologyAnnotation:
            arg_9: Decoder_1[OntologyAnnotation] = index(1, decode_oa(oa_table))
            object_arg_4: IRequiredGetter = get.Required
            return object_arg_4.Field("v", arg_9)

        def _arrow1641(__unit: None=None) -> Data:
            arg_11: Decoder_1[Data] = index(0, Data_compressedDecoder(string_table))
            object_arg_5: IRequiredGetter = get.Required
            return object_arg_5.Field("v", arg_11)

        return CompositeCell(1, _arrow1637()) if (match_value == "FreeText") else (CompositeCell(0, _arrow1638()) if (match_value == "Term") else (CompositeCell(2, _arrow1639(), _arrow1640()) if (match_value == "Unitized") else (CompositeCell(3, _arrow1641()) if (match_value == "Data") else to_fail(printf("Error reading CompositeCell from json string: %A"))(match_value))))

    return object(_arrow1642)
...
def ARCtrl_CompositeCell__CompositeCell_toJsonString_Static_71136F3F(spaces: int | None=None) -> Callable[[CompositeCell], str]:
    def _arrow1643(obj: CompositeCell, spaces: Any=spaces) -> str:
