...
def _arrow1565(Item: Sample) -> ProcessOutput:
    return ProcessOutput(0, Item)


def _arrow1566(Item_1: Data) -> ProcessOutput:
    return ProcessOutput(1, Item_1)


def _arrow1567(Item_2: Material) -> ProcessOutput:
    return ProcessOutput(2, Item_2)


ProcessOutput_ROCrate_decoder: Decoder_1[ProcessOutput] = one_of(of_array([map(_arrow1565, Sample_ROCrate_decoder), map(_arrow1566, Data_ROCrate_decoder), map(_arrow1567, Material_ROCrate_decoder)]))
...
def _arrow1569(Item: Sample) -> ProcessOutput:
    return ProcessOutput(0, Item)


def _arrow1570(Item_1: Data) -> ProcessOutput:
    return ProcessOutput(1, Item_1)


def _arrow1571(Item_2: Material) -> ProcessOutput:
    return ProcessOutput(2, Item_2)


ProcessOutput_ISAJson_decoder: Decoder_1[ProcessOutput] = one_of(of_array([map(_arrow1569, Sample_ISAJson_decoder), map(_arrow1570, Data_ISAJson_decoder), map(_arrow1571, Material_ISAJson_decoder)]))
...
def ARCtrl_Process_ProcessOutput__ProcessOutput_toISAJsonString_Static_Z3B036AA(spaces: int | None=None, use_idreferencing: bool | None=None) -> Callable[[ProcessOutput], str]:
    ...
    def _arrow1572(f: ProcessOutput, spaces: Any=spaces, use_idreferencing: Any=use_idreferencing) -> str:
...
def ARCtrl_Process_ProcessOutput__ProcessOutput_toROCrateJsonString_Static_71136F3F(spaces: int | None=None) -> Callable[[ProcessOutput], str]:
    def _arrow1573(f: ProcessOutput, spaces: Any=spaces) -> str:
