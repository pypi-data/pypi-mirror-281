...
def _arrow1195(Item: int) -> Value:
    return Value(1, Item)


def _arrow1196(Item_1: float) -> Value:
    return Value(2, Item_1)


def _arrow1197(Item_2: OntologyAnnotation) -> Value:
    return Value(0, Item_2)


def _arrow1198(Item_3: str) -> Value:
    return Value(3, Item_3)


Value_ISAJson_decoder: Decoder_1[Value] = one_of(of_array([map(_arrow1195, int_1), map(_arrow1196, float_1), map(_arrow1197, OntologyAnnotation_ISAJson_decoder), map(_arrow1198, string)]))
...
def ARCtrl_Process_Value__Value_toISAJsonString_Static_71136F3F(spaces: int | None=None) -> Callable[[Value], str]:
    def _arrow1199(v: Value, spaces: Any=spaces) -> str:
