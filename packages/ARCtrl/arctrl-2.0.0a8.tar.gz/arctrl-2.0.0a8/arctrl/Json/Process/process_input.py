...
def _arrow1543(Item: Source) -> ProcessInput:
    return ProcessInput(0, Item)


def _arrow1544(Item_1: Sample) -> ProcessInput:
    return ProcessInput(1, Item_1)


def _arrow1545(Item_2: Data) -> ProcessInput:
    return ProcessInput(2, Item_2)


def _arrow1546(Item_3: Material) -> ProcessInput:
    return ProcessInput(3, Item_3)


ProcessInput_ROCrate_decoder: Decoder_1[ProcessInput] = one_of(of_array([map(_arrow1543, Source_ROCrate_decoder), map(_arrow1544, Sample_ROCrate_decoder), map(_arrow1545, Data_ROCrate_decoder), map(_arrow1546, Material_ROCrate_decoder)]))
...
def _arrow1548(Item: Source) -> ProcessInput:
    return ProcessInput(0, Item)


def _arrow1549(Item_1: Sample) -> ProcessInput:
    return ProcessInput(1, Item_1)


def _arrow1550(Item_2: Data) -> ProcessInput:
    return ProcessInput(2, Item_2)


def _arrow1551(Item_3: Material) -> ProcessInput:
    return ProcessInput(3, Item_3)


ProcessInput_ISAJson_decoder: Decoder_1[ProcessInput] = one_of(of_array([map(_arrow1548, Source_ISAJson_decoder), map(_arrow1549, Sample_ISAJson_decoder), map(_arrow1550, Data_ISAJson_decoder), map(_arrow1551, Material_ISAJson_decoder)]))
...
def ARCtrl_Process_ProcessInput__ProcessInput_toISAJsonString_Static_Z3B036AA(spaces: int | None=None, use_idreferencing: bool | None=None) -> Callable[[ProcessInput], str]:
    ...
    def _arrow1552(f: ProcessInput, spaces: Any=spaces, use_idreferencing: Any=use_idreferencing) -> str:
