...
def Sample_ROCrate_encoder(oa: Sample) -> Json:
    ...
    def _arrow1519(value_4: str, oa: Any=oa) -> Json:
        return Json(0, value_4)

    def _arrow1520(x: Json, oa: Any=oa) -> Json:
        return x

    return Json(5, choose(chooser, of_array([("@id", Json(0, Sample_ROCrate_genID(oa))), ("@type", list_1_2(singleton(Json(0, "Sample")))), try_include("name", _arrow1519, oa.Name), try_include_list("additionalProperties", _arrow1520, additional_properties), ("@context", context_jsonvalue)])))


class ObjectExpr1522(Decoder_1[tuple[MaterialAttributeValue | None, FactorValue | None]]):
    def Decode(self, s: IDecoderHelpers_1[__A_], json: __A_) -> FSharpResult_2[tuple[MaterialAttributeValue | None, FactorValue | None], tuple[str, ErrorReason_1[__A_]]]:
        ...
        if (_arrow1521() if s.has_property("additionalType", json) else "") == "FactorValue":
...
Sample_ROCrate_additionalPropertyDecoder: Decoder_1[tuple[MaterialAttributeValue | None, FactorValue | None]] = ObjectExpr1522()

def _arrow1526(get: IGetters) -> Sample:
    ...
    def _arrow1525(__unit: None=None) -> FSharpList[Source] | None:
        arg_7: Decoder_1[FSharpList[Source]] = list_1_3(Source_ROCrate_decoder)
        object_arg_3: IOptionalGetter = get.Optional
        return object_arg_3.Field("derivesFrom", arg_7)

    return Sample(_arrow1523(), _arrow1524(), pattern_input[0], pattern_input[1], _arrow1525())


Sample_ROCrate_decoder: Decoder_1[Sample] = object(_arrow1526)
...
def Sample_ISAJson_encoder(id_map: Any | None, oa: Sample) -> Json:
    def f(oa_1: Sample, id_map: Any=id_map, oa: Any=oa) -> Json:
        ...
        def _arrow1528(value: str, oa_1: Any=oa_1) -> Json:
            return Json(0, value)

        def _arrow1529(value_2: str, oa_1: Any=oa_1) -> Json:
            return Json(0, value_2)

        def _arrow1530(oa_2: MaterialAttributeValue, oa_1: Any=oa_1) -> Json:
            return MaterialAttributeValue_ISAJson_encoder(id_map, oa_2)

        def _arrow1531(fv: FactorValue, oa_1: Any=oa_1) -> Json:
            return FactorValue_ISAJson_encoder(id_map, fv)

        def _arrow1532(oa_3: Source, oa_1: Any=oa_1) -> Json:
            return Source_ISAJson_encoder(id_map, oa_3)

        return Json(5, choose(chooser, of_array([try_include("@id", _arrow1528, Sample_ROCrate_genID(oa_1)), try_include("name", _arrow1529, oa_1.Name), try_include_list_opt("characteristics", _arrow1530, oa_1.Characteristics), try_include_list_opt("factorValues", _arrow1531, oa_1.FactorValues), try_include_list_opt("derivesFrom", _arrow1532, oa_1.DerivesFrom)])))

    if id_map is not None:
        def _arrow1533(s_1: Sample, id_map: Any=id_map, oa: Any=oa) -> str:
            return Sample_ROCrate_genID(s_1)

        return encode(_arrow1533, f, oa, id_map)
...
def _arrow1539(get: IGetters) -> Sample:
    ...
    def _arrow1536(__unit: None=None) -> FSharpList[MaterialAttributeValue] | None:
    ...
    def _arrow1537(__unit: None=None) -> FSharpList[FactorValue] | None:
        arg_7: Decoder_1[FSharpList[FactorValue]] = list_1_3(FactorValue_ISAJson_decoder)
        object_arg_3: IOptionalGetter = get.Optional
        return object_arg_3.Field("factorValues", arg_7)

    def _arrow1538(__unit: None=None) -> FSharpList[Source] | None:
        arg_9: Decoder_1[FSharpList[Source]] = list_1_3(Source_ISAJson_decoder)
        object_arg_4: IOptionalGetter = get.Optional
        return object_arg_4.Field("derivesFrom", arg_9)

    return Sample(_arrow1534(), _arrow1535(), _arrow1536(), _arrow1537(), _arrow1538())


Sample_ISAJson_decoder: Decoder_1[Sample] = Decode_objectNoAdditionalProperties(Sample_ISAJson_allowedFields, _arrow1539)
...
def ARCtrl_Process_Sample__Sample_toISAJsonString_Static_Z3B036AA(spaces: int | None=None, use_idreferencing: bool | None=None) -> Callable[[Sample], str]:
    ...
    def _arrow1540(f: Sample, spaces: Any=spaces, use_idreferencing: Any=use_idreferencing) -> str:
...
def ARCtrl_Process_Sample__Sample_toROCrateString_Static_71136F3F(spaces: int | None=None) -> Callable[[Sample], str]:
    def _arrow1541(f: Sample, spaces: Any=spaces) -> str:
