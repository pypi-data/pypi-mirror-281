    ...
    def create_at(index: int, cells: FsCellsCollection) -> FsColumn:
        def get_index_by(f: Callable[[Callable[[FsCell], int], IEnumerable_1[FsCell]], FsCell]) -> int:
            if length(FsCellsCollection__GetCellsInColumn_Z524259A4(cells, index)) == 0:
                ...
                def _arrow147(c: FsCell, f: Any=f) -> int:
                    return FsAddress__get_RowNumber(c.Address)

                return FsAddress__get_RowNumber(f(_arrow147)(FsCellsCollection__GetCellsInColumn_Z524259A4(cells, index)).Address)


        def _arrow150(projection: Callable[[FsCell], int]) -> Callable[[IEnumerable_1[FsCell]], FsCell]:
            def _arrow149(source_1: IEnumerable_1[FsCell]) -> FsCell:
                class ObjectExpr148:
                    @property
                    def Compare(self) -> Callable[[int, int], int]:
                        return compare_primitives

                return min_by(projection, source_1, ObjectExpr148())

            return _arrow149

        min_row_index: int = get_index_by(_arrow150) or 0
        def _arrow153(projection_1: Callable[[FsCell], int]) -> Callable[[IEnumerable_1[FsCell]], FsCell]:
            def _arrow152(source_2: IEnumerable_1[FsCell]) -> FsCell:
                class ObjectExpr151:
                    @property
                    def Compare(self) -> Callable[[int, int], int]:
                        return compare_primitives

                return max_by(projection_1, source_2, ObjectExpr151())

            return _arrow152

        max_row_index: int = get_index_by(_arrow153) or 0
