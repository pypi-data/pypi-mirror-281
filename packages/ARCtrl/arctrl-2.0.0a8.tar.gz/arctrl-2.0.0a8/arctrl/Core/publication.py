...
class Publication:
    ...
    def __str__(self, __unit: None=None) -> str:
        ...
        def _arrow345(__unit: None=None) -> str | None:
            option: OntologyAnnotation | None = this.Status
            def _arrow344(__unit: None=None) -> Callable[[OntologyAnnotation], str]:
                clo: Callable[[OntologyAnnotation], str] = to_text(printf("%A"))
                return clo

            return map_1(_arrow344(), option)

        def _arrow346(__unit: None=None) -> str:
            arg_1: Array[Comment] = this.Comments
            return to_text(printf("%A"))(arg_1)

        ignore(StringBuilder__Append_Z721C83C5(sb, join(",\n\t", map(mapping_2, choose(chooser, of_array([("PubMedID", this.PubMedID), ("DOI", this.DOI), ("Authors", this.Authors), ("Title", this.Title), ("Status", _arrow345()), ("Comments", _arrow346() if (length(this.Comments) > 0) else None)]))))))
        ignore(StringBuilder__Append_Z721C83C5(sb, "\n}"))
        return to_string(sb)


Publication_reflection = _expr347
