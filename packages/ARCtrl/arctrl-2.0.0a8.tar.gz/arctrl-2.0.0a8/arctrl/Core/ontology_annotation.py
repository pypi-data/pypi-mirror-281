...
class OntologyAnnotation:
    ...
    def to_string_object(oa: OntologyAnnotation, as_ontobee_purl_url_if_short: bool | None=None) -> dict[str, Any]:
        ...
        return {
            "TermAccessionNumber": _arrow324() if as_ontobee_purl_url_if_short_1 else default_arg(oa.TermAccessionNumber, ""),
            "TermName": TermName,
            ...
            def _arrow331(__unit: None=None) -> IEnumerable_1[str]:
                ...
                def _arrow330(__unit: None=None) -> IEnumerable_1[str]:
                    ...
                    def _arrow329(__unit: None=None) -> IEnumerable_1[str]:
                        ...
                        return singleton(_arrow328()) if (len(this.Comments) != 0) else empty()

                    return append(singleton(_arrow327()) if (this.TermAccessionNumber is not None) else empty(), delay(_arrow329))

                return append(singleton(_arrow326()) if (this.TermSourceREF is not None) else empty(), delay(_arrow330))

            return append(singleton(_arrow325()) if (this.Name is not None) else empty(), delay(_arrow331))

        ignore(StringBuilder__Append_Z721C83C5(sb, join("; ", to_list(delay(_arrow332)))))
        ignore(StringBuilder__Append_Z721C83C5(sb, "}"))
        return to_string(sb)
        ...
        def _arrow336(__unit: None=None) -> IEnumerable_1[Any]:
            def _arrow335(__unit: None=None) -> IEnumerable_1[Any]:
                ...
                if matchValue is not None:
                    ...
                    return _arrow333() if (matchValue_1 is None) else _arrow334()
            ...
            return append(singleton(box_hash_option(this.Name)), delay(_arrow335))

        return box_hash_array(to_array(delay(_arrow336)))
...
OntologyAnnotation_reflection = _expr337
