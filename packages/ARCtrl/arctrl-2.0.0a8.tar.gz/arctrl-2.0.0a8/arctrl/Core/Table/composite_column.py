...
class CompositeColumn(Record):
    ...
    def TryGetColumnUnits(self, __unit: None=None) -> Array[OntologyAnnotation] | None:
        this: CompositeColumn = self
        def _arrow428(__unit: None=None) -> IEnumerable_1[OntologyAnnotation]:
            def _arrow427(cell: CompositeCell) -> IEnumerable_1[OntologyAnnotation]:
                return singleton(cell.AsUnitized[1]) if cell.is_unitized else empty()

            return collect(_arrow427, this.Cells)

        arr: Array[OntologyAnnotation] = to_array(delay(_arrow428))
...
CompositeColumn_reflection = _expr429
