...
Init_reflection = _expr2034

def _expr2035() -> TypeInfo:
...
def Clone_createCloneContract_5000466F(remote_url: str, merge: bool | None=None, branch: str | None=None, token: tuple[str, str] | None=None, nolfs: bool | None=None) -> Contract:
    ...
    def _arrow2041(__unit: None=None, remote_url: Any=remote_url, merge: Any=merge, branch: Any=branch, token: Any=token, nolfs: Any=nolfs) -> IEnumerable_1[str]:
        def _arrow2040(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow2039(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow2038(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow2037(__unit: None=None) -> IEnumerable_1[str]:
                        def _arrow2036(__unit: None=None) -> IEnumerable_1[str]:
                            return singleton(".") if merge_1 else empty()

                        return append(singleton(remote_url_1), delay(_arrow2036))

                    return append(singleton(value(branch)) if (branch is not None) else empty(), delay(_arrow2037))

                return append(singleton(Clone_get_branchFlag()) if (branch is not None) else empty(), delay(_arrow2038))

            return append(singleton(Clone_get_noLFSConfig()) if nolfs_1 else empty(), delay(_arrow2039))

        return append(singleton(Clone_get_clone()), delay(_arrow2040))

    return create_git_contract(to_array(delay(_arrow2041)))
