...
def _arrow1577(Item: Sample) -> ProcessOutput:
    return ProcessOutput(0, Item)


def _arrow1578(Item_1: Data) -> ProcessOutput:
    return ProcessOutput(1, Item_1)


def _arrow1579(Item_2: Material) -> ProcessOutput:
    return ProcessOutput(2, Item_2)


ProcessOutput_ROCrate_decoder: Decoder_1[ProcessOutput] = one_of(of_array([map(_arrow1577, Sample_ROCrate_decoder), map(_arrow1578, Data_ROCrate_decoder), map(_arrow1579, Material_ROCrate_decoder)]))
...
def _arrow1581(Item: Sample) -> ProcessOutput:
    return ProcessOutput(0, Item)


def _arrow1582(Item_1: Data) -> ProcessOutput:
    return ProcessOutput(1, Item_1)


def _arrow1583(Item_2: Material) -> ProcessOutput:
    return ProcessOutput(2, Item_2)


ProcessOutput_ISAJson_decoder: Decoder_1[ProcessOutput] = one_of(of_array([map(_arrow1581, Sample_ISAJson_decoder), map(_arrow1582, Data_ISAJson_decoder), map(_arrow1583, Material_ISAJson_decoder)]))
...
def ARCtrl_Process_ProcessOutput__ProcessOutput_toISAJsonString_Static_Z3B036AA(spaces: int | None=None, use_idreferencing: bool | None=None) -> Callable[[ProcessOutput], str]:
    ...
    def _arrow1584(f: ProcessOutput, spaces: Any=spaces, use_idreferencing: Any=use_idreferencing) -> str:
...
def ARCtrl_Process_ProcessOutput__ProcessOutput_toROCrateJsonString_Static_71136F3F(spaces: int | None=None) -> Callable[[ProcessOutput], str]:
    def _arrow1585(f: ProcessOutput, spaces: Any=spaces) -> str:
