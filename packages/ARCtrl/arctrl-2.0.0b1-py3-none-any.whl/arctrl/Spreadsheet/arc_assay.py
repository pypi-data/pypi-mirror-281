...
def to_metadata_sheet(assay: ArcAssay) -> FsWorksheet:
    ...
    def _arrow1037(__unit: None=None, assay: Any=assay) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
        ...
        def _arrow1036(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
            def _arrow1035(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
                def _arrow1034(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
                    ...
                    return append(singleton(SparseRowModule_fromValues(to_enumerable(["ASSAY PERFORMERS"]))), delay(_arrow1033))

                return append(to_rows_1("Assay", singleton_1(assay_1)), delay(_arrow1034))

            return append(singleton(SparseRowModule_fromValues(to_enumerable(["ASSAY"]))), delay(_arrow1035))

        return delay(_arrow1036)

    iterate_indexed(action, _arrow1037())
    return sheet
        ...
        def loop(last_line_mut: str | None, assays_mut: FSharpList[ArcAssay], contacts_mut: FSharpList[Person], line_number_mut: int) -> ArcAssay:
            while True:
                ...
                if last_line is not None:
                    def _arrow1038(__unit: None=None, last_line: Any=last_line, assays: Any=assays, contacts: Any=contacts, line_number: Any=line_number) -> bool:
                        k: str = last_line
