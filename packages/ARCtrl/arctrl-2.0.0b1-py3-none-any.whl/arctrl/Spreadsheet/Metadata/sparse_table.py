...
SparseTable_reflection = _expr804
...
def SparseTable_Create_Z2192E64B(matrix: Any | None=None, keys: FSharpList[str] | None=None, comment_keys: FSharpList[str] | None=None, length: int | None=None) -> SparseTable:
    ...
    return SparseTable(default_arg(matrix, Dictionary([], ObjectExpr805())), default_arg(keys, empty()), default_arg(comment_keys, empty()), default_arg(length, 0))
...
def SparseTable_AddRow(key: str, values: IEnumerable_1[tuple[int, str]], matrix: SparseTable) -> SparseTable:
    ...
    def _arrow806(tuple: tuple[int, str], key: Any=key, values: Any=values, matrix: Any=matrix) -> int:
        return tuple[0]
    ...
    length: int = (0 if is_empty(values) else (1 + max_by(_arrow806, values, ObjectExpr807())[0])) or 0
...
def SparseTable_AddComment(key: str, values: IEnumerable_1[tuple[int, str]], matrix: SparseTable) -> SparseTable:
    if length_1(values) == 0:
        ...
        iterate(action, values)
        def _arrow808(tuple: tuple[int, str], key: Any=key, values: Any=values, matrix: Any=matrix) -> int:
            return tuple[0]
        ...
        length: int = (0 if is_empty(values) else (1 + max_by(_arrow808, values, ObjectExpr809())[0])) or 0
                ...
                if en.System_Collections_IEnumerator_MoveNext():
                    ...
                    if pattern_matching_result == 0:
                        ...
                        if active_pattern_result_1 is not None:
                            ...
                            if vals is not None:
                                def _arrow812(__unit: None=None, matrix: Any=matrix, remarks: Any=remarks, line_number_1: Any=line_number_1) -> bool:
                                    v_2: IEnumerable_1[tuple[int, str]] = vals
                                    k_1: str = key
                                    def _arrow811(label: str) -> bool:
                                        return k_1 == (prefix_1 + label)

                                    return exists(_arrow811, labels)
                        ...
                        if pattern_matching_result_1 == 0:
                            ...
                            def _arrow810(label_1: str, matrix: Any=matrix, remarks: Any=remarks, line_number_1: Any=line_number_1) -> bool:
                                return k_3 == (prefix_1 + label_1)

                            matrix_mut = SparseTable_AddRow(find(_arrow810, labels), v_3, matrix)
                            remarks_mut = remarks
                            line_number_1_mut = line_number_1 + 1
                            continue
...
def SparseTable_ToRows_759CAFC1(matrix: SparseTable, prefix: str | None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
    prefix_1: str = "" if (prefix is None) else (prefix + " ")
    def _arrow818(__unit: None=None, matrix: Any=matrix, prefix: Any=prefix) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
        def _arrow814(key: str) -> IEnumerable_1[tuple[int, str]]:
            def _arrow813(i: int) -> str:
                return SparseTable__TryGetValueDefault_5BAE6133(matrix, "", (key, i + 1))

            return SparseRowModule_fromValues(cons(prefix_1 + key, initialize_1(matrix.ColumnCount - 1, _arrow813)))

        def _arrow817(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
            def _arrow816(key_1: str) -> IEnumerable_1[tuple[int, str]]:
                def _arrow815(i_1: int) -> str:
                    return SparseTable__TryGetValueDefault_5BAE6133(matrix, "", (key_1, i_1 + 1))

                return SparseRowModule_fromValues(cons(Comment_wrapCommentKey(key_1), initialize_1(matrix.ColumnCount - 1, _arrow815)))

            return map(_arrow816, matrix.CommentKeys)

        return append_1(map(_arrow814, matrix.Keys), delay(_arrow817))

    return delay(_arrow818)
