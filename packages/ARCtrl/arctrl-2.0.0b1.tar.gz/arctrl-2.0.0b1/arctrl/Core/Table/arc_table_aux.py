...
def get_row_count(values: Any) -> int:
    if len(values) == 0:
        ...
        return 1 + max_by(projection, values.keys(), ObjectExpr434())[1]
...
def _007CIsUniqueExistingHeader_007C__007C(existing_headers: IEnumerable_1[CompositeHeader], input: CompositeHeader) -> int | None:
    if ((((input.tag == 3) or (input.tag == 2)) or (input.tag == 1)) or (input.tag == 0)) or (input.tag == 13):
        ...
        def _arrow435(h: CompositeHeader, existing_headers: Any=existing_headers, input: Any=input) -> bool:
            return True if (h.tag == 12) else False

        return try_find_index(_arrow435, existing_headers)

    elif input.tag == 11:
        def _arrow436(h_1: CompositeHeader, existing_headers: Any=existing_headers, input: Any=input) -> bool:
            return True if (h_1.tag == 11) else False

        return try_find_index(_arrow436, existing_headers)

    else: 
        def _arrow437(h_2: CompositeHeader, existing_headers: Any=existing_headers, input: Any=input) -> bool:
            return equals(h_2, input)

        return try_find_index(_arrow437, existing_headers)
...
def SanityChecks_validateColumnIndex(index: int, column_count: int, allow_append: bool) -> None:
    ...
    def _arrow438(__unit: None=None, index: Any=index, column_count: Any=column_count, allow_append: Any=allow_append) -> bool:
        x: int = index or 0
...
def SanityChecks_validateRowIndex(index: int, row_count: int, allow_append: bool) -> None:
    ...
    def _arrow439(__unit: None=None, index: Any=index, row_count: Any=row_count, allow_append: Any=allow_append) -> bool:
        x: int = index or 0
...
def SanityChecks_validate(headers: Array[CompositeHeader], values: Any, raise_exception: bool) -> bool:
    ...
    while en.System_Collections_IEnumerator_MoveNext() if is_valid else False:
        ...
        if (cell_is_not_freetext if (not cell.is_data) else False) if header_is_data else False:
            def _arrow440(message: str, headers: Any=headers, values: Any=values, raise_exception: Any=raise_exception) -> None:
                raise Exception(message)

            def _arrow442(__unit: None=None, headers: Any=headers, values: Any=values, raise_exception: Any=raise_exception) -> Callable[[str], None]:
                clo: Callable[[str], None] = to_console(printf("%s"))
                def _arrow441(arg: str) -> None:
                    clo(arg)

                return _arrow441

            (_arrow440 if raise_exception else _arrow442())(((("Invalid combination of header `" + str(header)) + "` and cell `") + str(cell)) + "`. Data header should contain either Data or Freetext cells.")
            is_valid = False

        if cell_is_not_freetext if header_is_freetext else False:
            def _arrow443(message_1: str, headers: Any=headers, values: Any=values, raise_exception: Any=raise_exception) -> None:
                raise Exception(message_1)

            def _arrow445(__unit: None=None, headers: Any=headers, values: Any=values, raise_exception: Any=raise_exception) -> Callable[[str], None]:
                clo_1: Callable[[str], None] = to_console(printf("%s"))
                def _arrow444(arg_1: str) -> None:
                    clo_1(arg_1)

                return _arrow444

            (_arrow443 if raise_exception else _arrow445())(((("Invalid combination of header `" + str(header)) + "` and cell `") + str(cell)) + "`. Freetext header should not contain non-freetext cells.")
            is_valid = False
...
def Unchecked_addColumn(new_header: CompositeHeader, new_cells: Array[CompositeCell], index: int, force_replace: bool, only_headers: bool, headers: Array[CompositeHeader], values: Any) -> None:
    ...
    if (has_duplicate_unique is None) if (index_1 < start_col_count) else False:
        def _arrow446(x: int, y: int, new_header: Any=new_header, new_cells: Any=new_cells, index: Any=index, force_replace: Any=force_replace, only_headers: Any=only_headers, headers: Any=headers, values: Any=values) -> int:
            return compare_primitives(x, y)

        last_column_index: int = max(_arrow446, start_col_count - 1, 0) or 0
    ...
    if not only_headers:
        ...
        def _arrow448(tupled_arg: tuple[int, int, CompositeCell], new_header: Any=new_header, new_cells: Any=new_cells, index: Any=index, force_replace: Any=force_replace, only_headers: Any=only_headers, headers: Any=headers, values: Any=values) -> Callable[[Any], None]:
            def _arrow447(values_1: Any) -> None:
                value: None = add_to_dict(values_1, (tupled_arg[0], tupled_arg[1]), tupled_arg[2])
                ignore(None)

            return _arrow447

        def _arrow450(tupled_arg_1: tuple[int, int, CompositeCell], new_header: Any=new_header, new_cells: Any=new_cells, index: Any=index, force_replace: Any=force_replace, only_headers: Any=only_headers, headers: Any=headers, values: Any=values) -> Callable[[Any], None]:
            def _arrow449(cells: Any) -> None:
                Unchecked_setCellAt(tupled_arg_1[0], tupled_arg_1[1], tupled_arg_1[2], cells)

            return _arrow449

        f: Callable[[tuple[int, int, CompositeCell], Any], None] = _arrow448 if (index_1 >= start_col_count) else _arrow450
...
def Unchecked_fillMissingCells(headers: Array[CompositeHeader], values: Any) -> None:
    ...
    class ObjectExpr452:
        ...
        def Equals(self) -> Callable[[int, int], bool]:
            def _arrow451(x: int, y: int) -> bool:
                return x == y
    ...
    column_key_groups: Any = of_array(Array_groupBy(projection, to_array(values.keys()), ObjectExpr452()), ObjectExpr453())
    for column_index in range(0, (column_count - 1) + 1, 1):
        ...
        if match_value is None:
            ...
            def _arrow454(tuple_1: tuple[int, int], headers: Any=headers, values: Any=values) -> int:
                return tuple_1[1]
            ...
            row_keys: Any = of_array_1(map(_arrow454, col_2, Int32Array), ObjectExpr455())
...
def Unchecked_addRow(index: int, new_cells: Array[CompositeCell], headers: Array[CompositeHeader], values: Any) -> None:
    ...
    if index < row_count:
        def _arrow456(x: int, y: int, index: Any=index, new_cells: Any=new_cells, headers: Any=headers, values: Any=values) -> int:
            return compare_primitives(x, y)

        last_row_index: int = max(_arrow456, row_count - 1, 0) or 0
...
def Unchecked_addRows(index: int, new_rows: Array[Array[CompositeCell]], headers: Array[CompositeHeader], values: Any) -> None:
    ...
    increase_row_indices: None
    if index < row_count:
        def _arrow457(x: int, y: int, index: Any=index, new_rows: Any=new_rows, headers: Any=headers, values: Any=values) -> int:
            return compare_primitives(x, y)

        last_row_index: int = max(_arrow457, row_count - 1, 0) or 0
        for row_index in range(last_row_index, index - 1, -1):
            for column_index in range(0, (column_count - 1) + 1, 1):
...
def Unchecked_alignByHeaders(keep_order: bool, rows: FSharpList[FSharpList[tuple[CompositeHeader, CompositeCell]]]) -> tuple[Array[CompositeHeader], Any]:
    ...
    values: Any = Dictionary([], ObjectExpr458())
    def loop(col_i_mut: int, rows_2_mut: FSharpList[FSharpList[tuple[CompositeHeader, CompositeCell]]], keep_order: Any=keep_order, rows: Any=rows) -> tuple[Array[CompositeHeader], Any]:
        while True:
            ...
            def _arrow459(arg: FSharpList[tuple[CompositeHeader, CompositeCell]], col_i: Any=col_i, rows_2: Any=rows_2) -> bool:
                return not is_empty(arg)

            if not exists(_arrow459, rows_2):
                return (headers, values)

            else: 
                def _arrow460(l: FSharpList[tuple[CompositeHeader, CompositeCell]], col_i: Any=col_i, rows_2: Any=rows_2) -> tuple[CompositeHeader, CompositeCell] | None:
                    return None if is_empty(l) else head(l)

                first_elem: CompositeHeader = pick(_arrow460, rows_2)[0]
