...
def get_protocol_names(process_sequence: FSharpList[Process]) -> FSharpList[str]:
    ...
    class ObjectExpr399:
        ...
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow398(x: str, y: str) -> bool:
                return x == y
    ...
    return List_distinct(choose(chooser, process_sequence), ObjectExpr399())
...
def get_protocols(process_sequence: FSharpList[Process]) -> FSharpList[Protocol]:
    ...
    return List_distinct(choose(chooser, process_sequence), ObjectExpr400())
...
def filter_by_protocol_name(protocol_name: str, process_sequence: FSharpList[Process]) -> FSharpList[Process]:
    def _arrow401(p: Protocol, protocol_name: Any=protocol_name, process_sequence: Any=process_sequence) -> bool:
        return equals(p.Name, protocol_name)

    return filter_by_protocol_by(_arrow401, process_sequence)
...
def get_parameters(process_sequence: FSharpList[Process]) -> FSharpList[ProtocolParameter]:
    ...
    return List_distinct(collect(mapping, process_sequence), ObjectExpr402())
...
def get_characteristics(process_sequence: FSharpList[Process]) -> FSharpList[MaterialAttribute]:
    ...
    return List_distinct(collect(mapping, process_sequence), ObjectExpr403())
...
def get_factors(process_sequence: FSharpList[Process]) -> FSharpList[Factor]:
    ...
    return List_distinct(collect(mapping, process_sequence), ObjectExpr404())
...
def get_root_inputs(process_sequence: FSharpList[Process]) -> FSharpList[ProcessInput]:
    ...
    outputs: Any = of_list(collect(mapping_2, process_sequence), ObjectExpr405())
...
def get_final_outputs(process_sequence: FSharpList[Process]) -> FSharpList[ProcessOutput]:
    ...
    inputs: Any = of_list(collect(mapping_1, process_sequence), ObjectExpr406())
...
def get_root_input_of(process_sequence: FSharpList[Process], sample: str) -> FSharpList[str]:
    ...
    def mapping_2(p: Process, process_sequence: Any=process_sequence, sample: Any=sample) -> FSharpList[tuple[str, str]]:
        ...
        return List_distinct(zip(map(mapping, value_4(p.Outputs)), map(mapping_1, value_4(p.Inputs))), ObjectExpr407())

    class ObjectExpr409:
        @property
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow408(x_1: str, y_1: str) -> bool:
                return x_1 == y_1
    ...
    mappings: Any = of_list_1(map(mapping_4, List_groupBy(projection, collect(mapping_2, process_sequence), ObjectExpr409())), ObjectExpr410())
...
def get_final_outputs_of(process_sequence: FSharpList[Process], sample: str) -> FSharpList[str]:
    ...
    def mapping_2(p: Process, process_sequence: Any=process_sequence, sample: Any=sample) -> FSharpList[tuple[str, str]]:
        ...
        return List_distinct(zip(map(mapping, value_4(p.Inputs)), map(mapping_1, value_4(p.Outputs))), ObjectExpr411())

    class ObjectExpr413:
        @property
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow412(x_1: str, y_1: str) -> bool:
                return x_1 == y_1
    ...
    mappings: Any = of_list_1(map(mapping_4, List_groupBy(projection, collect(mapping_2, process_sequence), ObjectExpr413())), ObjectExpr414())
    def loop(last_state_mut: FSharpList[str], state_mut: FSharpList[str], process_sequence: Any=process_sequence, sample: Any=sample) -> FSharpList[str]:
        while True:
            (last_state, state) = (last_state_mut, state_mut)
            if equals(last_state, state):
...
def get_units(process_sequence: FSharpList[Process]) -> FSharpList[OntologyAnnotation]:
    def _arrow415(p: Process, process_sequence: Any=process_sequence) -> FSharpList[OntologyAnnotation]:
    ...
    return List_distinct(collect(_arrow415, process_sequence), ObjectExpr416())
...
def get_data(process_sequence: FSharpList[Process]) -> FSharpList[Data]:
    ...
    return List_distinct(collect(mapping, process_sequence), ObjectExpr417())
...
def get_sources(process_sequence: FSharpList[Process]) -> FSharpList[Source]:
    ...
    return List_distinct(collect(mapping, process_sequence), ObjectExpr418())
...
def get_samples(process_sequence: FSharpList[Process]) -> FSharpList[Sample]:
    ...
    return List_distinct(collect(mapping, process_sequence), ObjectExpr419())
...
def get_materials(process_sequence: FSharpList[Process]) -> FSharpList[Material]:
    ...
    return List_distinct(collect(mapping, process_sequence), ObjectExpr420())
