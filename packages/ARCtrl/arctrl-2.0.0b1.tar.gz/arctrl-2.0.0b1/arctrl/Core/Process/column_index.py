...
def try_int(str_1: str) -> int | None:
    ...
    out_arg: int = 0
    def _arrow421(__unit: None=None, str_1: Any=str_1) -> int:
        return out_arg

    def _arrow422(v: int, str_1: Any=str_1) -> None:
        nonlocal out_arg
        out_arg = v or 0

    match_value = (try_parse(str_1, 511, False, 32, FSharpRef(_arrow421, _arrow422)), out_arg)
    if match_value[0]:
...
def try_get_parameter_column_index(param_value: ProcessParameterValue) -> int | None:
    def _arrow423(param: ProtocolParameter, param_value: Any=param_value) -> int | None:
        return try_get_parameter_index(param)

    return bind(_arrow423, param_value.Category)
...
def try_get_factor_column_index(factor_value: FactorValue) -> int | None:
    def _arrow424(factor: Factor, factor_value: Any=factor_value) -> int | None:
        return try_get_factor_index(factor)

    return bind(_arrow424, factor_value.Category)
...
def try_get_characteristic_column_index(characteristic_value: MaterialAttributeValue) -> int | None:
    def _arrow425(characteristic: MaterialAttribute, characteristic_value: Any=characteristic_value) -> int | None:
        return try_get_characteristic_index(characteristic)

    return bind(_arrow425, characteristic_value.Category)
