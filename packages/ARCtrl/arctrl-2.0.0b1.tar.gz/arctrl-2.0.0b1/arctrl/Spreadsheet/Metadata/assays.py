...
def from_sparse_table(matrix: SparseTable) -> FSharpList[ArcAssay]:
    if (length(matrix.CommentKeys) != 0) if (matrix.ColumnCount == 0) else False:
        ...
        def _arrow878(i: int, matrix: Any=matrix) -> ArcAssay:
        ...
        return initialize(matrix.ColumnCount, _arrow878)
...
def to_sparse_table(assays: FSharpList[ArcAssay]) -> SparseTable:
    ...
    class ObjectExpr880:
        ...
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow879(x: str, y: str) -> bool:
                return x == y
    ...
    return SparseTable(matrix.Matrix, matrix.Keys, reverse(List_distinct(comment_keys, ObjectExpr880())), matrix.ColumnCount)
