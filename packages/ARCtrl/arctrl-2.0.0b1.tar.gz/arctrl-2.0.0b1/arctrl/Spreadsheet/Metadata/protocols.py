...
def from_sparse_table(matrix: SparseTable) -> FSharpList[Protocol]:
    if (length(matrix.CommentKeys) != 0) if (matrix.ColumnCount == 0) else False:
        ...
        def _arrow869(i: int, matrix: Any=matrix) -> Protocol:
        ...
        return initialize(matrix.ColumnCount, _arrow869)
...
def to_sparse_table(protocols: FSharpList[Protocol]) -> SparseTable:
    ...
    class ObjectExpr871:
        ...
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow870(x: str, y: str) -> bool:
                return x == y
    ...
    return SparseTable(matrix.Matrix, matrix.Keys, reverse(List_distinct(comment_keys, ObjectExpr871())), matrix.ColumnCount)
