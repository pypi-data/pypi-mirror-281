...
def ArcStudy_toMetadataSheet(study: ArcStudy, assays: FSharpList[ArcAssay] | None=None) -> FsWorksheet:
    ...
    def _arrow1039(__unit: None=None, study: Any=study, assays: Any=assays) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
    ...
    iterate_indexed(action, _arrow1039())
    return sheet


def ArcStudy_fromMetadataSheet(sheet: FsWorksheet) -> tuple[ArcStudy, FSharpList[ArcAssay]]:
    try: 
        def _arrow1040(__unit: None=None) -> tuple[ArcStudy, FSharpList[ArcAssay]] | None:
        ...
        return default_arg(_arrow1040(), (ArcStudy.create(create_missing_identifier()), empty()))
