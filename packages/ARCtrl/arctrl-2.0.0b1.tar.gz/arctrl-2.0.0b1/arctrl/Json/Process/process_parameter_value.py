...
def _arrow1531(category: OntologyAnnotation | None=None, value: Value | None=None, unit: OntologyAnnotation | None=None) -> ProcessParameterValue:
    return ProcessParameterValue.create_as_pv(category, value, unit)


ProcessParameterValue_ROCrate_decoder: Decoder_1[ProcessParameterValue] = decoder_1(_arrow1531)
...
def ProcessParameterValue_ISAJson_encoder(id_map: Any | None, oa: ProcessParameterValue) -> Json:
    ...
    def _arrow1533(value: ProtocolParameter, id_map: Any=id_map, oa: Any=oa) -> Json:
        return ProtocolParameter_ISAJson_encoder(id_map, value)

    def _arrow1534(value_1: Value, id_map: Any=id_map, oa: Any=oa) -> Json:
        return Value_ISAJson_encoder(id_map, value_1)

    def _arrow1535(oa_1: OntologyAnnotation, id_map: Any=id_map, oa: Any=oa) -> Json:
        return OntologyAnnotation_ISAJson_encoder(id_map, oa_1)

    return Json(5, choose(chooser, of_array([try_include("category", _arrow1533, oa.Category), try_include("value", _arrow1534, oa.Value), try_include("unit", _arrow1535, oa.Unit)])))


def _arrow1539(get: IGetters) -> ProcessParameterValue:
    def _arrow1536(__unit: None=None) -> ProtocolParameter | None:
        object_arg: IOptionalGetter = get.Optional
        return object_arg.Field("category", ProtocolParameter_ISAJson_decoder)

    def _arrow1537(__unit: None=None) -> Value | None:
    ...
    return ProcessParameterValue(_arrow1536(), _arrow1537(), _arrow1538())


ProcessParameterValue_ISAJson_decoder: Decoder_1[ProcessParameterValue] = object(_arrow1539)
...
def ARCtrl_Process_ProcessParameterValue__ProcessParameterValue_toISAJsonString_Static_Z3B036AA(spaces: int | None=None, use_idreferencing: bool | None=None) -> Callable[[ProcessParameterValue], str]:
    ...
    def _arrow1540(f: ProcessParameterValue, spaces: Any=spaces, use_idreferencing: Any=use_idreferencing) -> str:
...
def ARCtrl_Process_ProcessParameterValue__ProcessParameterValue_toROCrateJsonString_Static_71136F3F(spaces: int | None=None) -> Callable[[ProcessParameterValue], str]:
    def _arrow1541(f: ProcessParameterValue, spaces: Any=spaces) -> str:
