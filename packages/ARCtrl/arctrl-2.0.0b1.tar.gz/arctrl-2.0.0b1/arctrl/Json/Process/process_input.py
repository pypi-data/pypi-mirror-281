...
def _arrow1566(Item: Source) -> ProcessInput:
    return ProcessInput(0, Item)


def _arrow1567(Item_1: Sample) -> ProcessInput:
    return ProcessInput(1, Item_1)


def _arrow1568(Item_2: Data) -> ProcessInput:
    return ProcessInput(2, Item_2)


def _arrow1569(Item_3: Material) -> ProcessInput:
    return ProcessInput(3, Item_3)


ProcessInput_ROCrate_decoder: Decoder_1[ProcessInput] = one_of(of_array([map(_arrow1566, Source_ROCrate_decoder), map(_arrow1567, Sample_ROCrate_decoder), map(_arrow1568, Data_ROCrate_decoder), map(_arrow1569, Material_ROCrate_decoder)]))
...
def _arrow1571(Item: Source) -> ProcessInput:
    return ProcessInput(0, Item)


def _arrow1572(Item_1: Sample) -> ProcessInput:
    return ProcessInput(1, Item_1)


def _arrow1573(Item_2: Data) -> ProcessInput:
    return ProcessInput(2, Item_2)


def _arrow1574(Item_3: Material) -> ProcessInput:
    return ProcessInput(3, Item_3)


ProcessInput_ISAJson_decoder: Decoder_1[ProcessInput] = one_of(of_array([map(_arrow1571, Source_ISAJson_decoder), map(_arrow1572, Sample_ISAJson_decoder), map(_arrow1573, Data_ISAJson_decoder), map(_arrow1574, Material_ISAJson_decoder)]))
...
def ARCtrl_Process_ProcessInput__ProcessInput_toISAJsonString_Static_Z3B036AA(spaces: int | None=None, use_idreferencing: bool | None=None) -> Callable[[ProcessInput], str]:
    ...
    def _arrow1575(f: ProcessInput, spaces: Any=spaces, use_idreferencing: Any=use_idreferencing) -> str:
