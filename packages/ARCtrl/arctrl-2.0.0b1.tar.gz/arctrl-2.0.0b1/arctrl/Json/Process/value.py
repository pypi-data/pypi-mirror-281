...
def _arrow1207(Item: int) -> Value:
    return Value(1, Item)


def _arrow1208(Item_1: float) -> Value:
    return Value(2, Item_1)


def _arrow1209(Item_2: OntologyAnnotation) -> Value:
    return Value(0, Item_2)


def _arrow1210(Item_3: str) -> Value:
    return Value(3, Item_3)


Value_ISAJson_decoder: Decoder_1[Value] = one_of(of_array([map(_arrow1207, int_1), map(_arrow1208, float_1), map(_arrow1209, OntologyAnnotation_ISAJson_decoder), map(_arrow1210, string)]))
...
def ARCtrl_Process_Value__Value_toISAJsonString_Static_71136F3F(spaces: int | None=None) -> Callable[[Value], str]:
    def _arrow1211(v: Value, spaces: Any=spaces) -> str:
