...
def _arrow1648(get: IGetters) -> CompositeCell:
    match_value: str
    ...
    def _arrow1644(__unit: None=None) -> OntologyAnnotation:
    ...
    def _arrow1647(__unit: None=None) -> Data:
    ...
    return CompositeCell(1, _arrow1643()) if (match_value == "FreeText") else (CompositeCell(0, _arrow1644()) if (match_value == "Term") else (CompositeCell(2, _arrow1645(), _arrow1646()) if (match_value == "Unitized") else (CompositeCell(3, _arrow1647()) if (match_value == "Data") else to_fail(printf("Error reading CompositeCell from json string: %A"))(match_value))))


CompositeCell_decoder: Decoder_1[CompositeCell] = object(_arrow1648)
...
def CompositeCell_decoderCompressed(string_table: Array[str], oa_table: Array[OntologyAnnotation]) -> Decoder_1[CompositeCell]:
    def _arrow1654(get: IGetters, string_table: Any=string_table, oa_table: Any=oa_table) -> CompositeCell:
        match_value: str
        arg_1: Decoder_1[str] = decode_string(string_table)
        object_arg: IRequiredGetter = get.Required
        match_value = object_arg.Field("t", arg_1)
        def _arrow1649(__unit: None=None) -> str:
            arg_3: Decoder_1[str] = index(0, decode_string(string_table))
            object_arg_1: IRequiredGetter = get.Required
            return object_arg_1.Field("v", arg_3)

        def _arrow1650(__unit: None=None) -> OntologyAnnotation:
        ...
        def _arrow1652(__unit: None=None) -> OntologyAnnotation:
            arg_9: Decoder_1[OntologyAnnotation] = index(1, decode_oa(oa_table))
            object_arg_4: IRequiredGetter = get.Required
            return object_arg_4.Field("v", arg_9)

        def _arrow1653(__unit: None=None) -> Data:
            arg_11: Decoder_1[Data] = index(0, Data_compressedDecoder(string_table))
            object_arg_5: IRequiredGetter = get.Required
            return object_arg_5.Field("v", arg_11)

        return CompositeCell(1, _arrow1649()) if (match_value == "FreeText") else (CompositeCell(0, _arrow1650()) if (match_value == "Term") else (CompositeCell(2, _arrow1651(), _arrow1652()) if (match_value == "Unitized") else (CompositeCell(3, _arrow1653()) if (match_value == "Data") else to_fail(printf("Error reading CompositeCell from json string: %A"))(match_value))))

    return object(_arrow1654)
...
def ARCtrl_CompositeCell__CompositeCell_toJsonString_Static_71136F3F(spaces: int | None=None) -> Callable[[CompositeCell], str]:
    def _arrow1655(obj: CompositeCell, spaces: Any=spaces) -> str:
