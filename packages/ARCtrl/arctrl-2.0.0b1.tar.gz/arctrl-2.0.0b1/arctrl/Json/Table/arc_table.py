...
def ArcTable_encoder(table: ArcTable) -> Json:
    def _arrow1674(__unit: None=None, table: Any=table) -> IEnumerable_1[tuple[str, Json]]:
        def _arrow1673(__unit: None=None) -> IEnumerable_1[tuple[str, Json]]:
            def _arrow1666(__unit: None=None) -> IEnumerable_1[Json]:
            ...
            def _arrow1672(__unit: None=None) -> IEnumerable_1[tuple[str, Json]]:
                def key_encoder(tupled_arg: tuple[int, int]) -> Json:
                    def _arrow1667(value: int, tupled_arg: Any=tupled_arg) -> Json:
                        return Json(7, int(value+0x100000000 if value < 0 else value))

                    def _arrow1668(value_2: int, tupled_arg: Any=tupled_arg) -> Json:
                        return Json(7, int(value_2+0x100000000 if value_2 < 0 else value_2))

                    return tuple2(_arrow1667, _arrow1668, tupled_arg[0], tupled_arg[1])

                def _arrow1670(__unit: None=None) -> IEnumerable_1[tuple[tuple[int, int], CompositeCell]]:
                    def _arrow1669(match_value: Any) -> IEnumerable_1[tuple[tuple[int, int], CompositeCell]]:
                    ...
                    return collect(_arrow1669, table.Values)

                class ObjectExpr1671:
                    @property
                    def Compare(self) -> Callable[[tuple[int, int], tuple[int, int]], int]:
                        return compare_arrays

                return singleton(("values", map_1(key_encoder, CompositeCell_encoder, of_seq(to_list(delay(_arrow1670)), ObjectExpr1671())))) if (len(table.Values) != 0) else empty()

            return append(singleton(("header", list_1(to_list(delay(_arrow1666))))) if (len(table.Headers) != 0) else empty(), delay(_arrow1672))

        return append(singleton(("name", Json(0, table.Name))), delay(_arrow1673))

    return Json(5, to_list(delay(_arrow1674)))


def _arrow1680(get: IGetters) -> ArcTable:
    def _arrow1675(__unit: None=None) -> FSharpList[CompositeHeader] | None:
        arg_1: Decoder_1[FSharpList[CompositeHeader]] = list_1_1(CompositeHeader_decoder)
        object_arg: IOptionalGetter = get.Optional
        return object_arg.Field("header", arg_1)

    decoded_header: Array[CompositeHeader] = list(default_arg(_arrow1675(), empty_1()))
    def _arrow1676(__unit: None=None) -> Any | None:
        arg_3: Decoder_1[Any] = map_0027(tuple2_1(int_1, int_1), CompositeCell_decoder)
        object_arg_1: IOptionalGetter = get.Optional
        return object_arg_1.Field("values", arg_3)

    class ObjectExpr1677:
        @property
        def Compare(self) -> Callable[[tuple[int, int], tuple[int, int]], int]:
            return compare_arrays

    class ObjectExpr1678:
        @property
    ...
    decoded_values: Any = Dictionary(default_arg(_arrow1676(), empty_2(ObjectExpr1677())), ObjectExpr1678())
    def _arrow1679(__unit: None=None) -> str:
        object_arg_2: IRequiredGetter = get.Required
        return object_arg_2.Field("name", string)

    return ArcTable.create(_arrow1679(), decoded_header, decoded_values)


ArcTable_decoder: Decoder_1[ArcTable] = object(_arrow1680)
...
def ArcTable_encoderCompressedColumn(column_index: int, row_count: int, cell_table: Any, table: ArcTable) -> Json:
    if True if table.Headers[column_index].IsIOType else (row_count < 100):
        def _arrow1682(__unit: None=None, column_index: Any=column_index, row_count: Any=row_count, cell_table: Any=cell_table, table: Any=table) -> IEnumerable_1[Json]:
            def _arrow1681(r: int) -> Json:
                return encode_cell(cell_table, get_item_from_dict(table.Values, (column_index, r)))

            return map(_arrow1681, range_big_int(0, 1, row_count - 1))

        return Json(6, to_array(delay(_arrow1682)))

    else: 
        current: CompositeCell = get_item_from_dict(table.Values, (column_index, 0))
        from_: int = 0
        def _arrow1690(__unit: None=None, column_index: Any=column_index, row_count: Any=row_count, cell_table: Any=cell_table, table: Any=table) -> IEnumerable_1[Json]:
            def _arrow1686(i: int) -> IEnumerable_1[Json]:
                next_1: CompositeCell = get_item_from_dict(table.Values, (column_index, i))
                def _arrow1683(__unit: None=None) -> Json:
                    value: int = from_ or 0
                    return Json(7, int(value+0x100000000 if value < 0 else value))

                def _arrow1684(__unit: None=None) -> Json:
                ...
                return append(singleton(Json(5, to_enumerable([("f", _arrow1683()), ("t", _arrow1684()), ("v", encode_cell(cell_table, current))]))), delay(_arrow1685)) if (not equals(next_1, current)) else empty()

            def _arrow1689(__unit: None=None) -> IEnumerable_1[Json]:
                def _arrow1687(__unit: None=None) -> Json:
                    value_2: int = from_ or 0
                    return Json(7, int(value_2+0x100000000 if value_2 < 0 else value_2))

                def _arrow1688(__unit: None=None) -> Json:
                    value_3: int = (row_count - 1) or 0
                    return Json(7, int(value_3+0x100000000 if value_3 < 0 else value_3))

                return singleton(Json(5, to_enumerable([("f", _arrow1687()), ("t", _arrow1688()), ("v", encode_cell(cell_table, current))])))

            return append(collect(_arrow1686, range_big_int(1, 1, row_count - 1)), delay(_arrow1689))

        return Json(6, to_array(delay(_arrow1690)))



def ArcTable_decoderCompressedColumn(cell_table: Array[CompositeCell], table: ArcTable, column_index: int) -> Decoder_1[None]:
    class ObjectExpr1692(Decoder_1[None]):
        def Decode(self, helper: IDecoderHelpers_1[__A_], column: __A_, cell_table: Any=cell_table, table: Any=table, column_index: Any=column_index) -> FSharpResult_2[None, tuple[str, ErrorReason_1[__A_]]]:
            ...
            if match_value.tag == 1:
                def _arrow1691(get: IGetters) -> None:
                    from_: int
                ...
                range_decoder: Decoder_1[None] = object(_arrow1691)
...
def ArcTable_arrayi(decoderi: Callable[[int], Decoder_1[_VALUE]]) -> Decoder_1[Array[_VALUE]]:
    class ObjectExpr1694(Decoder_1[Array[_VALUE_]]):
                ...
                def folder(acc: FSharpResult_2[Array[_VALUE_], tuple[str, ErrorReason_1[__A_]]], value_1: __A_) -> FSharpResult_2[Array[_VALUE_], tuple[str, ErrorReason_1[__A_]]]:
                    ...
                    if acc.tag == 0:
                        ...
                        if match_value.tag == 0:
                            ...
                            def _arrow1693(__unit: None=None, acc: Any=acc, value_1: Any=value_1) -> tuple[str, ErrorReason_1[__A_]]:
                            ...
                            return FSharpResult_2(1, _arrow1693())
...
def ArcTable_encoderCompressed(string_table: Any, oa_table: Any, cell_table: Any, table: ArcTable) -> Json:
    def _arrow1700(__unit: None=None, string_table: Any=string_table, oa_table: Any=oa_table, cell_table: Any=cell_table, table: Any=table) -> IEnumerable_1[tuple[str, Json]]:
        def _arrow1699(__unit: None=None) -> IEnumerable_1[tuple[str, Json]]:
            def _arrow1695(__unit: None=None) -> IEnumerable_1[Json]:
                return map(CompositeHeader_encoder, table.Headers)

            def _arrow1698(__unit: None=None) -> IEnumerable_1[tuple[str, Json]]:
                if len(table.Values) != 0:
                    row_count: int = table.RowCount or 0
                    def _arrow1697(__unit: None=None) -> IEnumerable_1[Json]:
                        def _arrow1696(c: int) -> Json:
                            return ArcTable_encoderCompressedColumn(c, row_count, cell_table, table)

                        return map(_arrow1696, range_big_int(0, 1, table.ColumnCount - 1))

                    return singleton(("c", Json(6, to_array(delay(_arrow1697)))))

                else: 
                    return empty()


            return append(singleton(("h", list_1(to_list(delay(_arrow1695))))) if (len(table.Headers) != 0) else empty(), delay(_arrow1698))

        return append(singleton(("n", encode_string(string_table, table.Name))), delay(_arrow1699))

    return Json(5, to_list(delay(_arrow1700)))
...
def ArcTable_decoderCompressed(string_table: Array[str], oa_table: Array[OntologyAnnotation], cell_table: Array[CompositeCell]) -> Decoder_1[ArcTable]:
    def _arrow1706(get: IGetters, string_table: Any=string_table, oa_table: Any=oa_table, cell_table: Any=cell_table) -> ArcTable:
        def _arrow1701(__unit: None=None) -> FSharpList[CompositeHeader] | None:
            arg_1: Decoder_1[FSharpList[CompositeHeader]] = list_1_1(CompositeHeader_decoder)
            object_arg: IOptionalGetter = get.Optional
            return object_arg.Field("h", arg_1)

        decoded_header: Array[CompositeHeader] = list(default_arg(_arrow1701(), empty_1()))
        def _arrow1702(__unit: None=None) -> str:
            arg_3: Decoder_1[str] = decode_string(string_table)
            object_arg_1: IRequiredGetter = get.Required
            return object_arg_1.Field("n", arg_3)

        class ObjectExpr1703:
            @property
            def Equals(self) -> Callable[[tuple[int, int], tuple[int, int]], bool]:
                return equal_arrays
        ...
        table: ArcTable = ArcTable.create(_arrow1702(), decoded_header, Dictionary([], ObjectExpr1703()))
        def _arrow1705(__unit: None=None) -> Array[None] | None:
            def _arrow1704(column_index: int) -> Decoder_1[None]:
                return ArcTable_decoderCompressedColumn(cell_table, table, column_index)

            arg_5: Decoder_1[Array[None]] = ArcTable_arrayi(_arrow1704)
...
def ARCtrl_ArcTable__ArcTable_toJsonString_Static_71136F3F(spaces: int | None=None) -> Callable[[ArcTable], str]:
    def _arrow1707(obj: ArcTable, spaces: Any=spaces) -> str:
...
def ARCtrl_ArcTable__ArcTable_fromCompressedJsonString_Static_Z721C83C5(json_string: str) -> ArcTable:
    def _arrow1709(get: IGetters, json_string: Any=json_string) -> ArcTable:
        ...
        def _arrow1708(__unit: None=None) -> Array[CompositeCell]:
            arg_5: Decoder_1[Array[CompositeCell]] = decoder_4(string_table, oa_table)
            object_arg_2: IRequiredGetter = get.Required
            return object_arg_2.Field("cellTable", arg_5)

        arg_7: Decoder_1[ArcTable] = ArcTable_decoderCompressed(string_table, oa_table, _arrow1708())
        object_arg_3: IRequiredGetter = get.Required
        return object_arg_3.Field("table", arg_7)

    match_value: FSharpResult_2[ArcTable, str] = Decode_fromString(object(_arrow1709), json_string)
    if match_value.tag == 1:
        raise Exception(to_text(printf("Error decoding string: %O"))(match_value.fields[0]))
...
def ARCtrl_ArcTable__ArcTable_ToCompressedJsonString_71136F3F(this: ArcTable, spaces: int | None=None) -> str:
    ...
    oa_table: Any = Dictionary([], ObjectExpr1710())
    class ObjectExpr1711:
        @property
    ...
    cell_table: Any = Dictionary([], ObjectExpr1711())
...
def ARCtrl_ArcTable__ArcTable_toCompressedJsonString_Static_71136F3F(spaces: int | None=None) -> Callable[[ArcTable], str]:
    def _arrow1712(obj: ArcTable, spaces: Any=spaces) -> str:
