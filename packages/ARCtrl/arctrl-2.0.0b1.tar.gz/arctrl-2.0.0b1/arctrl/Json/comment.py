...
def Comment_encoder(comment: Comment) -> Json:
    ...
    def _arrow1110(value: str, comment: Any=comment) -> Json:
        return Json(0, value)

    def _arrow1111(value_2: str, comment: Any=comment) -> Json:
        return Json(0, value_2)

    return Json(5, choose(chooser, of_array([try_include("name", _arrow1110, comment.Name), try_include("value", _arrow1111, comment.Value)])))


def _arrow1114(get: IGetters) -> Comment:
    def _arrow1112(__unit: None=None) -> str | None:
    ...
    return Comment(_arrow1112(), _arrow1113())


Comment_decoder: Decoder_1[Comment] = object(_arrow1114)
...
def Comment_ROCrate_encoder(comment: Comment) -> Json:
    ...
    def _arrow1115(value_2: str, comment: Any=comment) -> Json:
        return Json(0, value_2)

    def _arrow1116(value_4: str, comment: Any=comment) -> Json:
        return Json(0, value_4)

    return Json(5, choose(chooser, of_array([("@id", Json(0, Comment_ROCrate_genID(comment))), ("@type", Json(0, "Comment")), try_include("name", _arrow1115, comment.Name), try_include("value", _arrow1116, comment.Value), ("@context", context_jsonvalue)])))


def _arrow1119(get: IGetters) -> Comment:
    ...
    return Comment(_arrow1117(), _arrow1118())


Comment_ROCrate_decoder: Decoder_1[Comment] = object(_arrow1119)
...
def Comment_ISAJson_encoder(id_map: Any | None, comment: Comment) -> Json:
    def f(comment_1: Comment, id_map: Any=id_map, comment: Any=comment) -> Json:
        ...
        def _arrow1121(value: str, comment_1: Any=comment_1) -> Json:
            return Json(0, value)

        def _arrow1122(value_2: str, comment_1: Any=comment_1) -> Json:
            return Json(0, value_2)

        def _arrow1123(value_4: str, comment_1: Any=comment_1) -> Json:
            return Json(0, value_4)

        return Json(5, choose(chooser, of_array([try_include("@id", _arrow1121, Comment_ROCrate_genID(comment_1)), try_include("name", _arrow1122, comment_1.Name), try_include("value", _arrow1123, comment_1.Value)])))

    if id_map is None:
        return f(comment)

    else: 
        def _arrow1124(c: Comment, id_map: Any=id_map, comment: Any=comment) -> str:
            return Comment_ROCrate_genID(c)

        return encode(_arrow1124, f, comment, id_map)
...
def ARCtrl_Comment__Comment_toJsonString_Static_71136F3F(spaces: int | None=None) -> Callable[[Comment], str]:
    def _arrow1125(c: Comment, spaces: Any=spaces) -> str:
...
def ARCtrl_Comment__Comment_toROCrateJsonString_Static_71136F3F(spaces: int | None=None) -> Callable[[Comment], str]:
    def _arrow1126(c: Comment, spaces: Any=spaces) -> str:
...
def ARCtrl_Comment__Comment_toISAJsonString_Static_71136F3F(spaces: int | None=None) -> Callable[[Comment], str]:
    def _arrow1127(c: Comment, spaces: Any=spaces) -> str:
