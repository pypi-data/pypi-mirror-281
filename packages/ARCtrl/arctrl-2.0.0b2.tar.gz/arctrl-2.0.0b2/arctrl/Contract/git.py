...
Init_reflection = _expr2056

def _expr2057() -> TypeInfo:
...
def Clone_createCloneContract_5000466F(remote_url: str, merge: bool | None=None, branch: str | None=None, token: tuple[str, str] | None=None, nolfs: bool | None=None) -> Contract:
    ...
    def _arrow2063(__unit: None=None, remote_url: Any=remote_url, merge: Any=merge, branch: Any=branch, token: Any=token, nolfs: Any=nolfs) -> IEnumerable_1[str]:
        def _arrow2062(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow2061(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow2060(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow2059(__unit: None=None) -> IEnumerable_1[str]:
                        def _arrow2058(__unit: None=None) -> IEnumerable_1[str]:
                            return singleton(".") if merge_1 else empty()

                        return append(singleton(remote_url_1), delay(_arrow2058))

                    return append(singleton(value(branch)) if (branch is not None) else empty(), delay(_arrow2059))

                return append(singleton(Clone_get_branchFlag()) if (branch is not None) else empty(), delay(_arrow2060))

            return append(singleton(Clone_get_noLFSConfig()) if nolfs_1 else empty(), delay(_arrow2061))

        return append(singleton(Clone_get_clone()), delay(_arrow2062))

    return create_git_contract(to_array(delay(_arrow2063)))
