...
from ..Spreadsheet.arc_investigation import (to_fs_workbook, from_fs_workbook)
from .contract import (Contract, DTOType, DTO)

def _007CInvestigationPath_007C__007C(input: Array[str]) -> str | None:
    (pattern_matching_result,) = (None,)
    def _arrow2055(x: str, y: str, input: Any=input) -> bool:
        return x == y

    if (len(input) == 1) if (not equals_with(_arrow2055, input, None)) else False:
...
def ARCtrl_ArcInvestigation__ArcInvestigation_ToCreateContract(this: ArcInvestigation) -> Contract:
    return Contract.create_create("isa.investigation.xlsx", DTOType(2), DTO(0, to_fs_workbook(this)))


def ARCtrl_ArcInvestigation__ArcInvestigation_ToUpdateContract(this: ArcInvestigation) -> Contract:
    return Contract.create_update("isa.investigation.xlsx", DTOType(2), DTO(0, to_fs_workbook(this)))
