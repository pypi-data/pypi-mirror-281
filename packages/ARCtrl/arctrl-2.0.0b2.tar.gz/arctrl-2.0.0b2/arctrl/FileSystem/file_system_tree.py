...
class FileSystemTree(Union):
    ...
    def try_get_child_by_name(name: str) -> Callable[[FileSystemTree], FileSystemTree | None]:
        def _arrow278(fst: FileSystemTree) -> FileSystemTree | None:
    ...
    def contains_child_with_name(name: str) -> Callable[[FileSystemTree], bool]:
        def _arrow279(fst: FileSystemTree) -> bool:
    ...
    def AddFile(self, path: str) -> FileSystemTree:
        ...
        def _arrow281(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow280(__unit: None=None) -> IEnumerable_1[str]:
                return existing_paths

            return append(singleton(path), delay(_arrow280))

        file_paths: Array[str] = to_array(delay(_arrow281))
        return FileSystemTree.from_file_paths(file_paths)

    @staticmethod
    def add_file(path: str) -> Callable[[FileSystemTree], FileSystemTree]:
        def _arrow282(tree: FileSystemTree) -> FileSystemTree:
    ...
    def from_file_paths(paths: Array[str]) -> FileSystemTree:
        def loop(paths_1: Array[Array[str]], parent: FileSystemTree) -> FileSystemTree:
            ...
            class ObjectExpr284:
                ...
                def Equals(self) -> Callable[[str, str], bool]:
                    def _arrow283(x_3: str, y_2: str) -> bool:
                        return x_3 == y_2
            ...
            folders: Array[FileSystemTree] = map(mapping_2, Array_groupBy(projection, filter(predicate_1, paths_1), ObjectExpr284()), None)
            if parent.tag == 1:
                def _arrow286(__unit: None=None, paths_1: Any=paths_1, parent: Any=parent) -> IEnumerable_1[FileSystemTree]:
                    def _arrow285(__unit: None=None) -> IEnumerable_1[FileSystemTree]:
                        return folders

                    return append(files, delay(_arrow285))

                return FileSystemTree.create_folder(parent.fields[0], to_array(delay(_arrow286)))
        ...
        class ObjectExpr289:
            ...
            def Equals(self) -> Callable[[Array[str], Array[str]], bool]:
                def _arrow288(x: Array[str], y: Array[str]) -> bool:
                    def _arrow287(x_1: str, y_1: str) -> bool:
                        return x_1 == y_1

                    return equals_with(_arrow287, x, y)
        ...
        return loop(Array_distinct(map(split, paths, None), ObjectExpr289()), FileSystemTree.create_folder(FileSystemTree.ROOT_NAME(), []))
    ...
    def to_file_paths(remove_root: bool | None=None) -> Callable[[FileSystemTree], Array[str]]:
        def _arrow290(root: FileSystemTree) -> Array[str]:
    ...
    def filter_files(predicate: Callable[[str], bool]) -> Callable[[FileSystemTree], FileSystemTree | None]:
        def _arrow291(tree: FileSystemTree) -> FileSystemTree | None:
    ...
    def filter_folders(predicate: Callable[[str], bool]) -> Callable[[FileSystemTree], FileSystemTree | None]:
        def _arrow292(tree: FileSystemTree) -> FileSystemTree | None:
    ...
    def filter(predicate: Callable[[str], bool]) -> Callable[[FileSystemTree], FileSystemTree | None]:
        def _arrow293(tree: FileSystemTree) -> FileSystemTree | None:
    ...
    def Union(self, other_fst: FileSystemTree) -> FileSystemTree:
        this: FileSystemTree = self
        def _arrow294(__unit: None=None) -> Array[str]:
        ...
        class ObjectExpr296:
            @property
            def Equals(self) -> Callable[[str, str], bool]:
                def _arrow295(x: str, y: str) -> bool:
                    return x == y
        ...
        paths: Array[str] = Array_distinct(_arrow294(), ObjectExpr296())
...
FileSystemTree_reflection = _expr297
