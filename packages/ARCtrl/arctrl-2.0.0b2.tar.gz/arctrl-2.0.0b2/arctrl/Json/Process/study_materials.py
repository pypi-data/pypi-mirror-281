...
def encoder(id_map: Any | None, ps: FSharpList[Process]) -> Json:
    ...
    def _arrow1615(oa: Source, id_map: Any=id_map, ps: Any=ps) -> Json:
        return Source_ISAJson_encoder(id_map, oa)

    def _arrow1616(oa_1: Sample, id_map: Any=id_map, ps: Any=ps) -> Json:
        return Sample_ISAJson_encoder(id_map, oa_1)

    def _arrow1617(c: Material, id_map: Any=id_map, ps: Any=ps) -> Json:
        return Material_ISAJson_encoder(id_map, c)

    return Json(5, choose(chooser, of_array([try_include_list("sources", _arrow1615, source), try_include_list("samples", _arrow1616, samples), try_include_list("otherMaterials", _arrow1617, materials)])))
