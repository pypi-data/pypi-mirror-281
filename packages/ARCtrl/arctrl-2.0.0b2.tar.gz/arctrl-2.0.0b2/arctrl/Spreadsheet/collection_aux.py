...
def Array_ofIndexedSeq(s: IEnumerable_1[tuple[int, str]]) -> Array[str]:
    def _arrow606(tuple: tuple[int, str], s: Any=s) -> int:
        return tuple[0]
    ...
    def _arrow609(i: int, s: Any=s) -> str:
        def _arrow608(arg: tuple[int, str]) -> bool:
            return i == arg[0]

        match_value: tuple[int, str] | None = try_find(_arrow608, s)
    ...
    return initialize(1 + max_by(_arrow606, s, ObjectExpr607())[0], _arrow609, None)
...
def Array_map4(f: Callable[[_A, _B, _C, _D], _T], aa: Array[_A], ba: Array[_B], ca: Array[_C], da: Array[_D]) -> Array[_T]:
    ...
    def _arrow610(i: int, f: Any=f, aa: Any=aa, ba: Any=ba, ca: Any=ca, da: Any=da) -> _T:
        return f(aa[i], ba[i], ca[i], da[i])

    return initialize(len(aa), _arrow610, None)
...
def Dictionary_tryGetValue(k: _K, dict_1: Any) -> _V | None:
    ...
    out_arg: _V = None
    def _arrow611(__unit: None=None, k: Any=k, dict_1: Any=dict_1) -> _V:
        return out_arg

    def _arrow612(v: _V | None=None, k: Any=k, dict_1: Any=dict_1) -> None:
        nonlocal out_arg
        out_arg = v
...
def Dictionary_tryGetString(k: _K, dict_1: Any) -> str | None:
    ...
    out_arg: str = None
    def _arrow613(__unit: None=None, k: Any=k, dict_1: Any=dict_1) -> str:
        return out_arg

    def _arrow614(v: str, k: Any=k, dict_1: Any=dict_1) -> None:
        nonlocal out_arg
        out_arg = v

    pattern_input = (try_get_value(dict_1, k, FSharpRef(_arrow613, _arrow614)), out_arg)
