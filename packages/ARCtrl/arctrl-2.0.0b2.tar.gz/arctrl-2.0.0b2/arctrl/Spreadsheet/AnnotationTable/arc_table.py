...
def compose_columns(string_cell_columns: Array[Array[str]]) -> Array[CompositeColumn]:
    def _arrow1010(columns: Array[Array[str]], string_cell_columns: Any=string_cell_columns) -> CompositeColumn:
        return from_string_cell_columns(columns)

    return map_1(_arrow1010, group_columns_by_header(string_cell_columns), None)
...
def try_from_fs_worksheet(sheet: FsWorksheet) -> ArcTable | None:
    try: 
        ...
        if match_value is None:
            ...
            def _arrow1014(__unit: None=None) -> IEnumerable_1[Array[str]]:
                def _arrow1013(c: int) -> Array[str]:
                    def _arrow1012(__unit: None=None) -> IEnumerable_1[str]:
                        def _arrow1011(r: int) -> IEnumerable_1[str]:
                        ...
                        return collect(_arrow1011, range_big_int(1, 1, FsAddress__get_RowNumber(FsRangeAddress__get_LastAddress(FsRangeBase__get_RangeAddress(t)))))

                    return to_array_1(delay(_arrow1012))

                return map_2(_arrow1013, range_big_int(1, 1, FsAddress__get_ColumnNumber(FsRangeAddress__get_LastAddress(FsRangeBase__get_RangeAddress(t)))))

            composite_columns: Array[CompositeColumn] = compose_columns(map_1(fix_deprecated_ioheader, to_array_1(delay(_arrow1014)), None))
...
def to_fs_worksheet(table: ArcTable) -> FsWorksheet:
    ...
    if len(table.Columns) == 0:
        ...
        def _arrow1015(column_1: CompositeColumn, table: Any=table) -> FSharpList[FSharpList[str]]:
            return to_string_cell_columns(column_1)

        def _arrow1016(column: CompositeColumn, table: Any=table) -> enum_type("ARCtrl.Spreadsheet.ArcTable.ColumnOrder", int, [("InputClass", 1.0), ("ProtocolClass", 2.0), ("ParamsClass", 3.0), ("OutputClass", 4.0)]):
        ...
        columns: FSharpList[FSharpList[str]] = collect_1(_arrow1015, sort_by(_arrow1016, of_array(table.Columns), ObjectExpr1017()))
        ...
        def action_1(col_i: int, col: FSharpList[str], table: Any=table) -> None:
