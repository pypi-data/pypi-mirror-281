...
class OntologyAnnotation:
    ...
    def to_string_object(oa: OntologyAnnotation, as_ontobee_purl_url_if_short: bool | None=None) -> dict[str, Any]:
        ...
        def _arrow343(__unit: None=None) -> str:
            url: str = oa.TermAccessionAndOntobeeUrlIfShort
            return default_arg(oa.TermAccessionNumber, "") if (url == "") else url

        return {
            "TermAccessionNumber": _arrow343() if as_ontobee_purl_url_if_short_1 else default_arg(oa.TermAccessionNumber, ""),
            "TermName": TermName,
    ...
    def __str__(self, __unit: None=None) -> str:
        ...
        def _arrow351(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow344(__unit: None=None) -> str:
                arg: str = value_4(this.Name)
                return to_text(printf("Name = %A"))(arg)

            def _arrow350(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow345(__unit: None=None) -> str:
                    arg_1: str = value_4(this.TermSourceREF)
                    return to_text(printf("TSR = %A"))(arg_1)

                def _arrow349(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow346(__unit: None=None) -> str:
                        arg_2: str = value_4(this.TermAccessionNumber)
                        return to_text(printf("TAN = %A"))(arg_2)

                    def _arrow348(__unit: None=None) -> IEnumerable_1[str]:
                        def _arrow347(__unit: None=None) -> str:
                            arg_3: Array[Comment] = this.Comments
                            return to_text(printf("Comments = %A"))(arg_3)

                        return singleton(_arrow347()) if (len(this.Comments) != 0) else empty()

                    return append(singleton(_arrow346()) if (this.TermAccessionNumber is not None) else empty(), delay(_arrow348))

                return append(singleton(_arrow345()) if (this.TermSourceREF is not None) else empty(), delay(_arrow349))

            return append(singleton(_arrow344()) if (this.Name is not None) else empty(), delay(_arrow350))

        ignore(StringBuilder__Append_Z721C83C5(sb, join("; ", to_list(delay(_arrow351)))))
        ignore(StringBuilder__Append_Z721C83C5(sb, "}"))
        return to_string(sb)
    ...
    def __hash__(self, __unit: None=None) -> Any:
        ...
        def _arrow355(__unit: None=None) -> IEnumerable_1[Any]:
            ...
            return append(singleton(box_hash_option(this.Name)), delay(_arrow354))

        return box_hash_array(to_array(delay(_arrow355)))
...
OntologyAnnotation_reflection = _expr356
