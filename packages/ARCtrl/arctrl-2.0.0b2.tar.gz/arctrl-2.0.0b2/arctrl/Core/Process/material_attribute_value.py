...
class MaterialAttributeValue(Record):
    ...
    def PrintCompact(self, __unit: None=None) -> str:
        ...
        def _arrow388(__unit: None=None) -> str:
            value_2: str = value
            return value_2

        def _arrow389(__unit: None=None) -> str:
            category_2: str = category
            return (category_2 + ":") + "No Value"

        def _arrow390(__unit: None=None) -> str:
            category_1: str = category
            value_1: str = value
            return (category_1 + ":") + value_1

        return ("" if (value is None) else _arrow388()) if (category is None) else (_arrow389() if (value is None) else _arrow390())
...
MaterialAttributeValue_reflection = _expr391
...
def MaterialAttributeValue__SetCategory_ZDED3A0F(this: MaterialAttributeValue, c: OntologyAnnotation) -> MaterialAttributeValue:
    def _arrow392(__unit: None=None, this: Any=this, c: Any=c) -> MaterialAttribute | None:
    ...
    return MaterialAttributeValue(this.ID, _arrow392(), this.Value, this.Unit)
