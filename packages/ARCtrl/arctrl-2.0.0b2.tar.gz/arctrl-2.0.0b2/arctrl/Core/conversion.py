...
def ProcessParsing_tryComponentGetter(general_i: int, value_i: int, value_header: CompositeHeader) -> Callable[[Any, int], Component] | None:
    if value_header.tag == 0:
        ...
        def Value(matrix: Any, general_i: Any=general_i, value_i: Any=value_i, value_header: Any=value_header) -> Callable[[int], Component]:
            def _arrow842(i: int, matrix: Any=matrix) -> Component:
...
def ProcessParsing_tryParameterGetter(general_i: int, value_i: int, value_header: CompositeHeader) -> Callable[[Any, int], ProcessParameterValue] | None:
    if value_header.tag == 3:
        ...
        def Value(matrix: Any, general_i: Any=general_i, value_i: Any=value_i, value_header: Any=value_header) -> Callable[[int], ProcessParameterValue]:
            def _arrow843(i_1: int, matrix: Any=matrix) -> ProcessParameterValue:
...
def ProcessParsing_tryFactorGetter(general_i: int, value_i: int, value_header: CompositeHeader) -> Callable[[Any, int], FactorValue] | None:
    if value_header.tag == 2:
        ...
        def Value(matrix: Any, general_i: Any=general_i, value_i: Any=value_i, value_header: Any=value_header) -> Callable[[int], FactorValue]:
            def _arrow844(i_1: int, matrix: Any=matrix) -> FactorValue:
...
def ProcessParsing_tryCharacteristicGetter(general_i: int, value_i: int, value_header: CompositeHeader) -> Callable[[Any, int], MaterialAttributeValue] | None:
    if value_header.tag == 1:
        ...
        def Value(matrix: Any, general_i: Any=general_i, value_i: Any=value_i, value_header: Any=value_header) -> Callable[[int], MaterialAttributeValue]:
            def _arrow845(i_1: int, matrix: Any=matrix) -> MaterialAttributeValue:
...
def ProcessParsing_tryGetProtocolTypeGetter(general_i: int, header: CompositeHeader) -> Callable[[Any, int], OntologyAnnotation] | None:
    if header.tag == 4:
        def Value(matrix: Any, general_i: Any=general_i, header: Any=header) -> Callable[[int], OntologyAnnotation]:
            def _arrow846(i: int, matrix: Any=matrix) -> OntologyAnnotation:
...
def ProcessParsing_tryGetProtocolVersionGetter(general_i: int, header: CompositeHeader) -> Callable[[Any, int], str] | None:
    if header.tag == 7:
        def Value(matrix: Any, general_i: Any=general_i, header: Any=header) -> Callable[[int], str]:
            def _arrow850(i: int, matrix: Any=matrix) -> str:
                return get_item_from_dict(matrix, (general_i, i)).AsFreeText
...
def ProcessParsing_tryGetInputGetter(general_i: int, header: CompositeHeader) -> Callable[[Any, int], ProcessInput] | None:
    if header.tag == 11:
        def Value(matrix: Any, general_i: Any=general_i, header: Any=header) -> Callable[[int], ProcessInput]:
            def _arrow851(i: int, matrix: Any=matrix) -> ProcessInput:
...
def ProcessParsing_tryGetOutputGetter(general_i: int, header: CompositeHeader) -> Callable[[Any, int], ProcessOutput] | None:
    if header.tag == 12:
        def Value(matrix: Any, general_i: Any=general_i, header: Any=header) -> Callable[[int], ProcessOutput]:
            def _arrow852(i: int, matrix: Any=matrix) -> ProcessOutput:
...
def ProcessParsing_tryGetCommentGetter(general_i: int, header: CompositeHeader) -> Callable[[Any, int], Comment] | None:
    if header.tag == 14:
        def Value(matrix: Any, general_i: Any=general_i, header: Any=header) -> Callable[[int], Comment]:
            def _arrow853(i: int, matrix: Any=matrix) -> Comment:
...
def ProcessParsing_getProcessGetter(process_name_root: str, headers: IEnumerable_1[CompositeHeader]) -> Callable[[Any, int], Process]:
    ...
    if match_value is None:
        def _arrow855(matrix_1: Any, process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[int], FSharpList[ProcessInput]]:
            def _arrow854(i_1: int) -> FSharpList[ProcessInput]:
        ...
        input_getter_1 = _arrow855

    else: 
        input_getter: Callable[[Any, int], ProcessInput] = match_value
        def _arrow857(matrix: Any, process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[int], FSharpList[ProcessInput]]:
            def _arrow856(i: int) -> FSharpList[ProcessInput]:
    ...
    if match_value_1 is None:
        def _arrow859(matrix_3: Any, process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[int], FSharpList[ProcessOutput]]:
            def _arrow858(i_3: int) -> FSharpList[ProcessOutput]:
        ...
        output_getter_1 = _arrow859

    else: 
        output_getter: Callable[[Any, int], ProcessOutput] = match_value_1
        def _arrow861(matrix_2: Any, process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[int], FSharpList[ProcessOutput]]:
            def _arrow860(i_2: int) -> FSharpList[ProcessOutput]:
        ...
        output_getter_1 = _arrow861

    def _arrow863(matrix_4: Any, process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[int], Process]:
        def _arrow862(i_4: int) -> Process:
...
def ProcessParsing_groupProcesses(ps: FSharpList[Process]) -> FSharpList[tuple[str, FSharpList[Process]]]:
    ...
    class ObjectExpr865:
        ...
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow864(x_1: str, y: str) -> bool:
                return x_1 == y
    ...
    return List_groupBy(projection, ps, ObjectExpr865())
...
def ProcessParsing_mergeIdenticalProcesses(processes: FSharpList[Process]) -> FSharpList[Process]:
    ...
    l: FSharpList[tuple[tuple[str, Any, Any | None, Any | None], FSharpList[Process]]] = List_groupBy(projection, processes, ObjectExpr866())
...
def ProcessParsing_processToRows(p: Process) -> FSharpList[FSharpList[tuple[CompositeHeader, CompositeCell]]]:
    ...
    if match_value_1 is None:
        ...
        def _arrow871(__unit: None=None, p: Any=p) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
            def _arrow870(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                def _arrow869(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                    def _arrow868(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                        ...
                        return append(singleton_1((CompositeHeader(6), CompositeCell(1, value_10(prot_1.Uri)))) if (prot_1.Uri is not None) else empty_1(), delay(_arrow867))

                    return append(singleton_1((CompositeHeader(5), CompositeCell(1, value_10(prot_1.Description)))) if (prot_1.Description is not None) else empty_1(), delay(_arrow868))

                return append(singleton_1((CompositeHeader(4), CompositeCell(0, value_10(prot_1.ProtocolType)))) if (prot_1.ProtocolType is not None) else empty_1(), delay(_arrow869))

            return append(singleton_1((CompositeHeader(8), CompositeCell(1, value_10(prot_1.Name)))) if (prot_1.Name is not None) else empty_1(), delay(_arrow870))

        prot_vals = to_list(delay(_arrow871))
    ...
    def mapping_6(tupled_arg_1: tuple[tuple[str, str], FSharpList[tuple[ProcessInput, ProcessOutput]]], p: Any=p) -> FSharpList[tuple[CompositeHeader, CompositeCell]]:
        ...
        vals: FSharpList[tuple[CompositeHeader, CompositeCell]] = map_2(mapping_5, sort_by(projection_1, append_1(chars, append_1(components, append_1(pvs, map_2(mapping_4, ProcessOutput_getFactorValues_Z42C11600(output_for_factors))))), ObjectExpr872()))
        def _arrow877(__unit: None=None, tupled_arg_1: Any=tupled_arg_1) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
            def _arrow876(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                def _arrow875(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                    def _arrow874(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                        ...
                        return append(comments, delay(_arrow873))

                    return append(vals, delay(_arrow874))

                return append(prot_vals, delay(_arrow875))

            return append(singleton_1(JsonTypes_decomposeProcessInput(input_for_type)), delay(_arrow876))

        return to_list(delay(_arrow877))
    ...
    def _arrow878(__unit: None=None, p: Any=p) -> FSharpList[tuple[ProcessInput, ProcessOutput]]:
    ...
    return map_2(mapping_6, List_groupBy(projection, _arrow878(), ObjectExpr879()))
...
def ARCtrl_ArcTable__ArcTable_GetProtocols(this: ArcTable) -> FSharpList[Protocol]:
    if this.RowCount == 0:
        def _arrow883(__unit: None=None, this: Any=this) -> Protocol:
        ...
        return singleton(_arrow883())

    else: 
        def _arrow884(i: int, this: Any=this) -> Protocol:
        ...
        return List_distinct(initialize(this.RowCount, _arrow884), ObjectExpr885())
...
def ARCtrl_ArcTable__ArcTable_GetProcesses(this: ArcTable) -> FSharpList[Process]:
    if this.RowCount == 0:
        ...
        def _arrow886(arg: Any, this: Any=this) -> Callable[[int], Process]:
            clo_1: Callable[[int], Process] = clo(arg)
            return clo_1

        getter = _arrow886
        def _arrow888(__unit: None=None, this: Any=this) -> IEnumerable_1[Process]:
            def _arrow887(i: int) -> Process:
                return getter(this.Values)(i)

            return map_1(_arrow887, range_big_int(0, 1, this.RowCount - 1))

        return ProcessParsing_mergeIdenticalProcesses(to_list(delay(_arrow888)))
