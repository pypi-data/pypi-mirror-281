...
def get_protocol_names(process_sequence: FSharpList[Process]) -> FSharpList[str]:
    ...
    class ObjectExpr421:
        ...
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow420(x: str, y: str) -> bool:
                return x == y
    ...
    return List_distinct(choose(chooser, process_sequence), ObjectExpr421())
...
def get_protocols(process_sequence: FSharpList[Process]) -> FSharpList[Protocol]:
    ...
    return List_distinct(choose(chooser, process_sequence), ObjectExpr422())
...
def filter_by_protocol_name(protocol_name: str, process_sequence: FSharpList[Process]) -> FSharpList[Process]:
    def _arrow423(p: Protocol, protocol_name: Any=protocol_name, process_sequence: Any=process_sequence) -> bool:
        return equals(p.Name, protocol_name)

    return filter_by_protocol_by(_arrow423, process_sequence)
...
def get_characteristics(process_sequence: FSharpList[Process]) -> FSharpList[MaterialAttribute]:
    ...
    return List_distinct(collect(mapping, process_sequence), ObjectExpr425())
...
def get_factors(process_sequence: FSharpList[Process]) -> FSharpList[Factor]:
    ...
    return List_distinct(collect(mapping, process_sequence), ObjectExpr426())
...
def get_root_inputs(process_sequence: FSharpList[Process]) -> FSharpList[ProcessInput]:
    ...
    outputs: Any = of_list(collect(mapping_2, process_sequence), ObjectExpr427())
...
def get_final_outputs(process_sequence: FSharpList[Process]) -> FSharpList[ProcessOutput]:
    ...
    inputs: Any = of_list(collect(mapping_1, process_sequence), ObjectExpr428())
...
def get_root_input_of(process_sequence: FSharpList[Process], sample: str) -> FSharpList[str]:
    ...
    def mapping_2(p: Process, process_sequence: Any=process_sequence, sample: Any=sample) -> FSharpList[tuple[str, str]]:
        ...
        return List_distinct(zip(map(mapping, value_4(p.Outputs)), map(mapping_1, value_4(p.Inputs))), ObjectExpr429())
    ...
    mappings: Any = of_list_1(map(mapping_4, List_groupBy(projection, collect(mapping_2, process_sequence), ObjectExpr431())), ObjectExpr432())
...
def get_final_outputs_of(process_sequence: FSharpList[Process], sample: str) -> FSharpList[str]:
    ...
    def mapping_2(p: Process, process_sequence: Any=process_sequence, sample: Any=sample) -> FSharpList[tuple[str, str]]:
        ...
        return List_distinct(zip(map(mapping, value_4(p.Inputs)), map(mapping_1, value_4(p.Outputs))), ObjectExpr433())
    ...
    mappings: Any = of_list_1(map(mapping_4, List_groupBy(projection, collect(mapping_2, process_sequence), ObjectExpr435())), ObjectExpr436())
    def loop(last_state_mut: FSharpList[str], state_mut: FSharpList[str], process_sequence: Any=process_sequence, sample: Any=sample) -> FSharpList[str]:
        while True:
            (last_state, state) = (last_state_mut, state_mut)
            if equals(last_state, state):
...
def get_units(process_sequence: FSharpList[Process]) -> FSharpList[OntologyAnnotation]:
    def _arrow437(p: Process, process_sequence: Any=process_sequence) -> FSharpList[OntologyAnnotation]:
    ...
    return List_distinct(collect(_arrow437, process_sequence), ObjectExpr438())
...
def get_data(process_sequence: FSharpList[Process]) -> FSharpList[Data]:
    ...
    return List_distinct(collect(mapping, process_sequence), ObjectExpr439())
...
def get_sources(process_sequence: FSharpList[Process]) -> FSharpList[Source]:
    ...
    return List_distinct(collect(mapping, process_sequence), ObjectExpr440())
...
def get_samples(process_sequence: FSharpList[Process]) -> FSharpList[Sample]:
    ...
    return List_distinct(collect(mapping, process_sequence), ObjectExpr441())
...
def get_materials(process_sequence: FSharpList[Process]) -> FSharpList[Material]:
    ...
    return List_distinct(collect(mapping, process_sequence), ObjectExpr442())
