...
def to_fs_columns(dc: Array[DataContext]) -> FSharpList[FSharpList[FsCell]]:
    ...
    class ObjectExpr1019:
        @property
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow1018(x: str, y: str) -> bool:
                return x == y
    ...
    comment_keys: FSharpList[str] = to_list(distinct(collect(mapping_1, dc), ObjectExpr1019()))
    ...
    def _arrow1034(__unit: None=None, dc: Any=dc) -> IEnumerable_1[FSharpList[FsCell]]:
        def _arrow1033(__unit: None=None) -> IEnumerable_1[FSharpList[FsCell]]:
            def _arrow1032(dc_4: DataContext) -> FSharpList[FsCell]:
                dc_3: DataContext = dc_4
                def _arrow1031(__unit: None=None) -> IEnumerable_1[FsCell]:
                    def _arrow1023(__unit: None=None) -> FSharpList[FsCell]:
                    ...
                    def _arrow1030(__unit: None=None) -> IEnumerable_1[FsCell]:
                            ...
                            def _arrow1028(__unit: None=None) -> IEnumerable_1[FsCell]:
                                def _arrow1027(__unit: None=None) -> IEnumerable_1[FsCell]:
                                    def _arrow1026(__unit: None=None) -> IEnumerable_1[FsCell]:
                                        ...
                                        return append(create_text(DataContext__get_GeneratedBy(dc_3)), delay(_arrow1025))

                                    return append(create_text(DataContext__get_Description(dc_3)), delay(_arrow1026))

                                return append(create_term(DataContext__get_ObjectType(dc_3)), delay(_arrow1027))

                            return append(create_term(DataContext__get_Unit(dc_3)), delay(_arrow1028))

                        return append(create_term(DataContext__get_Explication(dc_3)), delay(_arrow1029))

                    return append(_arrow1023(), delay(_arrow1030))

                return to_list(delay(_arrow1031))

            return map_1(_arrow1032, dc)

        return append(singleton_1(headers), delay(_arrow1033))

    return transpose(to_list(delay(_arrow1034)))
