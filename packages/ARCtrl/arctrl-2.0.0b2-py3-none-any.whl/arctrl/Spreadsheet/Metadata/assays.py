...
def from_sparse_table(matrix: SparseTable) -> FSharpList[ArcAssay]:
    if (length(matrix.CommentKeys) != 0) if (matrix.ColumnCount == 0) else False:
        ...
        def _arrow900(i: int, matrix: Any=matrix) -> ArcAssay:
        ...
        return initialize(matrix.ColumnCount, _arrow900)
...
def to_sparse_table(assays: FSharpList[ArcAssay]) -> SparseTable:
    ...
    class ObjectExpr902:
        ...
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow901(x: str, y: str) -> bool:
                return x == y
    ...
    return SparseTable(matrix.Matrix, matrix.Keys, reverse(List_distinct(comment_keys, ObjectExpr902())), matrix.ColumnCount)
