...
def from_sparse_table(matrix: SparseTable) -> FSharpList[Factor]:
    if (length(matrix.CommentKeys) != 0) if (matrix.ColumnCount == 0) else False:
        ...
        def _arrow880(i: int, matrix: Any=matrix) -> Factor:
        ...
        return initialize(matrix.ColumnCount, _arrow880)
...
def to_sparse_table(factors: FSharpList[Factor]) -> SparseTable:
    ...
    class ObjectExpr882:
        ...
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow881(x: str, y: str) -> bool:
                return x == y
    ...
    return SparseTable(matrix.Matrix, matrix.Keys, reverse(List_distinct(comment_keys, ObjectExpr882())), matrix.ColumnCount)
