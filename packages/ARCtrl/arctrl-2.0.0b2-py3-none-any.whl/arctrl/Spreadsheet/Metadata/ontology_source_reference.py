...
def from_sparse_table(matrix: SparseTable) -> FSharpList[OntologySourceReference]:
    if (length(matrix.CommentKeys) != 0) if (matrix.ColumnCount == 0) else False:
        ...
        def _arrow903(__unit: None=None, matrix: Any=matrix) -> OntologySourceReference:
            return_val: OntologySourceReference = OntologySourceReference.create()
            return_val.Comments = comments
            return return_val

        return singleton(_arrow903())

    else: 
        def _arrow904(i: int, matrix: Any=matrix) -> OntologySourceReference:
        ...
        return initialize(matrix.ColumnCount, _arrow904)
...
def to_sparse_table(ontology_sources: FSharpList[OntologySourceReference]) -> SparseTable:
    ...
    class ObjectExpr906:
        ...
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow905(x: str, y: str) -> bool:
                return x == y
    ...
    return SparseTable(matrix.Matrix, matrix.Keys, reverse(List_distinct(comment_keys, ObjectExpr906())), matrix.ColumnCount)
