...
TemplateReadError_reflection = _expr1091
...
Metadata_Template_TemplateInfo_reflection = _expr1092
...
def Metadata_Template_TemplateInfo_ToSparseTable_48C39BE1(template: Template) -> SparseTable:
    ...
    def _arrow1094(__unit: None=None, template: Any=template) -> str:
        copy_of_struct: str = template.Id
        return str(copy_of_struct)

    def _arrow1095(__unit: None=None, template: Any=template) -> str:
        copy_of_struct_1: str = template.Id
        return str(copy_of_struct_1)

    add_to_dict(matrix.Matrix, ("Id", 1), "" if (_arrow1094().find("MISSING_IDENTIFIER_") == 0) else _arrow1095())
    ...
    class ObjectExpr1097:
        ...
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow1096(x: str, y: str) -> bool:
                return x == y
    ...
    return SparseTable(matrix.Matrix, matrix.Keys, reverse(List_distinct(comment_keys, ObjectExpr1097())), matrix.ColumnCount)
...
def Metadata_Template_toRows(template: Template) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
    def _arrow1105(__unit: None=None, template: Any=template) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
                ...
                def _arrow1102(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
                    def _arrow1101(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
                        def _arrow1100(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
                            def _arrow1099(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
                                def _arrow1098(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
                                    return to_rows_1("Author", of_seq(template.Authors))

                                return append(singleton(SparseRowModule_fromValues(to_enumerable(["AUTHORS"]))), delay(_arrow1098))

                            return append(Metadata_Tags_toRows(None, to_list(template.Tags)), delay(_arrow1099))

                        return append(singleton(SparseRowModule_fromValues(to_enumerable(["TAGS"]))), delay(_arrow1100))

                    return append(Metadata_ER_toRows(None, to_list(template.EndpointRepositories)), delay(_arrow1101))

                return append(singleton(SparseRowModule_fromValues(to_enumerable(["ERS"]))), delay(_arrow1102))

            return append(Metadata_Template_TemplateInfo_toRows_48C39BE1(template), delay(_arrow1103))

        return append(singleton(SparseRowModule_fromValues(to_enumerable(["TEMPLATE"]))), delay(_arrow1104))

    return delay(_arrow1105)
...
def Template_fromFsWorkbook(doc: FsWorkbook) -> Template:
    ...
    def _arrow1106(__unit: None=None, doc: Any=doc) -> ArcTable:
    ...
    return Template_fromParts(template_info, pattern_input[1], pattern_input[2], pattern_input[3], _arrow1106(), now())
