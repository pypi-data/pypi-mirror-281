...
def from_sparse_table(matrix: SparseTable) -> FSharpList[Factor]:
    if (length(matrix.CommentKeys) != 0) if (matrix.ColumnCount == 0) else False:
        ...
        def _arrow740(i: int, matrix: Any=matrix) -> Factor:
        ...
        return initialize(matrix.ColumnCount, _arrow740)
...
def to_sparse_table(factors: FSharpList[Factor]) -> SparseTable:
    ...
    class ObjectExpr744:
        ...
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow743(x: str, y: str) -> bool:
                return x == y
    ...
    return SparseTable(matrix.Matrix, matrix.Keys, reverse(List_distinct(comment_keys, ObjectExpr744())), matrix.ColumnCount)
