...
def Array_ofIndexedSeq(s: IEnumerable_1[tuple[int, str]]) -> Array[str]:
    def _arrow584(tuple: tuple[int, str], s: Any=s) -> int:
        return tuple[0]
    ...
    def _arrow587(i: int, s: Any=s) -> str:
        def _arrow586(arg: tuple[int, str]) -> bool:
            return i == arg[0]

        match_value: tuple[int, str] | None = try_find(_arrow586, s)
    ...
    return initialize(1 + max_by(_arrow584, s, ObjectExpr585())[0], _arrow587, None)
...
def Array_map4(f: Callable[[_A, _B, _C, _D], _T], aa: Array[_A], ba: Array[_B], ca: Array[_C], da: Array[_D]) -> Array[_T]:
    ...
    def _arrow588(i: int, f: Any=f, aa: Any=aa, ba: Any=ba, ca: Any=ca, da: Any=da) -> _T:
        return f(aa[i], ba[i], ca[i], da[i])

    return initialize(len(aa), _arrow588, None)
...
def Dictionary_tryGetValue(k: _K, dict_1: Any) -> _V | None:
    ...
    out_arg: _V = None
    def _arrow589(__unit: None=None, k: Any=k, dict_1: Any=dict_1) -> _V:
        return out_arg

    def _arrow590(v: _V | None=None, k: Any=k, dict_1: Any=dict_1) -> None:
        nonlocal out_arg
        out_arg = v

    pattern_input = (try_get_value(dict_1, k, FSharpRef(_arrow589, _arrow590)), out_arg)
    if pattern_input[0]:
...
def Dictionary_tryGetString(k: _K, dict_1: Any) -> str | None:
    ...
    out_arg: str = None
    def _arrow591(__unit: None=None, k: Any=k, dict_1: Any=dict_1) -> str:
        return out_arg

    def _arrow592(v: str, k: Any=k, dict_1: Any=dict_1) -> None:
        nonlocal out_arg
        out_arg = v

    pattern_input = (try_get_value(dict_1, k, FSharpRef(_arrow591, _arrow592)), out_arg)
