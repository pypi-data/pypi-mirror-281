...
def encoder(pv: IPropertyValue_1[Any]) -> Json:
    ...
    if match_value_1 is None:
        ...
        if v.tag == 1:
            ...
            def _arrow1315(value_3: str, pv: Any=pv) -> Json:
                return Json(0, value_3)

            def _arrow1316(value_5: str, pv: Any=pv) -> Json:
                return Json(0, value_5)

            pattern_input_1 = (map(_arrow1315, oa_1.Name), map(_arrow1316, oa_1.TermAccessionNumber))
    ...
    def _arrow1317(value_11: str, pv: Any=pv) -> Json:
        return Json(0, value_11)

    def _arrow1318(value_13: str, pv: Any=pv) -> Json:
        return Json(0, value_13)

    def _arrow1319(x: Json, pv: Any=pv) -> Json:
        return x

    def _arrow1320(x_1: Json, pv: Any=pv) -> Json:
        return x_1

    def _arrow1321(value_15: str, pv: Any=pv) -> Json:
        return Json(0, value_15)

    def _arrow1322(value_17: str, pv: Any=pv) -> Json:
        return Json(0, value_17)

    return Json(5, choose(chooser, of_array([("@id", Json(0, gen_id(pv))), ("@type", Json(0, "PropertyValue")), ("additionalType", Json(0, pv.GetAdditionalType())), try_include("category", _arrow1317, pattern_input[0]), try_include("categoryCode", _arrow1318, pattern_input[1]), try_include("value", _arrow1319, pattern_input_1[0]), try_include("valueCode", _arrow1320, pattern_input_1[1]), try_include("unit", _arrow1321, pattern_input_2[0]), try_include("unitCode", _arrow1322, pattern_input_2[1]), ("@context", context_jsonvalue)])))


def decoder(create: Callable[[OntologyAnnotation | None, Value | None, OntologyAnnotation | None], _T]) -> Decoder_1[_T]:
    def _arrow1324(get: IGetters, create: Any=create) -> _T:
        ...
        def _arrow1323(__unit: None=None) -> Value | None:
            value: str | None
        ...
        return create(category, _arrow1323(), unit)

    return object(_arrow1324)
