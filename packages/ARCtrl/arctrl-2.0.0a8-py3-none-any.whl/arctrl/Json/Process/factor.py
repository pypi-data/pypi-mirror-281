...
def Factor_ISAJson_encoder(id_map: Any | None, value: Factor) -> Json:
    def f_1(value_1: Factor, id_map: Any=id_map, value: Any=value) -> Json:
        ...
        def _arrow1326(value_2: str, value_1: Any=value_1) -> Json:
            return Json(0, value_2)

        def _arrow1327(value_4: str, value_1: Any=value_1) -> Json:
            return Json(0, value_4)

        def _arrow1328(oa: OntologyAnnotation, value_1: Any=value_1) -> Json:
            return OntologyAnnotation_ISAJson_encoder(id_map, oa)

        def _arrow1329(comment: Comment, value_1: Any=value_1) -> Json:
            return Comment_ISAJson_encoder(id_map, comment)

        return Json(5, choose(chooser, of_array([try_include("@id", _arrow1326, Factor_ISAJson_genID(value_1)), try_include("factorName", _arrow1327, value_1.Name), try_include("factorType", _arrow1328, value_1.FactorType), try_include_seq("comments", _arrow1329, default_arg(value_1.Comments, []))])))

    if id_map is not None:
        def _arrow1330(f_2: Factor, id_map: Any=id_map, value: Any=value) -> str:
            return Factor_ISAJson_genID(f_2)

        return encode(_arrow1330, f_1, value, id_map)
...
def ARCtrl_Process_Factor__Factor_toISAJsonString_Static_71136F3F(spaces: int | None=None) -> Callable[[Factor], str]:
    def _arrow1335(f: Factor, spaces: Any=spaces) -> str:
