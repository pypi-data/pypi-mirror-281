...
def Requests_createHeadersDict_9F3777D(headers: Any | None=None) -> Any | None:
    def mapping(values: Any, headers: Any=headers) -> Any:
        def _arrow2037(__unit: None=None, values: Any=values) -> IEnumerable_1[tuple[str, Any]]:
            def _arrow2036(pair: Any) -> tuple[str, Any]:
                return (pair[0], pair[1])

            return map_1(_arrow2036, values)

        return create_obj(to_list(delay(_arrow2037)))
...
def Requests_mapResponseType_Z730CA7D4(response: InteropResponseType) -> Response:
    ...
    def _arrow2039(__unit: None=None, response: Any=response) -> IEnumerable_1[tuple[str, str]]:
        def _arrow2038(header_name: str) -> IEnumerable_1[tuple[str, str]]:
            return singleton((header_name, response.headers[header_name]))

        return collect(_arrow2038, list(response.headers.keys()))
    ...
    return Response(status_code, text, of_list(to_list(delay(_arrow2039)), ObjectExpr2040()), encoding)
...
def download_file(url: str) -> Async[str]:
    def _arrow2043(__unit: None=None, url: Any=url) -> Async[str]:
        response: Response = Requests_get_43A074E6(url)
        return singleton_1.Return(response.text)

    return singleton_1.Delay(_arrow2043)
