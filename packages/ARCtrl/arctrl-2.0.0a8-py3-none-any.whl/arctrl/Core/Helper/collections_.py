...
def Dictionary_ofSeq(s: IEnumerable_1[tuple[_KEY, _T]]) -> Any:
    ...
    dict_1: Any = Dictionary([], ObjectExpr306())
...
def Dictionary_tryFind(key: _KEY, dict_1: Any) -> _T | None:
    ...
    out_arg: _T = None
    def _arrow307(__unit: None=None, key: Any=key, dict_1: Any=dict_1) -> _T:
        return out_arg

    def _arrow308(v: _T | None=None, key: Any=key, dict_1: Any=dict_1) -> None:
        nonlocal out_arg
        out_arg = v

    pattern_input = (try_get_value(dict_1, key, FSharpRef(_arrow307, _arrow308)), out_arg)
    if pattern_input[0]:
...
def Dictionary_ofSeqWithMerge(merge: Callable[[_T, _T], _T], s: IEnumerable_1[tuple[_KEY, _T]]) -> Any:
    ...
    dict_1: Any = Dictionary([], ObjectExpr309())
...
def ResizeArray_distinct(a: Array[__A]) -> Array[__A]:
    ...
    try: 
        while enumerator.System_Collections_IEnumerator_MoveNext():
            ...
            if not contains(i, b, ObjectExpr310()):
                (b.append(i))
