...
class Value(Union):
    ...
    def from_string(value: str) -> Value:
        ...
        out_arg: int = 0
        def _arrow349(__unit: None=None) -> int:
            return out_arg

        def _arrow350(v: int) -> None:
            nonlocal out_arg
            out_arg = v or 0

        match_value = (try_parse(value, 511, False, 32, FSharpRef(_arrow349, _arrow350)), out_arg)
        if match_value[0]:
            ...
            out_arg_1: float = 0.0
            def _arrow351(__unit: None=None) -> float:
                return out_arg_1

            def _arrow352(v_2: float) -> None:
                nonlocal out_arg_1
                out_arg_1 = v_2

            match_value_1 = (try_parse_1(value, FSharpRef(_arrow351, _arrow352)), out_arg_1)
    ...
    def from_options(value: str | None=None, term_source: str | None=None, term_accesssion: str | None=None) -> Value | None:
        def _arrow353(__unit: None=None) -> Value | None:
            value_1: str = value
            return Value.from_string(value_1)

        return ((None if (term_accesssion is None) else Value(0, OntologyAnnotation.create(default_arg(value, ""), term_source, term_accesssion))) if (term_source is None) else Value(0, OntologyAnnotation.create(default_arg(value, ""), term_source, term_accesssion))) if (value is None) else ((_arrow353() if (term_accesssion is None) else Value(0, OntologyAnnotation.create(default_arg(value, ""), term_source, term_accesssion))) if (term_source is None) else Value(0, OntologyAnnotation.create(default_arg(value, ""), term_source, term_accesssion)))
