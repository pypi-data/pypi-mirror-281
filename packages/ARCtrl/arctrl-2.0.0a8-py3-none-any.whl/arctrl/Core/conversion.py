...
def _arrow766(__unit: None=None) -> Callable[[str], str]:
    clo_1: Callable[[str], str] = to_text(printf("%s %s"))(Person_AssayIdentifierPrefix)
    return clo_1


Person_createAssayIdentifierKey: Callable[[str], str] = _arrow766()
...
def ProcessParsing_tryComponentGetter(general_i: int, value_i: int, value_header: CompositeHeader) -> Callable[[Any, int], Component] | None:
    if value_header.tag == 0:
        ...
        def Value(matrix: Any, general_i: Any=general_i, value_i: Any=value_i, value_header: Any=value_header) -> Callable[[int], Component]:
            def _arrow786(i: int, matrix: Any=matrix) -> Component:
...
def ProcessParsing_tryParameterGetter(general_i: int, value_i: int, value_header: CompositeHeader) -> Callable[[Any, int], ProcessParameterValue] | None:
    if value_header.tag == 3:
        ...
        def Value(matrix: Any, general_i: Any=general_i, value_i: Any=value_i, value_header: Any=value_header) -> Callable[[int], ProcessParameterValue]:
            def _arrow789(i_1: int, matrix: Any=matrix) -> ProcessParameterValue:
...
def ProcessParsing_tryFactorGetter(general_i: int, value_i: int, value_header: CompositeHeader) -> Callable[[Any, int], FactorValue] | None:
    if value_header.tag == 2:
        ...
        def Value(matrix: Any, general_i: Any=general_i, value_i: Any=value_i, value_header: Any=value_header) -> Callable[[int], FactorValue]:
            def _arrow791(i_1: int, matrix: Any=matrix) -> FactorValue:
...
def ProcessParsing_tryCharacteristicGetter(general_i: int, value_i: int, value_header: CompositeHeader) -> Callable[[Any, int], MaterialAttributeValue] | None:
    if value_header.tag == 1:
        ...
        def Value(matrix: Any, general_i: Any=general_i, value_i: Any=value_i, value_header: Any=value_header) -> Callable[[int], MaterialAttributeValue]:
            def _arrow792(i_1: int, matrix: Any=matrix) -> MaterialAttributeValue:
...
def ProcessParsing_tryGetProtocolTypeGetter(general_i: int, header: CompositeHeader) -> Callable[[Any, int], OntologyAnnotation] | None:
    if header.tag == 4:
        def Value(matrix: Any, general_i: Any=general_i, header: Any=header) -> Callable[[int], OntologyAnnotation]:
            def _arrow794(i: int, matrix: Any=matrix) -> OntologyAnnotation:
...
def ProcessParsing_tryGetProtocolREFGetter(general_i: int, header: CompositeHeader) -> Callable[[Any, int], str] | None:
    if header.tag == 8:
        def Value(matrix: Any, general_i: Any=general_i, header: Any=header) -> Callable[[int], str]:
            def _arrow795(i: int, matrix: Any=matrix) -> str:
...
def ProcessParsing_tryGetProtocolDescriptionGetter(general_i: int, header: CompositeHeader) -> Callable[[Any, int], str] | None:
    if header.tag == 5:
        def Value(matrix: Any, general_i: Any=general_i, header: Any=header) -> Callable[[int], str]:
            def _arrow796(i: int, matrix: Any=matrix) -> str:
                return get_item_from_dict(matrix, (general_i, i)).AsFreeText
...
def ProcessParsing_tryGetProtocolURIGetter(general_i: int, header: CompositeHeader) -> Callable[[Any, int], str] | None:
    if header.tag == 6:
        def Value(matrix: Any, general_i: Any=general_i, header: Any=header) -> Callable[[int], str]:
            def _arrow798(i: int, matrix: Any=matrix) -> str:
                return get_item_from_dict(matrix, (general_i, i)).AsFreeText
...
def ProcessParsing_tryGetProtocolVersionGetter(general_i: int, header: CompositeHeader) -> Callable[[Any, int], str] | None:
    if header.tag == 7:
        def Value(matrix: Any, general_i: Any=general_i, header: Any=header) -> Callable[[int], str]:
            def _arrow799(i: int, matrix: Any=matrix) -> str:
                return get_item_from_dict(matrix, (general_i, i)).AsFreeText
...
def ProcessParsing_tryGetInputGetter(general_i: int, header: CompositeHeader) -> Callable[[Any, int], ProcessInput] | None:
    if header.tag == 11:
        def Value(matrix: Any, general_i: Any=general_i, header: Any=header) -> Callable[[int], ProcessInput]:
            def _arrow802(i: int, matrix: Any=matrix) -> ProcessInput:
...
def ProcessParsing_tryGetOutputGetter(general_i: int, header: CompositeHeader) -> Callable[[Any, int], ProcessOutput] | None:
    if header.tag == 12:
        def Value(matrix: Any, general_i: Any=general_i, header: Any=header) -> Callable[[int], ProcessOutput]:
            def _arrow803(i: int, matrix: Any=matrix) -> ProcessOutput:
...
def ProcessParsing_tryGetCommentGetter(general_i: int, header: CompositeHeader) -> Callable[[Any, int], Comment] | None:
    if header.tag == 14:
        def Value(matrix: Any, general_i: Any=general_i, header: Any=header) -> Callable[[int], Comment]:
            def _arrow805(i: int, matrix: Any=matrix) -> Comment:
...
def ProcessParsing_getProcessGetter(process_name_root: str, headers: IEnumerable_1[CompositeHeader]) -> Callable[[Any, int], Process]:
    ...
    if match_value is None:
        def _arrow811(matrix_1: Any, process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[int], FSharpList[ProcessInput]]:
            def _arrow810(i_1: int) -> FSharpList[ProcessInput]:
        ...
        input_getter_1 = _arrow811

    else: 
        input_getter: Callable[[Any, int], ProcessInput] = match_value
        def _arrow819(matrix: Any, process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[int], FSharpList[ProcessInput]]:
            def _arrow818(i: int) -> FSharpList[ProcessInput]:
    ...
    if match_value_1 is None:
        def _arrow837(matrix_3: Any, process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[int], FSharpList[ProcessOutput]]:
            def _arrow835(i_3: int) -> FSharpList[ProcessOutput]:
        ...
        def _arrow841(matrix_2: Any, process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[int], FSharpList[ProcessOutput]]:
            def _arrow840(i_2: int) -> FSharpList[ProcessOutput]:
        ...
        output_getter_1 = _arrow841

    def _arrow843(matrix_4: Any, process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[int], Process]:
        def _arrow842(i_4: int) -> Process:
...
def ProcessParsing_groupProcesses(ps: FSharpList[Process]) -> FSharpList[tuple[str, FSharpList[Process]]]:
    ...
    class ObjectExpr845:
        ...
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow844(x_1: str, y: str) -> bool:
                return x_1 == y
    ...
    return List_groupBy(projection, ps, ObjectExpr845())
...
def ProcessParsing_mergeIdenticalProcesses(processes: FSharpList[Process]) -> FSharpList[Process]:
    ...
    l: FSharpList[tuple[tuple[str, Any, Any | None, Any | None], FSharpList[Process]]] = List_groupBy(projection, processes, ObjectExpr847())
...
def ProcessParsing_processToRows(p: Process) -> FSharpList[FSharpList[tuple[CompositeHeader, CompositeCell]]]:
    ...
    if match_value_1 is None:
        ...
        def _arrow852(__unit: None=None, p: Any=p) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
            def _arrow851(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                def _arrow850(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                    def _arrow849(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                        ...
                        return append(singleton_1((CompositeHeader(6), CompositeCell(1, value_10(prot_1.Uri)))) if (prot_1.Uri is not None) else empty_1(), delay(_arrow848))

                    return append(singleton_1((CompositeHeader(5), CompositeCell(1, value_10(prot_1.Description)))) if (prot_1.Description is not None) else empty_1(), delay(_arrow849))

                return append(singleton_1((CompositeHeader(4), CompositeCell(0, value_10(prot_1.ProtocolType)))) if (prot_1.ProtocolType is not None) else empty_1(), delay(_arrow850))

            return append(singleton_1((CompositeHeader(8), CompositeCell(1, value_10(prot_1.Name)))) if (prot_1.Name is not None) else empty_1(), delay(_arrow851))

        prot_vals = to_list(delay(_arrow852))
    ...
    def mapping_6(tupled_arg_1: tuple[tuple[str, str], FSharpList[tuple[ProcessInput, ProcessOutput]]], p: Any=p) -> FSharpList[tuple[CompositeHeader, CompositeCell]]:
        ...
        vals: FSharpList[tuple[CompositeHeader, CompositeCell]] = map_2(mapping_5, sort_by(projection_1, append_1(chars, append_1(components, append_1(pvs, map_2(mapping_4, ProcessOutput_getFactorValues_Z42C11600(output_for_factors))))), ObjectExpr853()))
        def _arrow858(__unit: None=None, tupled_arg_1: Any=tupled_arg_1) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
            def _arrow857(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                def _arrow856(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                    def _arrow855(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                        ...
                        return append(comments, delay(_arrow854))

                    return append(vals, delay(_arrow855))

                return append(prot_vals, delay(_arrow856))

            return append(singleton_1(JsonTypes_decomposeProcessInput(input_for_type)), delay(_arrow857))

        return to_list(delay(_arrow858))
    ...
    def _arrow859(__unit: None=None, p: Any=p) -> FSharpList[tuple[ProcessInput, ProcessOutput]]:
    ...
    return map_2(mapping_6, List_groupBy(projection, _arrow859(), ObjectExpr860()))
...
def ARCtrl_ArcTable__ArcTable_GetProtocols(this: ArcTable) -> FSharpList[Protocol]:
    if this.RowCount == 0:
        def _arrow861(__unit: None=None, this: Any=this) -> Protocol:
        ...
        return singleton(_arrow861())

    else: 
        def _arrow862(i: int, this: Any=this) -> Protocol:
        ...
        return List_distinct(initialize(this.RowCount, _arrow862), ObjectExpr863())
...
def ARCtrl_ArcTable__ArcTable_GetProcesses(this: ArcTable) -> FSharpList[Process]:
    if this.RowCount == 0:
        ...
        def _arrow864(arg: Any, this: Any=this) -> Callable[[int], Process]:
            clo_1: Callable[[int], Process] = clo(arg)
            return clo_1

        getter = _arrow864
        def _arrow866(__unit: None=None, this: Any=this) -> IEnumerable_1[Process]:
            def _arrow865(i: int) -> Process:
                return getter(this.Values)(i)

            return map_1(_arrow865, range_big_int(0, 1, this.RowCount - 1))

        return ProcessParsing_mergeIdenticalProcesses(to_list(delay(_arrow866)))
