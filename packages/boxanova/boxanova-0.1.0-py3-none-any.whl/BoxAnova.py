...
import warnings
from utils.renaming import formatting, renaming
...
class BoxAnova:
    ...
    def calc_sig(self, group_order: list, annotation=True) -> tuple[any, pd.DataFrame]:
    ...
    def calc_sig_levels_group(self, hue=False, rate_position_offset_with_hue=0.1, fine_tuning_kws: dict = None):
        ...
        mean_dif, df_res = self.calc_sig(group_order=self.order)  # Calc p-value
    ...
    def calc_sig_levels_hue(self, hue, hue_order: list[str] = None, show_group: bool = True,
        ...
        mean_dif, df_res = self.calc_sig(group_order=group_order, annotation=not show_group)  # Calc p-value
