...
class CopyableFieldsAdmin(ModelAdmin):
    ...
    class Media:
        js = ('admin/js/jquery.init.js', 'immunity-utils/js/copyable.js')
...
class UUIDAdmin(CopyableFieldsAdmin):
    ...
    Immunity modules show `uuid` as the only copyable field
    """
...
class ReceiveUrlAdmin(ModelAdmin):
    ...
    class Media:
        js = ('admin/js/jquery.init.js', 'immunity-utils/js/receive_url.js')
