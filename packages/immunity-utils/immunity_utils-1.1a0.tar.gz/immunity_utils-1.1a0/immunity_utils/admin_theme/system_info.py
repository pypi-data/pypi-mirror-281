...
def _get_immunity2_detail(attribute_name, fallback=None):
    try:
        return import_string(f'immunity2.{attribute_name}')
    except ImportError:
        return fallback


def get_immunity_version():
    return _get_immunity2_detail('__immunity_version__')


def get_immunity_installation_method():
    return _get_immunity2_detail('__immunity_installation_method__', 'unspecified')


def get_enabled_immunity_modules():
    enabled_packages = {}
