...
from immunity_utils import utils
from immunity_utils.admin_theme import system_info
...
class TestImmunityVersion(TestCase):
    def setUp(self):
        # The post_migrate signal creates the first ImmunityVersion object
...
_MODULES_RETURN_VALUE,
    ...
    @patch(
        'immunity_utils.admin_theme.system_info.import_string', side_effect=ImportError
    )
    def test_installation_method_not_defined(self, *args):
        self.assertEqual(system_info.get_immunity_installation_method(), 'unspecified')

    def test_log_module_version_changes_on_new_installation(
        self,
    ):
        ImmunityVersion.objects.all().delete()
        is_install, is_upgrade = ImmunityVersion.log_module_version_changes(
            system_info.get_enabled_immunity_modules()
        )
        self.assertEqual(
            is_install,
            True,
    ...
    @patch.object(models, 'get_immunity_version', return_value='23.0.0a')
    @patch.object(
        models,
...
_MODULES_RETURN_VALUE,
    ...
    @patch.object(ImmunityVersion, '_post_metrics')
    ...
    def test_new_installation(self, mocked_post, *args):
        ...
        self.assertEqual(ImmunityVersion.objects.count(), 1)
...
_MODULES_RETURN_VALUE,
    ...
    def test_modules_upgraded(self, mocked_post, *args):
        ...
        ImmunityVersion.objects.update(
            module_version={
                'Immunity Version': '22.10.0',
                'immunity-utils': '1.0.5',
                'immunity-users': '1.0.2',
            }
        )
        tasks.send_usage_metrics.delay(category='Upgrade')
        mocked_post.assert_called_with(_MODULES_UPGRADE_EXPECTED_METRICS)

        self.assertEqual(ImmunityVersion.objects.count(), 2)
        version = ImmunityVersion.objects.first()
        expected_module_version = {
            'Immunity Version': '23.0.0a',
            **_ENABLED_IMMUNITY
...
_MODULES_RETURN_VALUE,
    ...
    def test_post_usage_metrics_500_response(
        ...
        mocked_error.assert_called_with(
            'Collection of usage metrics failed, max retries exceeded.'
            ' Error: HTTPSConnectionPool(host=\'analytics.immunity.io\', port=443):'
                ...
                plan=[
                    (
                        migrations.Migration(
                            name='0001_initial', app_label='immunity_users'
                        ),
                        False,
                    )
                ]
