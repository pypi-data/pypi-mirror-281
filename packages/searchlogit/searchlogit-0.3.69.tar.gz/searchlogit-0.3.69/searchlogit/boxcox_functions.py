...
# {
    if lmda == 0:
        ...
        return np.nan_to_num((np.power(x, lmda) - 1)/lmda)
# }

def transform_derivative(x, lmda):
# {
    if lmda == 0:
        return 0.5*(np.log(x))**2  # i.e., (np.power(np.log(x)), 2)/2
    else:
        return (lmda * (np.power(x, lmda)) * np.log(x) - np.power(x, lmda) + 1) / np.power(lmda, 2)
# }
