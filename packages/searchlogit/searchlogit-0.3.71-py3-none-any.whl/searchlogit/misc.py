...
''' -------------------------------------------------------------------- '''
def initialise_fit_intercept(params):
# {
...
''' -------------------------------------------------------------------- '''
def setup_logit(i, logit, X, y, varnames, class_params_spec, avail, alts, transvars, gtol, mixed=False,
            seed=None, panels=None, randvars=None, correlation=None, n_draws=None, mnl_init=None):
# {
    ...
    seed_i = seed + i if seed else None
    ...
    # {
        ...
        logit.setup(X_i, y, varnames_i, alts, avail=avail, transvars=transvars_i,
                             randvars=randvars_i, panels=panels, seed=seed_i,
                             fit_intercept=fit_intercept_i, correlation=correlation_i, n_draws=n_draws,
                             verbose=0, mnl_init=mnl_init, gtol=gtol)
    # }
    else:
        logit.setup(X_i, y, varnames_i, alts, avail=avail, transvars=transvars_i,
              fit_intercept=fit_intercept_i, seed = seed, verbose=0, gtol=gtol)
    logit.fit()
    return logit
# }
