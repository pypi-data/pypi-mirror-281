...
# {
   ...
   '''
    ...
    def __init__(self, criterions, df, varnames, df_test=None, distr=None, avail=None, test_avail=None,
        ...
        intercept_opts=None, base_alt=None, val_share=0.25, seed=None):
    # {
        ...
        self.seed = seed
        self.generator = np.random.RandomState(seed) if seed else np.random
        self.obs_freq = None
        ...
        if df_test is None and self.nb_crit > 1:
        # {
...
# {
    ...
    # {
        ...
        self.data.setdefault('pval', None)
        self.data.setdefault('pval_member', None)
    ...
    # {
        ...
        #self['model'] = copy.deepcopy(sol.model)
    # }
...
# {
    ...
    generator: Generating random selections and distributions.
    """
    ...
    """"
    perturb_asfeature(self, sol);
    perturb_isfeature(self, sol);
    perturb_randfeature(self, sol);
    perturb_bcfeature(self, sol, pitch);
    perturb_corfeature(self, sol);
    perturb_member_class_feature(self, sol);
    perturb_member_paramfeature(self, sol);
    
    add_asfeature(self, solution);
    add_isfeature(self, solution);
    add_bcfeature(self, solution, prob);
    add_randfeature(self, solution);
    add_corfeature(self, solution);
    add_class_paramfeature(self, solution);
    add_member_paramfeature(self, solution);
    
    remove_asfeature(self, solution);
    remove_isfeature(self, solution);
    remove_bcfeature(self, solution);
    remove_randfeature(self, solution);
    remove_corfeature(self, solution);
    remove_class_paramfeature(self, solution);
    remove_member_paramfeature(self, solution);
    
    change_distribution(self, solution);
    remove_redundant_asvars(self, asvar_list, transasvars, asvarnames);
    increase_sol_by_one_class(self, sol);
    revise_solution(self, name, sol, ref_sol);
    
    already_generated(self, sol);
    create_dummy_column(self, asvars);
    generate_solution(self);
    evaluate_solution(self, sol);
    ...
    estimate_mnl(self, sol);
    estimate_mxl(self, sol);
    estimate_lccm(self, sol);
    estimate_lccmm(self, sol);
    
    dominates(self, sol1_obj, sol2_obj, criterion);
    get_fronts(self, soln);
    compute_crowding_dist(self, fronts, soln, key);
    crowding_dist(self, fronts, soln);
    get_pareto(self, fronts, soln);
    sort_solutions(self, fronts, v_dis, soln);
    find_best_sol(self, soln);
    non_dominant_sorting(self, soln);
    """
    ...
    # {
        return any(self.param.crit(i) == 'mae' for i in range(self.nb_crit))
    # }
    ...
    # {
        ...
        # {
            ...
            # {
                ...
                if gen_var:
                    gen_var_select.append(self.param.generator.choice(gen_var))
                ...
                choice = self.param.generator.randint(2)  # Chooses a 0 or 1
        ...
        if len(gen_var_select) != 0 and len(alspec_final) != 0:
            final_asvars = gen_var_select if self.param.generator.randint(2) else alspec_final
    ...
    # {
        ind_availasvar = [self.param.generator.randint(2) for _ in self.param.avail_asvars]
    ...
    # {
        ind_availisvar = [self.param.generator.randint(2) for _ in self.param.avail_isvars]
    ...
    ''' ---------------------------------------------------------- '''
    ''' Function. Determine if the model should include an intercept '''
    ''' ---------------------------------------------------------- '''
    def select_asc_ind(self):
    # {
        if self.param.ps_intercept is None:
            return (self.param.generator.rand() < 0.5)
        else:
    ...
    # {
        bcvars = []
        bctrans = self.param.ps_bctrans if self.param.ps_bctrans is not None else (self.param.generator.rand() < 0.5)
        if bctrans and self.param.allow_latent_bcvars:
        # {
            ind_availbcvar = [self.param.generator.randint(2) for _ in range(len(self.param.avail_bcvars))]
    ...
    ''' ------------------------------------------------- '''
    def get_specifications(self, asc_ind):
    # {
        ...
        # {
            ...
            # {
                tmp_class_spec = np.array([var for var in class_vars if self.param.generator.uniform() < 0.6])

                # If random selection leads to none chosen, force at least 1
                tmp_class_spec = np.random.choice(class_vars, 1) if len(tmp_class_spec) < 1 else tmp_class_spec
                class_params_spec[i] = np.sort(tmp_class_spec)
            # }

            tmp_member_spec = np.array([var for var in member_vars if self.param.generator.uniform() < 0.6])

            # If random selection leads to none chosen, force at least 1
            tmp_member_spec = np.random.choice(member_vars, 1) if len(tmp_member_spec) < 1 else tmp_member_spec

            tmp_member_spec = np.sort(tmp_member_spec)  # Sort the elements of tmp_member_spec
            # Make 'self.param.num_classes - 1' copies of tmp_member_spec and record within member_params_spec
            # in first 'self.param.num_classes - 1' elements
            member_params_spec[:self.param.num_classes - 1] = [sorted] * (self.param.num_classes - 1)
        # }
    ...
    # {
        ...
        distr = [self.param.generator.choice(self.param.distr) for _ in range(len(avail_rvar))]
        ...
        # {
            if var in asvars and var not in rand_vars.keys():
                rand_vars.update({var: self.param.generator.choice(distr)})
        # }
    ...
    # {
        corvars = []
        cor = (self.param.generator.rand() < 0.5) if self.param.ps_cor is None else self.param.ps_cor
        if cor:
        # {
            ind_availcorvar = [self.param.generator.randint(2) for _ in range(len(self.param.avail_corvars))]
    ...
    # {
        ...
        class_vars, member_vars, class_params_spec, member_params_spec = self.get_specifications(asc_ind)
    ...
    # {
        solution['asvars'].extend(new_asvar)
        ...
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        if self.param.ps_intercept is None:
            solution['asc_ind'] = bool(self.param.generator.randint(2))
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        return solution
    # }
    ...
    # {
        ...
        if len(candidate) > 0:
            new_asvar = self.param.generator.choice(candidate, 1)
    ...
    # {
        if solution['asvars']:
            rem_asvar = self.param.generator.choice(solution['asvars'])    # Randomly choose one
    ...
    ''' already in the solution.                                   '''
    ''' ---------------------------------------------------------- '''
    def add_isvar(self, add_isvar, solution):
    # {
        solution['isvars'] = sorted(list(set(solution['isvars']).union(add_isvar)))
    # }
    ...
    # {
        ...
        if len(candidate) >  0:
        # {
            add_isvar = self.param.generator.choice(candidate)
    ...
    # {
        if solution['isvars']:
            rem_isvar = self.param.generator.choice(solution['isvars'])
    ...
    # {
        ...
        if len(candidates) > 0:
        # {
            member_param = self.param.generator.choice(candidates)
    ...
    # {
        ...
        # {
            ...
            if len(candidates) > 1:
            # {
                rem_member_param = self.param.generator.choice(candidates)
    ...
    # {
        distr = self.param.generator.choice(self.param.distr)  # Choose a distribution
    ...
    # {
        ...
        if len(candidates) > 0:
            new_randvar = self.param.generator.choice(candidates)
    ...
    # {
        ...
        if len(candidates) > 0:
            rem_randvar = self.param.generator.choice(candidates) # Choose a random variable to remove
    ...
    # {
        if self.param.ps_bctrans is None:
            bctrans = bool(self.param.generator.randint(2, size=1))
        else:
        ...
        # {
            ...
            bools = self.param.generator.choice([0,1], size=len(solution['asvars']), p=[1 - prob, prob])
    ...
    # {
        if solution['bcvars']:
        # {
            rem_bcvar = self.param.generator.choice(solution['bcvars'])
    ...
    # {
        ...
        cor = bool(self.param.generator.randint(2, size=1)) if self.param.ps_cor is None else self.param.ps_cor
    ...
    # {
        ...
        # {
            ...
            if len(candidates) > 0:
                rem_corvar = self.param.generator.choice(candidates)
    ...
    # {
        ...
        if new_params:
        # {
            new_param = self.param.generator.choice(new_params, 1)  # Choose a parameter
    ...
    # {
        class_params_spec = solution['class_params_spec']
        choice = self.param.generator.randint(0, len(class_params_spec))
    ...
    # {
        ...
        # {
            rem_asvar = self.param.generator.choice(class_chosen)
    ...
    # {
        ...
        if len(candidates) > 0:
        # {
            chosen_randvar = self.param.generator.choice(candidates)  # Choose a randvar option
            cand_distr = [distr for distr in self.param.distr if distr not in solution['randvars'][chosen_randvar]]
            new_distr = self.param.generator.choice(cand_distr)
    ...
    # {
        ...
        # {
            ...
            chosen_randvar = self.param.generator.choice(randvars)
    ...
    # {
        if self.param.generator.rand() <= 0.5:
   ...
   # {
    ...
    # {
        ...
        # {
            if var in rem_asvars and var not in rem_rand_vars.keys():
                rem_rand_vars.update({var: self.param.generator.choice(remove_rv)})
        # }
    ...
    # {
        ...
        # {
            ...
            # {
                ...
                aic, bic, loglik, mae, asvars, isvars, randvars, bcvars, corvars, converged, \
                    pval, pval_member, coefs = self.evaluate_model(sol)
    ...
    # {
        ...
        # {
            ...
            # {
                aic, bic, loglik, mae, asvars, isvars, randvars, bcvars, corvars, converged, \
                    pval, pval_member, coeff_names = self.evaluate_model(sol)
                ...
                # {
                    self.converged += 1
                    sol['converged'] = True
                    sol['coeff_names'] = coeff_names
                    sol['pvals'], sol['pval_member'] = pval, pval_member
    ...
    # {
        ...
        rand_array = self.param.generator.choice([True, False], len(asvars))
    ...
    # {
        ...
        as_vars, is_vars, rand_vars, bc_vars, cor_vars, pvals, coef_names = [], [], {}, [], [], [], []
        pvals_member = []
        converged = False
        return (aic, bic, loglik, mae, as_vars, is_vars, rand_vars, bc_vars, cor_vars,
                converged, pvals, pvals_member, coef_names)
    # }
    ...
    # {
        ...
        model.setup(X=X, y=y, varnames=varnames, isvars=isvars, alts=alts,
            ...
            weights=weights, avail=avail, base_alt=base_alt, maxiter=maxiter, ftol=ftol, gtol=gtol, seed=seed)
        model.fit()
        return model
    # }

    def fit_mxl(self, X, y, varnames, alts, isvars, transvars, ids, panels, randvars, corvars,
            fit_intercept, init_coeff, n_draws, weights, avail, base_alt,  maxiter, seed,
                ftol, gtol, save_fitted_params):
    # {
        model = MixedLogit()
        model.setup(X=X, y=y, varnames=varnames, isvars=isvars, alts=alts, transvars=transvars, ids=ids,
            ...
            seed=seed, ftol=ftol, gtol=gtol, save_fitted_params=save_fitted_params)
        model.fit()
        return model
    # }
    ...
    # {
        ...
        model.setup(X=X, y=y, varnames=varnames, class_params_spec=class_params_spec,
            ...
            avail=avail, avail_latent=avail_latent, intercept_opts=intercept_opts,  weights=weights, seed=seed,
    ...
    def fit_lcmm(self, X, y, varnames, alts, class_params_spec, member_params_spec, num_classes,
                 ids, panels, randvars, n_draws, corvars, transvars, maxiter, avail, ftol,
                    ftol_lccmm ,gtol, gtol_membership_func, weights, base_alt, seed):
    # {
    ...
    # {
        ...
        # {
            ...
            model = self.fit_mnl(X=X, y=y, varnames=all_vars, isvars=is_vars, alts=self.param.alt_var,
                    ...
                    maxiter=self.param.maxiter, seed=seed, ftol=self.param.ftol, gtol=self.param.gtol)
            # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            sol['model'] = model # Store the model object
            converged = model.converged
            aic, bic, loglik = model.aic, model.bic, model.loglikelihood
            pvals, pvals_member = model.pvalues, []
            coeff, coeff_names = model.coeff_est, model.coeff_names
            ...
            # {
                ...
                seed = self.param.generator.randint(2 ** 31 - 1)
                model = self.fit_mnl(X_test, y_test, varnames=all_vars, isvars=is_vars,
                    alts=self.param.alt_var, ids=self.param.test_choice_id, fit_intercept=asc_ind,
                    transvars=bc_vars, maxiter=0, init_coeff=coeff, gtol=self.param.gtol, ftol=self.param.ftol,
                    avail=self.param.test_avail, weights=self.param.test_weight_var,
                    base_alt=self.param.base_alt, seed=seed)
            # }
            mae = model.mae
            # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            tuple = (aic, bic, loglik, mae, as_vars, is_vars, rand_vars, bc_vars, cor_vars,
                     converged, pvals, pvals_member, coeff_names)
        # }
        except Exception as e:
        # {
            self.exception += 1
    ...
    # {
        ...
        # {
            ...
            model = self.fit_mxl(X, y, varnames=all_vars, alts=self.param.alt_var, isvars=is_vars, transvars=bc_vars,
                    ...
                    ftol=self.param.ftol, gtol=self.param.gtol, save_fitted_params=False, seed=seed)
            # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            sol['model'] = model  # Store the model object
            converged = model.converged
            aic, bic, loglik = model.aic, model.bic, model.loglikelihood
            ...
            # {
                ...
                y_test = self.param.test_choices
                seed = self.param.generator.randint(2 ** 31 - 1)
                model = self.fit_mxl(X_test, y_test, varnames=all_vars, alts=self.param.test_alt_var, isvars=is_vars,
                        ...
                        init_coeff=coeff, gtol=self.param.gtol, ftol=self.param.ftol, weights=self.param.test_weight_var,
                        base_alt=self.param.base_alt, seed=seed, save_fitted_params=False)
                model.fit()
    ...
    # {
        ...
        model = self.fit_lcm(X, y, varnames=all_vars, class_params_spec=class_params_spec,
                    ...
                    weights=self.param.weights, seed=seed)
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        sol['model'] = model  # Store the model object
        converged = model.converged
        aic, bic, loglik = model.aic, model.bic, model.loglikelihood
        pvals,  pvals_member= model.pvalues, model.pvalues_member
        coeff_names, member_names = model.coeff_names, model.coeff_names_member
        coeff_names = np.concatenate((coeff_names, member_names))
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        # COMPUTE MAE
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        if self.mae_is_an_objective():
        ...
        tuple = (aic, bic, loglik, mae, as_vars, is_vars, rand_vars, transvars, cor_vars, converged,
                 pvals, pvals_member, coeff_names)
        return tuple
    # }
    ...
    ''' ---------------------------------------------------------- '''
    def run(self, with_latent=False):
    # {
        self.exception = 0

        if with_latent:
            self.run_search_latent()
        else:
            self.run_search()
    # }
# }
