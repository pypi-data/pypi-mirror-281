...
# {
    ...
    ''' ---------------------------------------------------------- '''
    def __init__(self, param:Parameters, ctrl, idnum=0):
    # {
        ...
        self.max_no_impr = 3        # Max number of iterations permitted without improvements
        self.terminate = False      # Termination flag
        self.rate = np.exp((1.0 / self.max_temp_steps) * np.log(self.tF/self.tI)) # Temperature reduction rate

        # Note: tF = tI * power(rate, max_temp_steps)
        ...
        self.current_sol = None     # Current solution
        ...
        self.perturb_fn: PerturbFn = self.perturb_single if self.nb_crit == 1 else self.perturb_multi
    ...
    # {
        ...
        return (self.param.generator.rand() < 0.10)
    # }
    ...
    # {
        ...
        # {
            ...
            choice = self.param.generator.choice(choices) # Make a choice
            add = self.param.generator.randint(2)  # Choose to add or remove feature
    ...
    #{
        ...
        chosen = self.param.generator.choice(num_par)
    ...
    # {
        ...
        choice = self.param.generator.choice(choices)
        if choice == self.perturb_bcfeature:
            new_sol = self.perturb_bcfeature(new_sol, prob)
        else:
            new_sol = choice(new_sol)
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

        adj_sol, converged = self.evaluate(new_sol)
        ...
        # {
            ...
            # {
                self.log_kpi(new_sol, self.debug_file,True)
                self.accepted += 1
            ...
            # }
                ...
                self.not_accepted += 1
                self.log_kpi(new_sol, self.debug_file, False)
        # }
    # }
    ...
    # {
        ...
        # {
            ...
            choice = self.param.generator.randint(len(self.archive))
...
# {
    ...
    # {
        ...
        self.solvers = [SA(param, ctrl, idnum=i) for i in range(nthrds)]
