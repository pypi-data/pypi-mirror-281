...
from .boxcox_functions import boxcox_param_deriv, boxcox_transformation, truncate_lower, truncate
...
# {
    ...
    int seed:           Random seed for numpy random generator / default=None
    ...
    # {
        ...
        H[H == 0] = min_val  # Remove zero values
        H[np.isnan(H)] = min_val  # QUERY. IS THIS REALLY REQUIRED?
        H = truncate(H, -max_val, max_val)  # Scale elements if too big or small
    ...
    def setup(self, X, y, varnames=None, alts=None, isvars=None, transvars=None,
        ...
        seed=None, ftol=1e-6, gtol=1e-6, return_grad=True,
        return_hess=True, verbose=1, method="bfgs", scipy_optimisation=True):
    # {
        ...
        self.seed = seed
        np.random.seed(seed) if seed is not None else None  # Reseed random number generator
