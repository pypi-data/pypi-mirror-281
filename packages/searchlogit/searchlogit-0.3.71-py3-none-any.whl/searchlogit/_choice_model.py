...
# {
    ...
    # {
        ...
        # {
            ...
            self.fxidx = np.insert(fxidx_bool, 0, np.repeat(True, nbFalse))

            fxtransidx_bool =np.array(self.fxtransidx, dtype="bool")
            self.fxtransidx = np.insert(fxtransidx_bool, 0, np.repeat(False, nbFalse))

            if hasattr(self, 'rvidx'):
                rvidx_bool = np.array(self.rvidx, dtype=bool) # Convert self.rvidx to boolean array if it's not already
                self.rvidx = np.insert(rvidx_bool, 0, np.repeat(False, nbFalse))# Insert False values at the beginning of the boolean array
            ...
            if hasattr(self, 'rvtransidx'):
                ...
                self.rvidx = np.insert(rvtransidx_bool, 0, np.repeat(False, nbFalse))# Insert False values at the beginning of the boolean array
        # }
        ...
        # {
            ...
            if '_inter' not in self.isvars:  # stop running in validation
                ...
                self.rvidx = np.insert(getattr(self, 'rvidx', np.repeat(False, J - 1)), 0, False)
                self.rvtransidx = np.insert(getattr(self, 'rvtransidx', np.repeat(False, J - 1)), 0, False)
        # }
        ...
        randpos = [lst.index(str) for str in randvars]  # Position of asvarss
    ...
    # {
        ...
        significance_symbols = {0.001: "***", 0.01: "**", 0.05: "*", 0.1: "."}

        # Iterate through the coefficients
        for i, coeff in enumerate(self.coeff_est):
        # {
            # Get the corresponding significance symbol
            signif = next((symbol for threshold, symbol in significance_symbols.items() if self.pvalues[i] < threshold), "")
