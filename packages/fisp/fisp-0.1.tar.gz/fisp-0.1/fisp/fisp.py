...
class Spectrum:
    ...
    def save_csv(self, name, bins=400):
        ...
        name = name+'.csv' if name[-4:] != '.csv' else name
        name = directory + name
    ...
    def sort(self):
        ...
        """
        indexes = np.argsort(self.energies, kind='mergesort')
...
def simulate():
    ...
    while continue_passes is True:
        ...
        for direction in ('back', 'front'):
            ...
            if direction == 'front':
                reverse_slices_list()
            for slice_ in tqdm(SLICES_LIST, disable=None):
