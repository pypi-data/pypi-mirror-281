...
class Relationship:
    ...
    def get_interaction_options(self):
        ...
        if self.relationship_type in ["恋人", "配偶"]:
            ...
            options["牵手"] = "hold_hands"
            options["亲吻"] = "kiss"
        if self.relationship_type == "配偶":
            options["做爱"] = "make_love"
    ...
    def hold_hands(self):
        if self.relationship_type in ["恋人", "配偶"]:
            print(f"你牵起了{self.character.name}的手。")
            if random.random() < (self.player.charm / 5):
                self.affection += 10
                print(f"{self.character.name}感到很温暖，对你的好感增加了！当前好感度：{self.affection}")
            else:
                self.affection -= 2
                print(f"{self.character.name}有点尴尬，对你的好感减少了。当前好感度：{self.affection}")
        else:
            print(f"你不能牵{self.character.name}的手，因为他/她不是你的恋人或配偶。")

    def kiss(self):
        if self.relationship_type in ["恋人", "配偶"]:
            print(f"你亲吻了{self.character.name}。")
            if random.random() < (self.player.charm / 4):
                self.affection += 15
                print(f"{self.character.name}对你的亲吻感到很幸福，对你的好感增加了！当前好感度：{self.affection}")
            else:
                self.affection -= 5
                print(f"{self.character.name}有点惊讶，对你的好感减少了。当前好感度：{self.affection}")
        else:
            print(f"你不能亲吻{self.character.name}，因为他/她不是你的恋人或配偶。")

    def make_love(self):
        if self.relationship_type == "配偶":
            print(f"你和{self.character.name}共度了亲密的时光。")
            if random.random() < (self.player.charm / 3):
                self.affection += 30
                print(f"{self.character.name}对与你的亲密关系感到很满足，对你的好感大幅增加！当前好感度：{self.affection}")
            else:
                self.affection -= 10
                print(f"{self.character.name}有点疲惫，对你的好感减少了。当前好感度：{self.affection}")
        else:
            print(f"你不能与{self.character.name}共度亲密时光，因为他/她不是你的配偶。")
