...
def loyalty_flavor(
    ...
    # get env file data
    flavor, bundle_id, app_name, app_icon_path = getEnv(flavor, root, console=console)

    with console.status(
            "Updating android settigns", spinner="arc", spinner_style="bold green"
    ) as status:
        updateGradle(root, bundle_id, app_name)
        status.update("Updating app icon")
        changeAppIcon(root, app_icon_path.replace("/", os.sep))
...
def izpay(
    ...
    # change the flavor in the app
    # get env file data
    flavor, bundle_id, app_name, app_icon_path, secondary_color = getEnv(
    ...
    with console.status(
        ...
        if platform == "all" or platform == "android":
            ...
            updateGradle(root, bundle_id, app_name, project="izpay")
        status.update("Updating app icon")
        changeAppIcon(root, app_icon_path.replace("/", os.sep), project="izpay")
        status.update("Updating Splash Screen")
        changeSplash(
            root, app_icon_path.replace("/", os.sep), secondary_color, project="izpay"
        )
        status.update("Updating onesignal icons")
