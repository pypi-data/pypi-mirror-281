...
class Simulation:
        ...
        for i in range(grid_size):
            for j in range(grid_size):
                if agent_count < num_agents:  # Check if more agents need to be placed
                    ...
                    robotId = p.loadURDF(
                        "ot_2_simulation_v6.urdf", position, [0, 0, 0, 1], flags=p.URDF_USE_INERTIA_FROM_FILE
                    )
    ...
    def drop(self, robotId):
        ...
        sphereBody = p.createMultiBody(
            baseMass=0.1, baseVisualShapeIndex=visualShapeId, baseCollisionShapeIndex=collision
        )
