...
# encoding: utf-8
# -*- coding: utf-8 -*-
"""CacheTTL v1.0.2 - An elegant LRU TTL Cache decorators with methods cache_info() and cache_clear() that also works with asyncio."""
"""
                _         _____ _____ _
...
def async_cachettl_min(ttl=60, maxsize=None, typed=False):
    ...
    def _decorator(func):
        @functools.lru_cache(maxsize=maxsize, typed=typed)
        async def _new_lrucache(*args, __time_modificator, **kwargs):
            return await func(*args, **kwargs)

        @functools.wraps(func)
        async def _wrapped(*args, **kwargs):
            return await _new_lrucache(*args, **kwargs, __time_modificator=int(int(time.time()) / ttl))
