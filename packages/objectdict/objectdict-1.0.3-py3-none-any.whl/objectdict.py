...
class ObjectDict(dict):
    ...
    def __init__(self, *args, **kwargs):
        for k, v in self.__class__.__dict__.items():
            if not k.startswith('_') and not isfunction(v) and not isinstance(v, (classmethod, staticmethod, property)):
                self[k] = copy.deepcopy(v) if isinstance(v, (dict, set, list, tuple)) else v
