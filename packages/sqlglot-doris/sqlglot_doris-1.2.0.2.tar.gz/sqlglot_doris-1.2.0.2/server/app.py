...
DORIS_FUNCTION_MAP = {"doris-2.1.4-rc03": ["replace_empty"], "doris-2.1.5-rc01": []}


def version_to_tuple(version_str):
    """Convert complex version strings to tuples for comparison"""
    # Extract the version number, skipping the prefix (assuming there is a prefix before the version number, such as "doris-" and "selectdb-doris-")
    version_str = version_str.split("-", 1)[1] if "-" in version_str else version_str
    # Use regular expressions to match numbers and pre-release information in version numbers
    version_pattern = r"(?:.*-)?(\d+)\.(\d+)\.(\d+)(?:-rc(\d+))?"
    match = re.match(version_pattern, version_str)
    if not match:
        raise ValueError("Invalid version format")

    # Convert the matched version number part to an integer, ignoring groups that are None
    version_parts = [int(x) for x in match.groups() if x and not x.startswith("-rc")]
    rc_part = [int(x[2:]) for x in match.groups() if x and x.startswith("rc")]
    version_tuple = tuple(version_parts + rc_part)
    return version_tuple


def find_unknown_functions(doris_version, used_functions):
    """Checks whether a given Doris version uses functions that are unique to later versions.

    Parameter:
    - doris_version: String, specifying the version of Doris, in the format of "doris-2.1.4-rc02"
    - used_functions: List containing the function name strings used
    """

    # Convert the input Doris version string into a tuple
    doris_version_tuple = version_to_tuple(doris_version)
    unknown_functions = []
    for version, functions in DORIS_FUNCTION_MAP.items():
        # Convert the version string in the mapping table into a tuple for comparison
        if version_to_tuple(version) > doris_version_tuple:
            for func in functions:
                if func.upper() in used_functions and func.upper() not in unknown_functions:
...
def convert():
    ...
    doris_version = j_data["doris_version"]
    if unknown_function == set() and doris_version != "doris-0.0.0-":
        unknown_function = find_unknown_functions(doris_version, transformedSQL)
        logging.info(f"find_unknown_functions: {unknown_function}")
    ...
    if response_code == 0 and unknown_function_tag == 0:
        ...
        message_info = f"Unknown_function:{unknown_function}"
    else:
