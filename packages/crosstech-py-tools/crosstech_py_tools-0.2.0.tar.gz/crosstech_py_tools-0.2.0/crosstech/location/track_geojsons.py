...
class TrackGeoJSON:
    ...
    @classmethod
    def download_multiple(cls, tracks: pd.DataFrame = None, **kwargs) -> gpd.GeoDataFrame:
        """
        ...
        **kwargs : dict, optional
        ...
        show_progres = kwargs.get("show_progres", True)
        save = kwargs.get("save", False)

        if use_cache:
            logging.warning("Using cache! Data may be incomplete / or outdated.")
            with open("tracks.pkl", "rb") as f:
                return gpd.GeoDataFrame(pickle.load(f))

        if tracks is None:
            raise ValueError("Please provide a DataFrame with 'track_id' and 'geojson' columns.")
