...
class SQLAInterface:
    ...
    def get_type(self, col: str) -> Type:
        ...
        if self.is_text(col) or self.is_string(col):
            ...
            return datetime
        elif self.is_relation_one_to_one(col) or self.is_relation_many_to_one(col):
            return self.get_related_interface(col).schema
        elif self.is_relation_one_to_many(col):
            return list[self.get_related_interface(col, with_fk=False).schema]
        elif self.is_relation_many_to_many(col):
            return list[self.get_related_interface(col, with_fk=False).schema]
        else:
            return Any
    ...
    def get_type_name(self, col: str) -> str:
        ...
        """
        return self.list_columns[col].type.__class__.__name__
