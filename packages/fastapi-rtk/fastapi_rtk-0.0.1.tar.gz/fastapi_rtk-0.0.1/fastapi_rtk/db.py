    ...
    async def add_options(
        ...
        *,
        ...
        for col, filter_class, value in filter_classes:
            await self.filter_class(col, filter_class, value)
...
class DatabaseSessionManager(BaseDatabaseSessionManager):
    ...
    _engines: dict[str, AsyncEngine] = None
    _async_session_makers: dict[str, async_sessionmaker[AsyncSession]] = None
    ...
    def init_db(self, url: str, binds: dict[str, str] | None = None):
        ...
        if binds:
            ...
            for key, value in binds.items():
                self._engines[key] = create_async_engine(value)
                self._async_session_makers[key] = async_sessionmaker(
    ...
    async def close(self):
        ...
        if self._engines:
            ...
            self._engines = None
            self._async_session_makers = None
    ...
    async def connect(self, bind: str | None = None) -> AsyncIterator[AsyncConnection]:
        ...
        if bind:
            ...
            async with self._engines[bind].begin() as connection:
                try:
                    yield connection
                except Exception:
    ...
    async def session(self, bind: str | None = None) -> AsyncIterator[AsyncSession]:
        ...
        """
        if bind:
            if self._async_session_makers is None:
                raise Exception("DatabaseSessionManager is not initialized")
            session = self._async_session_makers[bind]()
            try:
                yield session
            except Exception:
    ...
    async def create_all(self, binds: list[str] | Literal["all"] | None = "all"):
        ...
        if not self._engines or not binds:
            return

        bind_keys = self._engines.keys() if binds == "all" else binds
