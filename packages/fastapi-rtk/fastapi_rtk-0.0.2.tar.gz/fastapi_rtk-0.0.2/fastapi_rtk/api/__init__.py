...
from ..utils import dump_schema, generate_report, merge_schema, update_self_signature
...
class ModelRestApi(BaseApi):
    ...
    def _init_info_endpoint(self):
        ...
        async def get_info(
            *,
            ...
            db: AsyncSession = Depends(get_db(self.__bind_key__)),
        ):
    ...
    def _init_download_endpoint(self):
        ...
        async def download(
            ...
            return await self.download_headless(body, db)

    async def download_headless(self, body: QueryBody, db: AsyncSession):
        """
    ...
    def _init_bulk_endpoint(self):
        ...
        async def bulk(
            ...
            return await self.bulk_headless(handler, body, db)

    async def bulk_headless(self, handler: str, body: dict | list, db: AsyncSession):
        """
    ...
    def _init_list_endpoint(self):
        ...
        async def get_all(
            ...
            return await self.get_list_headless(q, db)

    async def get_list_headless(self, q: QuerySchema, db: AsyncSession):
        """
        ...
        await self.pre_get_list(body, {"q": q, "db": db})
        return body
    ...
    def _init_get_endpoint(self):
        ...
        async def get_one(
            ...
            return await self.get_headless(id, db)

    async def get_headless(self, id: str | int, db: AsyncSession):
        """
        ...
        await self.pre_get(body, {"id": id, "db": db})
        return body
    ...
    def _init_create_endpoint(self):
        ...
        async def create(
            ...
            return await self.post_headless(body, db)

    async def post_headless(self, body: BaseModel, db: AsyncSession):
        """
        ...
        await self.pre_add(item, {"body": body, "db": db})
        db.add(item)
        await db.commit()
        await self.post_add(item, {"body": body, "db": db})
        pk, data = self._convert_to_result(item)
    ...
    def _init_update_endpoint(self):
        ...
        async def update(
            ...
            return await self.put_headless(id, body, db)

    async def put_headless(self, id: str | int, body: BaseModel, db: AsyncSession):
        """
        ...
        await self.pre_update(item, {"body": body, "db": db})
        await db.commit()
        await self.post_update(item, {"body": body, "db": db})
        pk, data = self._convert_to_result(item)
        body = self.add_return_schema(id=pk, result=data)
        return body
    ...
    def _init_delete_endpoint(self):
        ...
        async def delete(
            ...
            return await self.delete_headless(id, db)

    async def delete_headless(self, id: str | int, db: AsyncSession):
        """
        ...
        if not item:
            raise HTTPException(status_code=404, detail="Item not found")
        await self.pre_delete(item, {"id": id, "db": db})
        await db.delete(item)
        await db.commit()
        await self.post_delete(item, {"id": id, "db": db})
