...
def encasulated_advanced_pubmed_paper_counter():
    ...
    def search_and_store_papers(query_terms, filename, default_search_location, threshold, retmax, start_date=None, end_date=None):
        try:
    ...
    def root_PubMed_search_query():
        try:
            ...
            retmax = input("Enter the maximum number of records to retrieve (retmax, optional): ")
            retmax = int(retmax) if retmax else threshold

            if retmax > threshold:
                print(f"retmax ({retmax}) is greater than threshold ({threshold}). Setting retmax to threshold.")
                retmax = threshold
            ...
            search_and_store_papers(query_terms, output_file_pubmed_paper_query_name, default_search_location, threshold, retmax, start_date=start_date, end_date=end_date)
...
def entry_point_02A(): # maybe in subsequent updates called by entry_point_01A
    ...
    def getabstracts():
        ...
        def fetch_pubmed_details(pubmed_ids_batch):
        ...
        def create_xml(pubmed_details, pubmed_ids_batch):
            root = ET.Element("PubmedArticleSet")

            with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
                futures = [executor.submit(fetch_abstract_and_create_xml_entry, uid, pubmed_details, root) for uid in pubmed_ids_batch]
        ...
        for pubmed_ids_batch in split_into_batches(pubmed_ids):
            ...
            pubmed_details = fetch_pubmed_details(pubmed_ids_batch)

            # Generate XML for the current batch
            print("Generating XML for the current batch...")
            batch_xml_tree = create_xml(pubmed_details, pubmed_ids_batch)
...
def entry_point_02B():
    ...
    def transform_xml_to_html(xml_file, xslt_content, output_file):
        # Load XML file
    ...
    xslt_content = """
        ...
        <xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
            ...
            <html>
              ...
              <body>
                <div class="container">
                  ...
                  <xsl:apply-templates>
                    <xsl:sort select="MedlineCitation/YearOfPublication" order="{$sortOrder}"/>
                  </xsl:apply-templates>
                </div>
              </body>
            </html>
          ...
          <xsl:template match="PubmedArticle">
            <div class="article" data-pubmed-id="{MedlineCitation/@PMID}" data-year="{MedlineCitation/YearOfPublication}">
              <input type="checkbox" class="remove-checkbox checkbox" value="{MedlineCitation/@PMID}"/> Remove Article
              <span><strong>Year:</strong> <xsl:value-of select="MedlineCitation/YearOfPublication"/></span>
              <button class="copy-citation-button" onclick="copyToClipboard(
                'APA Citation: {MedlineCitation/@PMID}, {MedlineCitation/ArticleTitle}. ({MedlineCitation/YearOfPublication}). {substring-before(MedlineCitation/DOI, 'doi: ')}{substring-after(MedlineCitation/DOI, 'doi: ')}'
              )">Copy APA Citation</button>
              <div class="article-title"><xsl:value-of select="MedlineCitation/ArticleTitle"/></div>
              <div class="abstract-content">
                <p><strong>PubMed ID:</strong> <xsl:value-of select="MedlineCitation/@PMID"/></p>
                <p>
                  <strong>DOI:</strong>
                  <xsl:choose>
                    <xsl:when test="string(MedlineCitation/DOI)">
                      <a href="https://doi.org/{substring-after(MedlineCitation/DOI, 'doi: ')}">
                        <xsl:value-of select="MedlineCitation/DOI"/>
                      </a>
                    </xsl:when>
                    <xsl:otherwise>
                      No DOI available
                ...
                <p><strong>Abstract:</strong> <xsl:value-of select="MedlineCitation/Abstract"/></p>
              </div>
            </div>
          </xsl:template>
    ...
    transform_xml_to_html(xml_file, xslt_content, output_file)
