...
class ClusterDB:
    ...
    @staticmethod
    @log_exception_with_args
    def get_clusters_export_info(
            dataset_id: UUID,
            seed_cluster_ids: List[UUID]) -> List[dict]:
        stmt = """
            with recursive cte as (
                select * from clusters where id = ANY(:cluster_ids) AND 
                clusters.dataset_id = :dataset_id
                union all
                select clusters.* from clusters join cte on clusters.parent_id = cte.id
            )
            select                
                images.id as image_id,
                images.dataset_id as dataset_id,
                cte.id as cluster_id,
            ...
            where
                cte.id = images_to_clusters.cluster_id and
                images.id = images_to_clusters.image_id
        """
        with get_session() as session:
            return session.execute(
                sa.text(stmt), {'cluster_ids': seed_cluster_ids, 'dataset_id': dataset_id}).mappings().all()
