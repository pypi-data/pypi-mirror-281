...
def test_single_scan_single_detector_without_qt(tmpdir):
    ...
    assert_single_scan_single_detector_without_qt(filename, tmpdir)


def test_single_scan_single_detector_with_qt(ewoks_orange_canvas, tmpdir):
    from orangecontrib.ewoksfluo.categories.demo import tutorials

    filename = resource_files(tutorials).joinpath(
        "mesh_single_scan_single_detector.ows"
    )
    assert_single_scan_single_detector_with_qt(ewoks_orange_canvas, filename, tmpdir)


def assert_single_scan_single_detector_without_qt(filename, tmpdir):
...
def assert_single_scan_single_detector_with_qt(ewoks_orange_canvas, filename, tmpdir):
