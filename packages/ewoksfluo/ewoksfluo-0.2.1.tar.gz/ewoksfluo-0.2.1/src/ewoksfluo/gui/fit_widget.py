...
class OWFitWidget(OWEwoksWidgetOneThread, **ow_build_opts):
    ...
    def _init_control_area(self, multiscan: bool = False, multidetector: bool = False):
        ...
        defaults = {
            **DEFAULTS,
            "output_uri_template": "processed_data.h5::/results/fit/{}",
        }
        ...
        parameters["output_uri_template"] = {
            "label": "Output URI",
            "value_for_type": "",
            "select": "h5group",
        }
    ...
    def _get_output_filenames(self):
        if self._multidetector:
            uris = self.get_task_output_value("fit_results_uris")
            if not uris:
                return list()
            return [split_h5uri(uri)[0] for uri in uris]
        else:
            uri = self.get_task_output_value("fit_results_uri")
            if not uri:
                return list()
    ...
    def _refresh_output_file(self):
        ...
        for filename in filenames:
            self._viewer.updateFile(filename, mode="a")
