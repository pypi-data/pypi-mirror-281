...
def change_unpaid_status(url : str , path : str ,query : dict, data : dict, admin_name : str, admin_id : str):
    ...
    try:
        response = session.post(url=url+path,params=query, data=encoded_data, cookies=cookies, timeout=5,headers={"Content-Type": "application/x-www-form-urlencoded; charset=UTF-8"})
        if check.type_name(response) == 'NoneType':
            return {
                "result": False,
                "message": "failed",
