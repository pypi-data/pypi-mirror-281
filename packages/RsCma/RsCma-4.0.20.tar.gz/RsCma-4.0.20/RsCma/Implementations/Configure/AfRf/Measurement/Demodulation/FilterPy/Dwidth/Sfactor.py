...
class SfactorCls:
	...
	def set(self, factor_left: float, factor_right: float) -> None:
		...
		Snippet: driver.configure.afRf.measurement.demodulation.filterPy.dwidth.sfactor.set(factor_left = 1.0, factor_right = 1.0) \n
		Sets the distortion filter width factor for a user-defined distortion filter width. CONF:AFRF:MEAS:DEM:FILT:DWID UDEF For
		FM stereo, the settings configure the left and the right audio channel. For other modulation types, only <FactorLeft> is
		relevant. <FactorRight> has no effect. \n
		...
		self._core.io.write(f'CONFigure:AFRF:MEASurement<Instance>:DEModulation:FILTer:DWIDth:SFACtor {param}'.rstrip())
	...
	def get(self) -> SfactorStruct:
		"""SCPI: CONFigure:AFRF:MEASurement<Instance>:DEModulation:FILTer:DWIDth:SFACtor \n
		Snippet: value: SfactorStruct = driver.configure.afRf.measurement.demodulation.filterPy.dwidth.sfactor.get() \n
		Sets the distortion filter width factor for a user-defined distortion filter width. CONF:AFRF:MEAS:DEM:FILT:DWID UDEF For
		FM stereo, the settings configure the left and the right audio channel. For other modulation types, only <FactorLeft> is
		relevant. <FactorRight> has no effect. \n
			:return: structure: for return value, see the help for SfactorStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:AFRF:MEASurement<Instance>:DEModulation:FILTer:DWIDth:SFACtor?', self.__class__.SfactorStruct())
