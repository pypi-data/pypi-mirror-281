...
class RsCma:
	"""2202 total commands, 21 Subgroups, 0 group commands"""
	_driver_options = "SupportedInstrModels = CMA180, SupportedIdnPatterns = CMA, SimulationIdnString = 'Rohde&Schwarz,CMA180,100001,4.0.20.0031'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core.driver_version = '4.0.20.0031'
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '4.0.20.0031'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsCma version failed. Current version: '4.0.20.0031', minimum required version: '{min_version}'")
