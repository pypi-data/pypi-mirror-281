...
from dealcloud_sdk.utils.common_argument_parse import (
...
class DealCloudData(DealCloudSchema):
    ...
    def list_configured_views(self, is_private: Optional[bool] = None):
        """
    ...
    def _get_choice_reference_fields(self, object_id: Union[str, int]) -> list[Field]:
        ...
        choice_reference_fields = list(
            filter(
                ...
                [c for c in self.get_fields(object_id)],
            )
        )
        return choice_reference_fields
    ...
    def read_data(
        ...
        if fields:
            self._validate_field_mapping(object_id, set(fields))
        ...
        if rows.totalRecords > 1000:
            ...
            if query:
                ...
                result = multi_threaded_request_with_retry(
                    ...
                    max_workers=DATA_READ_WORKER_COUNT
                    if object_id
                    else VIEW_READ_WORKER_COUNT,
                )

            # process rows
            rows = list([Rows(**rows).rows for rows in result])
            rows = flatten_nested_list(rows)
        ...
        if len(rows) == 0:
            ...
            if output == "pandas":
                if fields:
                    ...
                    columns = list([c.apiName for c in self.get_fields(object_id)]) + [
                        "EntryId"
                    ]
        ...
        columns = list([c.apiName for c in self.get_fields(object_id)]) + ["EntryId"]
                ...
                if object_id:
                    ...
                    for choice_ref_field in choice_reference_fields:
                        df[choice_ref_field.apiName] = df[
                            ...
                            lambda x: "; ".join(
                                [str(resolve_object(i, resolve)) for i in x]
                            )
                        )
                else:
                    self._logger.warning(
    ...
    def _resolve_field_value(self, object_name: str, field: Field, value):
        """
        ...
        # parse reference field
            ...
            for ref in field.entryLists:
                if not id_maps:
                    id_maps = self._id_map.get(ref)
                else:
                    id_maps = id_maps.update(self._id_map.get(ref))
            for v in str(value).split("; "):
    ...
    def _validate_field_mapping(self, object_api_name: str, columns: set[str]):
        ...
        """
        column_names = list([i.apiName for i in self.get_fields(object_api_name)])
    ...
    def _build_id_map(
        ...
        if include_object:
            object_data = self.read_data(object_api_name, output="list")
            ...
            self._id_map[object_api_name] = dict(
                {
                    i.get(lookup_column): i.get(list(i.keys())[0])
                    for i in self.read_data(object_api_name, output="list")
                }
            )
        ...
        for referenced_object in referenced_objects:
            if lookup_column:
                self._id_map[referenced_object] = dict(
                    {
                        i.get(lookup_column): i.get(list(i.keys())[0])
                        for i in self.read_data(referenced_object, output="list")
                    }
                )
            else:
                self._id_map[referenced_object] = dict(
                    {
                        i.get(list(i.keys())[1]).get("name"): i.get(list(i.keys())[0])
                        for i in self.read_data(referenced_object, output="list")
                    }
                )
    ...
    def update_data(
        self,
        object_api_name: str,
        data: Union[list[dir], pd.DataFrame],
        use_dealcloud_ids: bool = True,
        lookup_column: Optional[str] = None,
        output: str = "list",
    ) -> Union[list[dict], pd.DataFrame]:
