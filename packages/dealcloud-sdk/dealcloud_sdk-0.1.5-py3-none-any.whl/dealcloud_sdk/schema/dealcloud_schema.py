...
class DealCloudSchema(DealCloudBase):
    ...
    def get_fields(
        self, object_id: Union[int, str] = None, field_id: int = None
        ...
        if isinstance(response.json(), list):
            fields = list([Field.from_api(field) for field in response.json()])
        else:
            fields = Field.from_api(response.json())
    ...
    def get_schema(self, key_type: str = "api") -> Schema:
        ...
        for o in objects:
            ...
            if key_type == "api":
                object_fields = dict({f.apiName: f for f in object_fields})
                schema[o.apiName] = ObjectWithFields(object=o, fields=object_fields)
            elif key_type == "display":
                object_fields = dict({f.name: f for f in object_fields})
                schema[o.name] = ObjectWithFields(object=o, fields=object_fields)
            elif key_type == "id":
                object_fields = dict({f.id: f for f in object_fields})
                schema[o.id] = ObjectWithFields(object=o, fields=object_fields)
            else:
