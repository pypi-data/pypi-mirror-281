...
def page_left_aside(main_content: html.Div, 
                    aside_content: html.Div) -> html.Div:
    """
...
def page_right_aside(main_content: html.Div, 
                     aside_content: html.Div) -> html.Div:
    """
...
def page_left_right_asides(
        page_id: str,
        left_aside: html.Div | None = None,
        main_content: html.Div | None = None,
        right_aside: html.Div | None = None) -> html.Div:
    """
    Generates a page layout with both a left and right aside.
    """
    left_aside = dmc.AppShellNavbar(
        id=f'{page_id}{LEFT_ASIDE_ID}',
        children=dmc.ScrollArea(left_aside),
        zIndex=100,
        style={'padding-inline': '0px', 'width': '16vw', 'padding': '20px'},
        withBorder=True, visibleFrom='md')

    right_aside = dmc.AppShellAside(
        id=f'{page_id}{RIGHT_ASIDE_ID}',
        children=dmc.ScrollArea(right_aside),
        zIndex=100,
        style={'padding-inline': '0px', 'width': '16vw', 'padding': '20px'},
        withBorder=True, visibleFrom='md')
    ...
    main_content = dmc.Stack([
        dmc.Group([
            dmc.Space(visibleFrom='md', style={'margin-left': '16vw'}),
            html.Div(id=f'{page_id}{MAIN_CONTENT_ID}', 
