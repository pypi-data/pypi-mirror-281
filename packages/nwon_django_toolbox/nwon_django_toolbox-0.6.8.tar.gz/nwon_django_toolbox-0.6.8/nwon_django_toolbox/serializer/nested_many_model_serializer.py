            ...
            if create_parameter[key]:
                ...
                for element in elements:
                    if nested_model.field_on_related_model:
                        element[nested_model.field_on_related_model] = created_object.pk
                    ...
                    for key, value in element.items():
                        if isinstance(value, Model):
                            element[key] = value.pk

                    serializer = nested_model.serializer(data=dict(element))
        ...
        for nested_model in nested_models:
            ...
            if update_parameters[key]:
                ...
                for parameter in parameters:
                    ...
                    if (
                        nested_model.field_on_related_model
                        and nested_model.primary_key_field_on_serializer is not None
                    ):
                        model_to_update = nested_model.model.objects.get(
                            pk=parameter[nested_model.primary_key_field_on_serializer]
                        )
