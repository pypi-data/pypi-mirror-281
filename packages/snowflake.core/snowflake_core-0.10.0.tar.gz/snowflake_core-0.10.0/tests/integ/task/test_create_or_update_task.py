...
def test_create_or_update_finalizer(database, schema, tasks):
    task_name1 = random_object_name()
    task_name2 = random_object_name()
    ...
    try:
        ...
        task2 = tasks[task_name2]
        task2.create_or_update(Task(name=task_name2, definition="select current_version()", finalize=task1.name))

        fetched = task2.fetch()
        fetched1 = task1.fetch()
        assert fetched.finalize == f"{database.name.upper()}.{schema.name.upper()}.{fetched1.name}"

        task3 = tasks[task_name3]
        task3.create_or_update(
            Task(
                name=task_name3, definition="select current_version()", schedule=timedelta(minutes=100), config={"a": 1}
            )
        )

        task2.create_or_update(Task(name=task_name2, definition="select current_version()", finalize=task3.name))

        fetched_again = task2.fetch()
        fetched3 = task3.fetch()
        assert fetched_again.finalize == f"{database.name.upper()}.{schema.name.upper()}.{fetched3.name}"
    finally:
        with suppress(Exception):
            tasks[task_name1].delete()
            tasks[task_name2].delete()
