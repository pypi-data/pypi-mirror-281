...
from .types import WorkerContext, WorkerTask, YieldResult
...
class CrawlerWorker(Stoppable):
    ...
    async def loop_done(self, session: Session) -> bool:
    ...
    async def _process_todo_message(self, message):
        ...
        try:
            ...
            if qm.type == QueueMessageType.Task:
                ...
                is_loop_done = False
                while True:
                    is_loop_done = await self.loop_done(session)
                    if is_loop_done:
                        break
                    ...
                    if plugin == MAX_INSTANCES_ERROR:
                        ...
                        continue
                    break
                if is_loop_done or plugin is None or plugin == MAX_INSTANCES_ERROR:
                    logger.info(f"loop done or no plugin or max instances, rejecting {is_loop_done=} {plugin=}")
                    await self.todo_queue.reject(message, requeue=is_loop_done or plugin == MAX_INSTANCES_ERROR)
                ...
                for attempt in range(0, self.cfg.ATTEMPTS_PER_URL):
                    ...
                    try:
                        ...
                        async for process_res in plugin.process(task):
                            ...
                            is_loop_stopped = await self.loop_done(session)
                            if is_loop_stopped:
                                break

                        logger.info("plugin.process done")
                        if is_session_postponed is False:
                            await self._set_task_status(
                                qm.session_id,
                                task,
                                CrawlerHintURLStatus.Success if not is_loop_stopped else CrawlerHintURLStatus.Canceled,
                            )
    ...
    def _get_plugin_object(self, cls, session: Session) -> BasePlugin:
        ctx = WorkerContext(session=session, last_result=YieldResult.NoResult)  # type: ignore
