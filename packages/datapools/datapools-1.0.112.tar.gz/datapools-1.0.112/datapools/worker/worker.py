...
class CrawlerWorker(Stoppable):
        ...
        while not await self.is_stopped(timeout=timeout):
            ...
            if message:
                ...
                if qm.type == QueueMessageType.DelayedTask:
                    if time.time() - qm.data["timestamp"] > self.cfg.DELAYED_TASK_REQUEUE_PERIOD:
        ...
        try:
            ...
            if qm.type == QueueMessageType.Task:
                ...
                if loop_result is not None or plugin is None or plugin == MAX_INSTANCES_ERROR:
                    logger.info(f"loop done or no plugin or max instances, rejecting {loop_result=} {plugin=}")
                ...
                for attempt in range(0, self.cfg.ATTEMPTS_PER_URL):
                    ...
                    try:
                        ...
                        async for process_res in plugin.process(task):
                            ...
                            if t is CrawlerContent:
                                ...
                                logger.info(f"TryLater, postponing session {session.id=}")
                                await session.postpone()
                                is_session_postponed = True
                                break
                        ...
                        if is_session_postponed is False and loop_result is None:
                            is_task_done = True
                        break
                    except Exception as e:
                    ...
                    if is_task_done or is_session_postponed:
                        break
                ...
                if is_task_done:
                    ...
                    await self._set_task_status(session, task, CrawlerHintURLStatus.Success)
                else:
                    logger.info(f"sending reject for {message.message_id=} {loop_result=} {is_session_postponed=}")
                    requeue = loop_result == LoopResult.WorkerStopped and not is_session_postponed
                    await self.todo_queue_r.reject(message, requeue=requeue)
                    status = (
                        CrawlerHintURLStatus.Failure
                        if not is_session_postponed and loop_result != LoopResult.SessionClosed
                        else CrawlerHintURLStatus.Canceled
                    )
                    await self._set_task_status(session, task, status)
    ...
    async def _set_task_status(self, session: Session, task: WorkerTask, status: CrawlerHintURLStatus):
        await session.set_url_status(task.url, status)
        await self.reports_queue.push(
            QueueMessage(session.id, QueueMessageType.Report, {"task": task.to_dict(), "status": status.value})
        )
    ...
    async def _process_crawled_content(
        ...
        if True:  # not await session.has_content(cc.url):
