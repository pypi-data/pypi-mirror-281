...
class CrawlerWorker(Stoppable):
    ...
    async def delayed_tasks_loop(self):
        while not await self.is_stopped(timeout=5):
            ...
            if message:
                ...
                if qm.type == QueueMessageType.DelayedTask:
                    if time.time() - qm.data["timestamp"] > self.cfg.DELAYED_TASK_REQUEUE_PERIOD:
                        # enqueue task back to the main worker queue
        ...
        try:
            ...
            if qm.type == QueueMessageType.Task:
                ...
                if is_task_done:
                    ...
                    await self.todo_queue_r.reject(message, requeue=status == CrawlerHintURLStatus.Canceled)
