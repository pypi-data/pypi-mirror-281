...
class RoboticArm(ArmState, MovePlan, JointConfigSettings, JointConfigReader, ArmTipVelocityParameters,
    ...
    """机械臂连接、断开、日志设置等操作
    """
    # @brief 初始化RoboticArm类并设置相关参数
    # @param thread_mode int: 设置线程模式
    #     - 0: 单线程模式，单线程阻塞等待数据返回
    #     - 1: 双线程模式，增加接收线程监测队列中的数据
    #     - 2: 三线程模式，在双线程模式基础上增加线程监测UDP接口数据
    # @param ip str: 机械臂的IP地址
    # @param port int: 机械臂的端口号
    # @param level int, optional: 日志打印等级，默认为3
    #     - 0: debug模式
    #     - 1: info模式
    #     - 2: warning模式
    #     - 3: error模式
    # @param log_func CFUNCTYPE, optional: 自定义日志打印函数（当前Python版本API暂不支持）
    # @return None: 该方法不返回任何值，而是初始化对象并设置相关属性
    # @code
    # 以下是一个使用RoboticArm类进行机械臂控制的基本示例：
    # >>> from rm_robot_interface import RoboticArm
    # >>> arm = RoboticArm(thread_mode=2, ip="192.168.1.18", port=8080, level=3)
    # >>> zero_position = [0, 0, 0, 0, 0, 0]
    # >>> target_position = [0, 0, 0, 0, 90.0, 0]
    # >>> arm.movej(zero_position, 20, 0, 0, 1)  # 移动机械臂到零位
    # >>> arm.movej(target_position, 20, 0, 0, 1)  # 移动机械臂到目标位置
    # >>> arm.rm_delete_robot_arm()  # 断开与机械臂的连接
    # @endcode

    def __init__(self, thread_mode, ip, port, level=3, log_func=None):
    ...
    def rm_get_robot_info(self):
        ...
        @return tuple: 包含两个元素的元组。  
            - int: 函数执行的状态码，0表示成功，非0表示出错，具体错误码参考相关文档。  
            - rm_robot_info_t: 包含机器人的自由度、型号、末端力传感器类型等基本信息。  
        """
        info = rm_robot_info_t()
    ...
    def rm_get_arm_event_call_back(self, event_callback: rm_event_callback_ptr):
        ...
        @attention 单线程无法使用该回调函数 
        @param event_callback (rm_event_callback_ptr): 机械臂事件回调函数，该回调函数接收rm_event_push_data_t类型的数据作为参数，没有返回值
        ...
        # 当机械臂事件发生时，`event_callback`函数将被调用，并接收一个包含事件数据的对象作为参数  
            ...
            >>> arm = RoboticArm(2,"192.168.1.18", 8080,level=3)
    ...
    def rm_realtime_arm_state_call_back(self, arm_state_callback):
        ...
        @param arm_state_callback (rm_realtime_arm_state_callback_ptr): 
                机械臂实时状态信息回调函数
