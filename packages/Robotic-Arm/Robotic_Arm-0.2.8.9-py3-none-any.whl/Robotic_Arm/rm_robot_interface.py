...
class RoboticArm(ArmState, MovePlan, JointConfigSettings, JointConfigReader, ArmTipVelocityParameters,
    ...
    def rm_create_robot_arm(self, ip: str, port: int, level: int = 3, log_func: CFUNCTYPE = None):
        """
        ...
        Returns:
            None: 此方法不返回任何值，而是初始化对象的状态。

        Notes:
            在进行机械臂连接之前，需首先调用rm_init函数进行线程模式设置。
        ...
        Example:
            ...
            # 创建RoboticArm对象
            >>> arm = RoboticArm(ip="192.168.1.18", port=8080, level=3)
            # 定义零位和目标位置（关节角度）
