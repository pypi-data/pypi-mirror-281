...
class MessageManager:
    ...
    def add_to_update_message(
        ...
        to_update_message: str = (
            f"{name} - {get_color(current_version, 'yellow')
                        } -> {get_color(latest_version, 'red')}"
        )
    ...
    def add_warning_message(self, name: str, version: str, reason: str) -> None:
        warning_message: str = (
            f"{
            name} - {get_color(version, 'yellow')} ({reason})"
        )
        ...
        for message in messages:
            for symbol in (message["icon"], message["icon_alt"], message["text"]):
                try:
                    click.echo(
                        f"{message['message']} {
                            get_color(click.style(symbol, bold=True), message['color'])}"
                    )
                    break
