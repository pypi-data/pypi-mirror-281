...
def _find_all_overhead_sats(space_track_credentials, ground_loc, time_list, eclipsed = False, method = 'krag'):
	'''
	...
	for i in range(len(over_indices[0])):
		...
		try:
			...
			if sat['NORAD_CAT_ID'] in overhead_sats:
				...
				overhead_sats[sat['NORAD_CAT_ID']].append({'name':sat['OBJECT_NAME'],'time': time_list[t].datetime.isoformat(), 'alt':altaz[s,t].alt.dms, 'az':altaz[s,t].az.dms, 'TLE_LINE1':sat['TLE_LINE1'], 'TLE_LINE2':sat['TLE_LINE2'], 'AVM' : avm, 'cloud_cover' : cloud_cover[time_list[t].datetime.hour]})
			else:
				overhead_sats[sat['NORAD_CAT_ID']] = [{'name':sat['OBJECT_NAME'],'time': time_list[t].datetime.isoformat(), 'alt':altaz[s,t].alt.dms, 'az':altaz[s,t].az.dms, 'TLE_LINE1':sat['TLE_LINE1'], 'TLE_LINE2':sat['TLE_LINE2'], 'AVM' : avm, 'cloud_cover' : cloud_cover[time_list[t].datetime.hour]}]
...
if __name__ == '__main__':
	...
	daytime = datetime(2024, 6, 18, hour=16, minute=30)
