...
class ChatBot:
    def __init__(self, api_key: str):
        ...
        """
        self.client = OpenAI(api_key=api_key)  # Initialize the OpenAI client with API key
        self.history = [{"role": "system", "content": "You are a helpful assistant."}]  # Initialize conversation history
    ...
    def convert_to_embeddings(self, sentences: List[str]) -> List[List[float]]:
        ...
        for sentence in sentences:
            ...
            response = self.client.embeddings.create(
                input=sentence, model="text-embedding-ada-002"  # Ensure the correct model is used
            )
    ...
    def rag(self, prompt: str, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['similarity_score'] = [
            self.quantized_influence(prompt_embedding, df['questions_embed'][i], k=16, use_dagger=False)[0]
    ...
    def run_rag(self, openai_api_key: str, current_prompt: str, df: pd.DataFrame, top_n: int = 2) -> str:
        """
        ...
        references = ' '.join(
            ["Q: " + df_short['questions'].to_list()[i] + "; A: " + df_short['answers'].to_list()[i] for i in range(top_n)]
        )

        # Generate the final response based on the references
        response = bot.generate_response(f"""
