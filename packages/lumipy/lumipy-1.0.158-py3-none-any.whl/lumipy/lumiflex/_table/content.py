...
from pydantic import StrictStr, Field, PositiveInt, NonNegativeInt
...
class CoreContent(BaseContent):
    ...
    label_: StrictStr = Field('content', const=True, alias='label')
    table: BaseTable
    select_cols: Tuple
    ...
    def _validate_content(cls, values):
        if 'select_cols' not in values or 'table' not in values:
            return values

        table = values['table']

        select_cols = values['select_cols']
        ...
        where_filter = values['where_filter']
        ...
        group_by_cols = values['group_by_cols']

        agg_cols = values['aggregates']

        having_filter = values['having_filter']

        order_bys = values['order_bys']

        # construct parents so column DAGs are linked to the main query DAG
        where_filter = tuple() if where_filter is None else (where_filter,)
        having_filter = tuple() if having_filter is None else (having_filter,)
        values['parents_'] = select_cols + where_filter + group_by_cols + agg_cols + having_filter + order_bys
        return values
