...
class AivBot:
    ...
    def getOutPath(self,childPath):
        ''' 2024.3
            获取输入文件的目录
            @param childPath : 需要创建的子目录
            @return : 返回一个在系统输出目录下的子目录 (一般在 d:/aivc/data/out)
    ...
    def getFileList(self):
        ''' 2024.6        
            @return : 任务的文件列表 (数据格式与后面的一致: aivagent.aivtool.py 的 getFileInfo() 函数返回数据格式 )
            获取客户端发送给Bot的文件列表
            列表是已经下载并保存在本地的文件路径、大小、文件名等信息(一般是保存在 d:/aivc/data/temp 目录下)
            (客户端发送的文件一般只保留24小时, 严格的即用即删除)
            * 也可以直接读取 aivBot.task 里面的'paramIn'对象
        '''
        from loguru import logger
        fileList = []
    ...
    def getParamList(self,type= None, paramName= None, task = None, param = 'paramIn'):
        ''' 2024.6
            获取客户端在task中的某类参数 (也可以取某类参数中的某个参数)
            ...
            @param task : 如果指定此值, 则可以解析此task的值, 获取task的 'paramIn' 或 'paramOut' 内容
            @param param : 可以指定读取哪个字段的内容, 可选值是 'paramIn' , 'paramOut'
            @return :返回值是一个列表 [] , 注意!
            * 也可以直接读取 aivBot.task里面的'paramIn'对象
        '''
        if task is None:
            task = self.task
        
        paramIn = task[param] # 可以读出 'paramIn' 或 'paramOut'
        if(type is None):
            return paramIn
        else:
            for paramLs in paramIn:
                if paramLs['type'].lower() == type.lower():
                    if paramName is None:
                        return paramLs['data']  #返回所有数据 (如果是'string',可能有多个 'string'类型的参数在  paramIn 对象中; 而如果 'file'类型,则都只会写入一个对象中) 2024.6
                    
                    else:
                        if paramLs['name'].lower() == paramName.lower(): # 比如获取 'prompt'参数的值:  getParam('string','prompt')
                            return paramLs['data']  #读出所有行 (paramLs['data'] 是一个列表)
