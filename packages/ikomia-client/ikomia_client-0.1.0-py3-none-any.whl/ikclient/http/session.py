...
class ExtraSessionMixin:
    ...
    def raise_for_status(self, response: httpx.Response):
        ...
        if response.status_code == 520:
            request = response._request  # pylint: disable=W0212
...
class AsyncSession(httpx.AsyncClient, ExtraSessionMixin, PollerMixin):
    ...
    # Semaphore to limit the number of concurrent run w.r.t to Ikomia Scale capabilities
    ...
    @classmethod
    def set_concurrent_call(cls, count: int):
        """
        ...
        Raises:
            ValueError: when try to set a value upper than AsyncClient.concurrent_call_limit
        """
        if count > cls.concurrent_call_limit:
            raise ValueError(f"Can't set more than {cls.concurrent_call_limit} concurrent call")
        cls.semaphore = asyncio.Semaphore(count)
