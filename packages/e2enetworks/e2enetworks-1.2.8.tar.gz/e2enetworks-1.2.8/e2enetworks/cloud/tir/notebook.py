...
class Notebooks:
    ...
    def list_attached_ssh_keys(self, notebook_id: int):
        ...
        response = client.Default.make_request(req)
        if response.status_code == 200:
            ssh_keys = response.json()["data"]["ssh_keys"]
            ssh_keys_table = PrettyTable()
            ssh_keys_table.field_names = ['Label', 'SSH Key']
            for ssh_key in ssh_keys:
                ssh_keys_table.add_row([ssh_key['label'], ssh_key['ssh_key']])
            return ssh_keys_table
        return prepare_object(response)
