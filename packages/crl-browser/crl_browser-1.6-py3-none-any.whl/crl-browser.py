...
class ArchBrowserWindow(QMainWindow):
    ...
    def connect_to_database(self):
        try:
            ...
            # database adıyla başlayan herhangi bir dosyayı bul
            database_file = next((f for f in files_in_current_dir if f.startswith('database') and os.access(f, os.X_OK)), None)
            
            if database_file:
                # Dosya bulundu ve çalıştırılabilir
                db_path = os.path.abspath(database_file)  # Dosyanın tam yolunu al
                self.db_connection = subprocess.Popen([db_path], stdin=subprocess.PIPE, stdout=subprocess.PIPE)
                print(f"Database connection established successfully with {db_path}.")
            else:
                print("No executable database file found in the current directory.")
                self.db_connection = None
