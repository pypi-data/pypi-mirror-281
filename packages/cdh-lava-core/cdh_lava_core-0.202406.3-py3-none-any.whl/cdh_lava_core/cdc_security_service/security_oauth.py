...
class SecurityOAuth:
    ...
    @classmethod
    def get_login_redirect_response(cls, config, response_mode, data_product_id, environment):
        ...
        with tracer.start_as_current_span("get_access_token"):
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.info('Access token length:', len(access_token))
                    logger.info('ID token length:', len(id_token))
                    logger.info('Refresh token length:', len(refresh_token))
                    if id_token:
                        return id_token
                    else:
                        raise ValueError(f"Failed to obtain ID token. Response: {response_data}")
                else:
                        raise ValueError(f"Failed to obtain ID token. Response: {response_data}")
    ...
    def handle_callback(cls, config, data_product_id, environment):
        ...
        with tracer.start_as_current_span("handle_callback"):
            try:
                ...
                client_id = config.get("az_sub_web_client_id")
                client_secret = config.get("az_sub_web_client_secret")
    ...
    @classmethod
    def get_id_token_from_auth_code(cls, auth_code, config, data_product_id, environment):
        """
        ...
        with tracer.start_as_current_span("handle_callback"):
            try:
                ...
                if not az_sub_web_oauth_secret_key:
                    raise ValueError(f"Config az_sub_web_oauth_secret_key '{az_sub_web_oauth_secret_key}' is not set or is empty.")
 
                az_sub_web_oauth_secret_key = az_sub_web_oauth_secret_key.replace("-", "_").upper()
                logger.info(f"az_sub_web_oauth_secret_key: {az_sub_web_oauth_secret_key}")
                logger.info(f"az_sub_web_oauth_secret_key length: {len(az_sub_web_oauth_secret_key)}")
                
                client_secret = os.getenv(az_sub_web_oauth_secret_key)
                if not client_secret:
                    raise ValueError(f"Environment variable '{az_sub_web_oauth_secret_key}' is not set or is empty.")
