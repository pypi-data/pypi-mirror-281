...
def azure_ad_authentication(func):
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            if id_token is None:
                raise ValueError("No id_token found in request")
            else:
                ...
                logger.info("Set user_id cookie.")
                ...
                logger.info("Set id_token cookie.")
                ...
                logger.info("Cleared redirect_attempted cookie.")
            ...
            # response_mode = "query"
            response = obj_security_oauth.get_login_redirect_response(config, response_mode, DATA_PRODUCT_ID, ENVIRONMENT)
...
class AuthCallback(Resource):
    ...
    def get(self):
        ...
        obj_security_oauth = security_oauth.SecurityOAuth()
        id_token = obj_security_oauth.get_id_token_from_auth_code(auth_code, config, data_product_id, environment)
    ...
    def post(self):
        ...
        try:
            ...
            # Get the redirect_url from the query parameters and unquote it
            # redirect_url = unquote(request.args.get('redirect_url'))
            logger.info("Base64 decoding state parameter.")
            decoded_bytes = base64.urlsafe_b64decode(base64_string)

            # Decode the bytes to a string
            decoded_string = decoded_bytes.decode("utf-8")
            logger.info(f"Decoded string from state parameter: {decoded_string}")
            ...
            # Convert the string to a dictionary
            data = ast.literal_eval(url_decoded_string)
            logger.info(f"Data dictionary: {data}")
            ...
            # Set redirect_url based on the first url that is not the callback url and is not blank or None
            redirect_url = next(
                (url for url in urls if url and "cdc_security/callback" not in url),
                None,
            )
            logger.info(f"redirect_url: {redirect_url}")
            logger.info("Getting ID token from authorization code.")
            ...
            if id_token and "." in id_token and id_token.count(".") >= 2:
                try:
                    logger.info("Decoding ID token without verification.")
                    decoded_token = jwt.decode(
                        id_token, options={"verify_signature": False}
                    )
                    logger.info(f"Decoded ID token: {decoded_token}")
                    ...
                    logger.info(f"Redirecting user to the appropriate URL: {redirect_url}.")
                    # Redirect the user to the home page, or wherever they should go next
                    return response
                    ...
                    response = make_response(
                        {"message": f"Error in decoding ID token: {str(ex)}"}, 502
                    )
                    secure = request.scheme == "https"
                    response.set_cookie(
                        "redirect_attempted",
