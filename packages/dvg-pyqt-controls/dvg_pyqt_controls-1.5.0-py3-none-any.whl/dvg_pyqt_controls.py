...
works as intended and will respect the colors as given by the stylesheet. I.e.,
the button gets colored green or red depending on its state.

This code should be equivalent:

.. code-block:: python

    button = QtWid.QPushButton(**kwargs)
    button.setCheckable(True)
    button.setEnabled(False)
    button.setStyleSheet(SS_LED_INDICATOR)

but it's not. The colors from the stylesheet are not respected anymore!

Follow-up:
    Okay. It's getting weird now. Apparently, it's an intermittent problem
    between different Qt API's. After toggling the button on and off, the color
    /is/ respected. It's the initial display state that is incorrect.
"""
...
SS_GROUP = (
    "QGroupBox {"
        ...
        "font: bold;"
        "padding: 8 0 0 0px;"
        "margin-top: " + ("2ex" if qtpy.API in ("pyqt5", "pyside2") else "1.2ex") + ";}"
    "QGroupBox:title {"
