...
class PartitionManager(FileManager, AtomicProperties): 
    ...
    def read_files(self, file_location: str = None, source: str = None, subfolders: bool = False,
        ...
        try:
            ...
            error_message = f"Error: Source '{source}' is not compatible. Details: {str(e)}"
            print(error_message, file=sys.stderr)
            print("Traceback:", file=sys.stderr)
            traceback.print_exc(file=sys.stderr)
            raise ValueError(error_message) from e
        except Exception as e:
            error_message = f"An unexpected error occurred while reading files: {str(e)}"
            print(error_message, file=sys.stderr)
            print("Traceback:", file=sys.stderr)
            traceback.print_exc(file=sys.stderr)
            raise
    ...
    def read_OUTCAR(self, file_location:str=None, add_container:bool=True, container_index:int=None, verbose=False, **kwargs):
        '''
        '''
        ...
        if isinstance(container_index, int):
            self.add_container(container=new_container[container_index])
        else:
            for SR in new_container:     
                self.add_container(container=SR)
