...
from datachain.lib.parquet import process_parquet
from datachain.lib.webdataset import process_webdataset
from datachain.lib.webdataset_laion import LaionParquet, WDSLaion, process_laion_meta
...
meta = meta_emd.merge(
    meta_pq, on=["emd.file.name", "emd.index"], right_on=["pq.file.name", "pq.index"]
)

res = wds.merge(meta, on="laion.json.uid", right_on="pq.uid")
