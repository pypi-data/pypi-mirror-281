...
def get_server_config(room_id, secret, signaling_base_url) -> ServerConfig:
    ...
    try:
        ...
        logging.debug("error while api request for init-config")
        raise MeetingInitializationError(e)

    logging.debug(f"server config found for room :: {room_id}")
    if data:
...
def aes_decrypt(value):
    try:
        ...
        logging.error("Error during decryption", exc_info=e)
        raise MeetingInitializationError(e)
