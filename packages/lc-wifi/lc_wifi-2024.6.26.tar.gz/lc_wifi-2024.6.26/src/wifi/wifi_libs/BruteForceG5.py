import itertools
from baseG5 import channel_quality
...
def estimate_environment(radars_l, devices_l):
    ...
    for radar in radars_l:
        radar_bw = radar["bandwidth"]
        radar_data = list(filter(lambda device: 
                        device["radar_cpeid"] == radar["cpeid"], aliens_proc)) 
        radarq_l = {chan: channel_quality(chan, radar_data, radar_bw)
                        for chan in radar["possible_channels"]}
...
def get_combs(radars_l, devices_l=None, what_cmbs = 'all'):
    ...
    channels_l = [radar["possible_channels"]
    ...
    if (what_cmbs == 'all'):
        ...
        for idx_r, radar in enumerate(radars_l):
            ...
            # Is it possible to increase channel quality?
            #cur_q = radar.get('qual_current', 0.0)
            #if cur_q == None:
            #    cur_q = 0.0
            #if all(v<=cur_q for k, v in cq_i.items()):
            #    continue

            # Exclude repeated channel qualities (due to channel bonding)
            check_l = []
            uniq_cq_i = {}
            for k, v in cq_i.items():
                if v not in check_l:
                   check_l.append(v)
                   uniq_cq_i.update({k:v})

            # The best channels
            sorted_chans = {k: v for k, v in sorted(uniq_cq_i.items(), 
                                reverse=True, key=lambda item: item[1])}
            top_chans = list(sorted_chans)[-TOP:]
            best_chans.append(top_chans)
...
def run(radars_l, devices_l, what_cmbs='all', dict_out=False):
    """
    Inputs:
        radars_l: list of dictionaries
            Contains JSON like data about radars
    ...
    Calculates all of the possible channel qualities for each radar; 
    ...
    if len(radars_l) > 0 and len(devices_l) > 0:
        ...
        for comb in combs_l:
            ...
            for radar in radars_l:
                ...
                channelq = channel_quality(radar['channel'], cq_data, radar['bandwidth'])
        ...
        for index, radar in enumerate(radars_l):
            ...
            #TODO: results should be in dict; not corrected because used in cluster-optimization-2.4ghz
    return results_l
