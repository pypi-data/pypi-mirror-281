...
class Video:
    ...
    def _get_data(self):
        ...
        if parameters.get("get_stream"):
            self.stream_url = self._get_stream().get('url') if not self.is_live else self._get_stream(_hls=True)
    ...
    def download(self, path: str = ".", quality: str = "best", keep: bool = False, only_audio: bool = False,
                 only_video: bool = False, target_fps: int = -1, target_itag: int = -1, live_duration: int = 10,
                 thumbnail: bool = False, preferred_video_format: str = "mp4", preferred_audio_format: str = "mp3",
                 overwrite: bool = True, chunk_size: int = 1024*1024, force_ffmpeg: bool = False, _raw_number=0,
                 _show_bar=True):
        """
        Downloads an appropriate video/audio stream.
        ...
        now = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")+"_"

        if thumbnail:
            img_path = os.path.join(dir_path, download_title+".png")
            with open(img_path, "wb") as f:
                f.write(requests.get(self.thumbnail.get('url')).content)
        ...
        if os.path.exists(output_path[:-1] + "." + exts[0]) and overwrite and len(stream_urls) == 1:
        ...
        if len(stream_urls) > 1:
            ...
            out = output_path[:-1]+"."+exts[1] if overwrite else output_path[:-1]+now+"."+exts[1]
            ...
            try:
                ...
                os.rename(output_path + now + ffmpeg_str + itags[0] + "." + exts[0], output_path[:-1] + "." + exts[0])
            except:
                pass
...
class GenericExtractor:
    ...
    Methods
    -------
    download(path: str = ".", chunk_size: int = 1024*1024, _show_bar: bool = True)
        path: str - Output path.
        chunk_size: int - Stream download chunk size.
    """
    ...
    def download(self, path: str = ".", chunk_size: int = 1024*1024, _show_bar: bool = True, _retries: int = 4,
                 _request_interval: float = 0.75, **kwargs):
        """
        Downloads an appropriate video/audio stream.
