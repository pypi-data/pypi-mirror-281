...
class TMEasy(object):
    ...
    def cap(self, capture):
	ret, img = captrue.read()
    	if not ret:
        	break
	...
	cv2.putText(img, f"{result['class_name']}: {result['class_confidence']:.2f}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
	return img