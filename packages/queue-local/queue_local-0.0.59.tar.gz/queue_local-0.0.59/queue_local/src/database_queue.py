...
class DatabaseQueue(OurQueue, GenericCRUD, metaclass=MetaLogger, object=queue_logger_object):
    ...
    def push(self, data_dict: dict = None, queue_dict: dict = None) -> int:
        queue_dict=queue_dict or data_dict
        ...
        self.logger.end("...", object={'push_result': queue_id} )
        return queue_id
        # TODO Can we do this everywhere?  [I will make it auto in the meta logger]


    def push_back(self, queue_item: dict = None, data_dict: dict = None) -> None:
        """Push a taken queue item back to the queue."""
        queue_dict  = queue_item or data_dict
        queue_dict = {"queue_status_id": QueueStatusEnum.NEW.value, "process_id": None}
        self.update_by_column_and_value(column_value=queue_item[self.queue_item_id_column_name], queue_dict=queue_dict)

    def get(self, *, action_ids: tuple = (), custom_condition: str = "",data_dict: dict = None) -> dict:
        """
