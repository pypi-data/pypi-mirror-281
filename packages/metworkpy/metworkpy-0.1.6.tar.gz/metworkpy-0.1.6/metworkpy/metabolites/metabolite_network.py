...
def find_metabolite_network_reactions(model: cobra.Model,
    ...
    if method == "pfba":
        ...
        raise ValueError(f"Method must be 'pfba' or 'essential' but received"
                         f"{method}")
    ...
    for metabolite in tqdm(res_df.columns, disable=not progress_bar):
        with MetaboliteObjective(model=model, metabolite=metabolite) as m:
            if method == "essential":
                ...
                res_df.loc[
                    gene_fluxes_max.abs() >=
                    gene_fluxes_min.abs(), metabolite] = gene_fluxes_max[
                    gene_fluxes_max.abs() >= gene_fluxes_min.abs()]
                res_df.loc[
                    gene_fluxes_max.abs() <
                    gene_fluxes_min.abs(), metabolite
                ] = gene_fluxes_min[
                    gene_fluxes_max.abs() <
                    gene_fluxes_min.abs()
                ]
...
class MetaboliteObjective:
    ...
    def __enter__(self):
        ...
        met_sink_reaction = cobra.Reaction(id=self.rxn_added,
