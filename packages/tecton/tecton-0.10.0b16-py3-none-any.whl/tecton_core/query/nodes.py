...
class AdjustAnchorTimeToWindowEndNode(QueryNode):
    ...
    Attributes:
        input_node: The input node to be transformed.
        anchor_time_column_map: A dictionary mapping the anchor time columns to the size of the tiles.
        feature_store_format_version: The feature store format version for the feature view.
    """
    ...
    def as_str(self) -> str:
        return f"For each anchor time column ({list(self.anchor_time_column_map.keys())}), add the following units ({list(self.anchor_time_column_map.values())}) to each timestamp to represent the end of the window."
