...
class AdjustAnchorTimeToWindowEndSparkNode(SparkExecNode):
    ...
    def _to_dataframe(self, spark: pyspark.sql.SparkSession) -> pyspark.sql.DataFrame:
        ...
        for column in self.anchor_time_column_map.keys():
            interval_seconds = core_time_utils.convert_timedelta_for_version(
                self.anchor_time_column_map[column], self.feature_store_format_version
            )
            output_df = output_df.withColumn(column, F.col(column) + interval_seconds)
        return output_df
