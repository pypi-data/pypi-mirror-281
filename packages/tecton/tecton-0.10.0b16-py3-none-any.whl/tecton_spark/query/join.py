...
class _AsofJoinerAndWindowAggregator:
    ...
    def get_range_between_window_spec(
        ...
        extra_partition_col: Optional[str] = None,
    ) -> spark_window.WindowSpec:
        assert (
            extra_partition_col is None or extra_partition_col in self._extra_partition_cols
        ), f"Expected extra_partition_col {extra_partition_col} in {self._extra_partition_cols}."
        assert (
        ...
        extra_partition_cols = [extra_partition_col] if extra_partition_col else []
        window_spec = (
    ...
    def _generate_aggregation_window_spec(
        ...
        if isinstance(time_window, RelativeTimeWindowSpec):
            ...
            return join_spec.get_range_between_window_spec(
                ...
                extra_partition_col=self.sawtooth_aggregation_data.get_anchor_time_to_partition_columns_map()[
                    timestamp_col
                ],
            )
