...
def stat_UIEX_cert_acc():
    ...
    stat_map = {}
    tag = 'all'
    tag_map = PyFile.get_json(rf'E:\Projects\pp-uiex-certs\anns\pred\tag_{tag}.json')
    pred_map = PyFile.get_json(rf'E:\Projects\pp-uiex-certs\anns\pred\pred_{tag}.json')
    for image_id, m in tag_map.items():
        for label, v in m.items():
            ...
            pred_v = label_amend(v.strip(), label, pred_v)
...
def label_amend(tag, label, pred):
    ...
    if label in ['姓名']:
        pred = pred.replace('Full Name', '').replace('#', '')
...
def merge_json():
    PyFile.merge_json(
        list(filter(lambda x: 'pred_' in x, PyFile.get_file_list(r'E:\Projects\pp-uiex-certs\anns\pred'))),
        r"E:\Projects\pp-uiex-certs\anns\pred\pred_all.json",
        data={}
    )
