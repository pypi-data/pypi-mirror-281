...
def load_ncc_file(file_path, active_only=False,support_chrom=None, nmax=int(1e6)):
  """Load chromosome and contact data from NCC format file, as output from NucProcess"""

  from numpy import array
  if not support_chrom:
    support_chrom = ['chr%s' % i for i in range(1, 22)] + ['chrX']
  contact_dict = {}
  ambig_group = 0
  ...
  with open_file(file_path) as file_obj:
    ...
    for i, line in enumerate(file_obj):
      ...
      if chr_a not in support_chrom or chr_b not in support_chrom:
      ...
      if active_only and (int(mapq1) < 30 or int(mapq2) < 30): # inactive
        continue
...
def calc_genome_structure(ncc_file_path, out_file_path, general_calc_params, anneal_params,
                          ...
                          isolation_threshold_trans=int(10e6), out_format=PDB, num_cpu=MAX_CORES,
                          ...
                          remove_models_size=DEFAULT_REMOVE_MODELS_SIZE, support_chrom=None):
  ...
  contact_dict = load_ncc_file(ncc_file_path, support_chrom=support_chrom)
  # print(contact_dict)
  ...
  for stage, particle_size in enumerate(particle_sizes):
    ...
    if particle_size <= struct_ambig_size and not using_inactive:
      contact_dict = load_ncc_file(ncc_file_path, active_only=False,support_chrom=support_chrom)
      using_inactive = True
      
    stage_contact_dict = dict(contact_dict) # Each stage reconsiders all contacts

    anneal_params['temp_steps'] = anneal_params['temp_steps']
    anneal_params['dynamics_steps'] = anneal_params['dynamics_steps']
...
def test_imports(gui=False):
  ...
  try:
    from sc3dg.model import dyn_util
  except ImportError as err:
    import os
    cwd = os.getcwd()
    try:
