...
def run_exec(type_, opt, fq, log_out):
    if type_ in ['sn_m3c', 'scSPRITE', 'scNano']:
...
def count(output, fastq, logging, type, enzyme, resolution, index, sample, exist_barcode, qc, add_columns, thread, worker, select, max_mismatch, aligner, repeat_masker, scnano_barcode, zoomify_res):
