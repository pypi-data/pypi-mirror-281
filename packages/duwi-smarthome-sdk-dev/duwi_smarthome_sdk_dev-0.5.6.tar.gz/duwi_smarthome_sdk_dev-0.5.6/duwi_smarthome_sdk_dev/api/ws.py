...
class DeviceSynchronizationWS:
    ...
    async def connect(self):
        try:
            self._connection = await asyncio.wait_for(websockets.connect(self._server_uri), timeout=5)
    ...
    def start_listen_new_thread(self):
        ...
        t = threading.Thread(target=thread_target, args=(loop,))
        t.start()

        # 在线程安全的方式下在线程中调度协程
        asyncio.run_coroutine_threadsafe(self.listen(), loop)

    def start_keep_alive_new_thread(self):
        def thread_target(loop):
            asyncio.set_event_loop(loop)
            # 在线程中启动事件循环
            loop.run_forever()
        ...
        # 在线程安全的方式下在线程中调度协程
        asyncio.run_coroutine_threadsafe(self.keep_alive(), loop)

    def start_refresh_token_new_thread(self):
        def thread_target(loop):
            asyncio.set_event_loop(loop)
            # 在线程中启动事件循环
            loop.run_forever()
        ...
        # 在线程安全的方式下在线程中调度协程
        asyncio.run_coroutine_threadsafe(self.refresh_token(), loop)
    ...
    async def keep_alive(self):
        while not self._is_over:
            try:
                _LOGGER.debug("start keep_alive...")
