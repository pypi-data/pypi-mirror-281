...
def check_typevar_model(
    ...
    if template.origin in (Union, types.UnionType):
        return any(check_typevar_model(instance, t_sub_arg) for t_sub_arg in template.args) # type: ignore
    if instance.origin in (Union, types.UnionType):
        return any(check_typevar_model(i_sub_arg, template) for i_sub_arg in instance.args) # type: ignore
