...
def create_openshift_enpoint(name, openshift_server, oc_path, oc_user, version="0.9.4.3", environment_variables=None,
                             port=8999, namespace="bdp-rts-dev", replicas=1, volume="tc-bdp-rts-dev-disk", use_oc=True):
    """
    ...
    if use_oc:
        ...
        with oc.api_server(openshift_server), oc.client_path(oc_path):
            ...
            while not all_runtimes_started:
                ...
                for pod_obj in oc.selector("pods", labels={"deployment": name}).objects():
                    ...
                    try:
                        # exec_result = oc.selector(f'pod/{pod_name}').object().execute(['tail', '-10', '/data/logs/pulse_responder.log'])
            ...
            oc.invoke("cp", ["cassandra.conf", f"{pod_name}:/config/cassandra.conf"])
    else:
        print(
        ...
        print(
            "Step 3: Create a Service for the Deployment that you have created. Copy the contents of the service.yml file which has been created in this folder. Open the Networking section of the left hand menu and select Services. Click the blue Create Service button in the top right corner of the Serivces section. Replace the text in the editor with the text you copied from service.yml and click the Create button below the editor.")
        print("")
        print(
            f"Step 4: Create a Route linked to the Deployment. Open the Networking section of the left hand menu and select Routes. Click the blue Create Route button in the top right corner of the deployment section. Set the Name to be {name}. Leave Hostname and Path empty. Select the Service you created and the Port for the service from the dropdowns. Leave the Secure Route tick box empty. Click the Create button.")
        print("")
        print(
            "Step 5: Test the endpoint that you have created by navigating to the Route url (which you can find by Navigating to Networking -> Routes in the left hand menu) and calling the refresh api. The response to the refresh API call should include success.")
        print("")
        print("Step 6: Set the runtime_path variable in the notebook to be the url of the Route that you created")

        runtime_path = "This variables needs to be set manually as oc is not being used in the setup"
