...
def load_languages_file (db):
    for entry in iter_data_file(db.table_filename):
...
def load_names_file (db):
    for (code, name, inv_name) in iter_data_file(db.names_filename):
        lg = db.bycode[code]
...
def load_macrolanguages_file (db):
    for (mcode, lcode, status) in iter_data_file(db.macrolanguages_filename):
...
def load_retirements_file (db):
    for entry in iter_data_file(db.retirements_filename):
...
def load_groups_file (db):
    with open(db.iso5_filename, 'r', encoding='utf8') as f:
        for line in f:
...
class LanguageTables (object):
    ...
    def __init__ (self):
        ...
        self.table_filename = data.path('iso/639-3/iso-639-3_20170202.tab')
        self.names_filename = data.path('iso/639-3/iso-639-3_Name_Index_20170217.tab')
        self.macrolanguages_filename = data.path('iso/639-3/iso-639-3-macrolanguages_20170131.tab')
        self.retirements_filename = data.path('iso/639-3/iso-639-3_Retirements_20170131.tab')
        self.iso5_filename = data.path('iso/639-5.txt')
