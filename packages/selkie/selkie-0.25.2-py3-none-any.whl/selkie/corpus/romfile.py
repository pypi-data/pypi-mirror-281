...
import os
from ..cld.seal.io import load_dict, pprint
from .rom import (Decoder, find_rom,
                  default_registry as global_registry,
                  Romanization as make_romanization)
from ..pyx.formats import BaseFile
from ..pyx.disk import Directory
...
class Registry (Directory):
    ...
    def find_rom (self, name):
        if name == 'default':
            return ReadOnlyRomanization(make_romanization())
        elif name in self:
            return self[name]
        else:
            rom = find_rom(name)
