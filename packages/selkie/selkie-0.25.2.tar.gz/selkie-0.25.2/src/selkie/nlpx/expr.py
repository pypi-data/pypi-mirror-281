...
class Expr (tuple):
    ...
    def __repr__ (self):
        ...
        for c in self:
            if isinstance(c, str):
                if contains_space(c):
                    cs.append(DefaultSyntax.scanable_string(c))
                else:
                    cs.append(c)
            else:
                cs.append(repr(c))
...
def load_exprs (filename):
    return list(_iter_exprs(filename))

def _iter_exprs (filename):
    toks = DefaultSyntax.iter_tokens(filename)
    while toks:
        yield scan_expr(toks)
