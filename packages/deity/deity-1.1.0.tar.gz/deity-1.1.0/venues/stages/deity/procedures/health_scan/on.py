...
def turn_on_health_check (
	...
	env ["deity___status_path"] = the_path
		
	env ["deity___harbor_host"] = "0.0.0.0"
	env ["deity___harbor_port"] = str (aggregator_procedure ["port"])	
	
	
	if (type (relative_path) == str and len (relative_path) >= 1):
		name = os.path.relpath (the_path, relative_path)
		env ["deity___status_relative_path"] = relative_path
	else:
		name = "scan " + the_path
		env ["deity___status_relative_path"] = ''
