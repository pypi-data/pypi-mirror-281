...
def main():
    ...
    parser_genome.add_argument(
        ...
        help=(
            ...
            'Be aware that the masking methods differ depending on the database. \n'
            'Thus, if masking affects your analysis, choose the database accordingly. \n'
    ...
    parser_geneset.add_argument(
        ...
        help=(
            ...
            'agustus       : GenArk Augustus gene prediction - run in purely de novo mode (GFF)\n'
    ...
    parser_seqmeta.add_argument(
        '--info', 
        action='store_true', 
        help='Show full information about parameters \n '
        )
    ...
    parser_seqmeta.add_argument(
        ...
        help=(
            ...
            join_variables_with_newlines(list(DIC_PLATFORM.keys()))
            )
        )
    parser_seqmeta.add_argument(
        '-sl', '--selection',
        ...
        help=(
            ...
            join_variables_with_newlines(list(DIC_SELECTION.keys()))
            )
        )
    parser_seqmeta.add_argument(
        '-fi', '--filter',
        ...
        default='', 
        help=(
            'Specific criteria to refine search results' +
            join_variables_with_newlines(list(DIC_FILTER.keys()))
            )
        )
    parser_seqmeta.add_argument(
        '-ly', '--layout',
        ...
        default='', 
        help=(
            'Library layout of the sequencing data: paired or single (default: paired,single) \n'
            )
        )
    parser_seqmeta.add_argument(
        '-ac', '--access',
        ...
        default='public', 
        help=(
            'Data accessibility: public or controlled (default: public)'
            )
        )
    parser_seqmeta.add_argument(
        '-bp', '--bioproject',
    ...
    parser_seqmeta.add_argument(
        ...
        default='', 
        help=(
            'Additional attributes of the sequencing data'
            )
        )
    parser_seqmeta.add_argument(
        '-rl', '--readlength',
    ...
    # Gencube seqmeta
        ...
        if not args.keywords and not args.info and not args.organism and not args.strategy and not args.source and not args.platform and not args.layout and not args.selection and not args.bioproject and not args.biosample and not args.accession and not args.title and not args.aligned and not args.author and not args.filter and not args.mbases and not args.publication and not args.modification and not args.properties and not args.readlength and not args.textword and not args.exclude and not args.metadata:
