...
def createFileSysem(duration 
    ...
    error = f"./{masterDirectory}/condor/error"
    output = f"./{masterDirectory}/condor/output"
    log = f"./{masterDirectory}/condor/log"
    submit = f"./{masterDirectory}/condor/submit"
    ...
    for i in range(len(segments)):
        ...
        output_file_name = list(f"{segmentFileName}_{det}.pkl" for det in detectors)

        remaps = list(f"{segmentFileName}_{det}.pkl = {masterDirectory}/{det}/{segmentFileName}.pkl" for det in detectors)
        ...
        job = Job(name=segmentFileName
                  ,executable=masterDirectory+'script_'+segmentFileName+'.py' 
                   ...
                   ,getenv=True
                   ,dag=dagman
                   ,requirements='Has_sse4_1 == True'
                   ,extra_lines=["accounting_group_user="+accounting_group_user
                                    ,"accounting_group="+accounting_group
                                    ,f"transfer_output_files   = {','.join(output_file_name)}"
                                    ,f"transfer_output_remaps = \"{';'.join(remaps)}\""
