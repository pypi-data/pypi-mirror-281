...
from backendservice.web.services.tasks.grpcs.dataservice_pb2 import TaskRequest_, \
     PageObjResult_, PageObj_, PageMission_, PageMissionAlg_
from backendservice.web.services.tasks.grpcs.grpc_models import TaskRequest, PageObj, PageMissionAlg, PageMission
...
class pb2_to_local:
    ...
    def to_TaskRequest(prop: TaskRequest_) -> TaskRequest:
        out = TaskRequest(prop.objId, prop.code, prop.type)
        return out
...
class local_to_pb2:
    ...
    def to_TaskRequest_(prop: TaskRequest) -> TaskRequest_:
        out = TaskRequest_(objId=value_or_empty(prop.id_), code=value_or_empty(prop.code), type=value_or_empty(prop.type))
        return out
