...
from backendservice.grpc.tasks.grpc_models import TaskRequest, PageObj, DetectAudit
...
class DataServiceApi:
    ...
    def GetTask(self, id_: str, code: str, type_: str = "") -> PageObj:
        ...
        if resp.status != ErrorCode.ok:
            raise ErrorDetailException(resp.status, resp.message, resp.details)
        return pb2_to_local.to_PageObj(resp.data)

    @wrapper_rpc_error
    def DetectResult(self,  da: DetectAudit) :
        req = local_to_pb2.to_DetectAudit_(da)
        resp = self._stub.DetectResult(req)
