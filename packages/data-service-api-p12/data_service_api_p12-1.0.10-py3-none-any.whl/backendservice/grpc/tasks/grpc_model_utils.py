...
class local_to_pb2:
    ...
    @staticmethod
    def to_PageObjResult_(status: int, message: str,
                          p: PageObj = None) -> PageObjResult_:

        if p is not None:
            data = local_to_pb2.to_PageObj_(p)
            req = PageObjResult_(status=status, message=value_or_empty(message), data=data)
        else:
            req = PageObjResult_(status=status, message=value_or_empty(message))
        return req
