...
class PolibaseDBApi:
    ...
    def create_dump(file_name: str | None = None, test: nbool = None) -> None:
        ...
        # step 1
        A.E.backup_polibase_creation_db_dump_start(not test)
        if not test:
            os.system(
                j(
                    (
        ...
        A.E.backup_polibase_creation_db_dump_complete(
            os.path.getsize(
        ...
        # step 2
        A.E.backup_polibase_creation_db_dump_archive_start()
        ...
        # step 3
        A.E.backup_polibase_db_dump_archive_moving_start(A.CT_H.NAS.ALIAS)
        A.A_B.start_robocopy_job_by_name(
            "move_polibase_database_dump_archive", force=True, block=True
        )
        A.E.backup_polibase_db_dump_archive_moving_complete(A.CT_H.NAS.ALIAS)
        # step 4
        A.E.backup_polibase_db_dump_moving_start(A.CT_H.POLIBASE2.ALIAS)
        A.A_B.start_robocopy_job_by_name(
            "move_polibase_database_dump", force=True, block=True
        )
        A.E.backup_polibase_db_dump_moving_complete(A.CT_H.POLIBASE2.ALIAS)
