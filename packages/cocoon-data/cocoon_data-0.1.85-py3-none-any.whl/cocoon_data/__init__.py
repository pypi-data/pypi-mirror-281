...
```"""
        ...
        words_in_desc = summary['relation_desc'].split()
        if len(words_in_desc) > 20:
            raise ValueError(f"relation_desc should be less than 20 words, but has {len(words_in_desc)} words")

        lower_desc = summary['relation_desc'].lower()
        for entity in entities:
            if entity.lower() not in lower_desc:
                raise ValueError(f"Entity '{entity}' is not mentioned in the relation_desc")
