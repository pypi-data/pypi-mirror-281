...
def get_predictions(model_path, df, emotion_mapping_path=None, model_type='roberta'):
    ...
    - emotion_mapping_path (str, optional): Path to JSON file mapping numeric emotions to string labels.
    - model_type (str, optional): Type of model to use ('roberta' or 'rnn'). Default is 'roberta'.
    ...
    if model_type == 'roberta':
        ...
        def tokenize(text):
            return tokenizer.encode(text, max_length=512, truncation=True, padding='max_length')
    ...
    # Load and map the numeric emotions back to string labels if emotion_mapping_path is provided
