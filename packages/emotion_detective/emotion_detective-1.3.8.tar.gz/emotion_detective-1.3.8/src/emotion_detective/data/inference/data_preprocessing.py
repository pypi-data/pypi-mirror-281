...
def transcribe_translate(path: str, language: str = "en") -> pd.DataFrame:
    ...
    try:
        ...
        text_dict = {"text": [], "start_time": [], "end_time": []}

        logger.info("Loading Whisper model...")
        model = whisper.load_model("base")

        logger.info("Transcribing audio file...")
        result = model.transcribe(audio=path, language=language)

        logger.info("Extracting transcription chunks...")
        for segment in result['segments']:
            text_dict["text"].append(segment['text'])
...
def dataset_loader(df: pd.DataFrame,
    ...
    try:
        ...
        logger.debug("Max length of tokenized texts: %s", max_length)
        ...
        logger.debug(f"DataLoader size: {len(data_loader)}")
