...
def show_instructions():
    instructions = (
        ...
        f"{Fore.CYAN}This tool processes 🎥 video and 🎵 audio files to detect emotions.{Style.RESET_ALL}\n\n"
        f"{Fore.GREEN}🚀 Instructions: {Style.RESET_ALL}\n"
        f"1. {Fore.BLUE}input_media_path: {Style.RESET_ALL} Path to the input audio (mp3) or video file (mp4). {Fore.RED}[required]{Style.RESET_ALL}\n"
        f"2. {Fore.BLUE}model_path: {Style.RESET_ALL} Path to the saved NLP model. {Fore.RED}[required]{Style.RESET_ALL}\n"
        f"3. {Fore.BLUE}model_type: {Style.RESET_ALL} Type of NLP model ('roberta' or 'rnn'). {Fore.RED}[required]{Style.RESET_ALL}\n"
        f"4. {Fore.BLUE}emotion_mapping_path: {Style.RESET_ALL} Path to the emotion mapping file. {Fore.RED}[required]{Style.RESET_ALL}\n"
...
def main(
    ...
    if input_media_path is None:
        input_media_path = input("Please enter the path to the input audio (mp3) or video file (mp4): ")
    ...
    if emotion_mapping_path is None:
        emotion_mapping_path = input("Please enter the path to the emotion mapping file: ")
    ...
    predictions_df = get_predictions(model_path, transcribed_df, emotion_mapping_path, model_type=model_type)
