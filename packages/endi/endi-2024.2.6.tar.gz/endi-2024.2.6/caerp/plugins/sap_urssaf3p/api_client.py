...
class URSSAF3PClient:
    ...
    def request(self, url, payload: StructuredData) -> StructuredData:
        ...
        try:
            ...
            logger.info("< Received HTTP {response.status_code} (to POST {url}).")
            logger.debug(f"Received HTTP {response.status_code} {response.content}")
    ...
    def transmettre_demande_paiement(self, payload: dict) -> str:
        ...
        assert len(response_list) == 1, "Unexpected format"
