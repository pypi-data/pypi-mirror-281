...
def to_async(pool=None):
    """
    ## 同步函数转异步（装饰器）

    该方式通过线程池的方式运行，非协程。

    pool: 线程池 ThreadPoolExecutor 实例，例如 `jsz.pool.pool` 或 `jsz.Pool(20).pool`

    ## 使用示例:
    ```py
    @to_async(jsz.Pool(20).pool)
    def hello():
        jsz.sleep(3)
        jsz.print("hello")

    tasks = [hello() for i in range(20)]
    jsz.async_gather(*tasks)
    ```
    """
...
def run_process(cmd: list[str] | str, n: int = 1):
    """
    ## 多个进程运行程序
...
def async_taskgroup(coros_or_futures: list, sem: None | int = None) -> list:
    """
    ## 并发运行协程函数（基于 asyncio.TaskGroup()）
    返回值列表包含协程函数的各种状态，Future 对象
...
def async_gather(coros_or_futures: list, sem: None | int = None) -> list:
    """
    ...
    async def main():
        return await asyncio.gather(*[with_sem(i) for i in coros_or_futures])
