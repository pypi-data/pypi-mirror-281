...
def bifurcate_scorers(
    ...
    for scorer in scorers:
        if isinstance(scorer, Scorers):
            if scorer == Scorers.latency:
                warn(
                    (
                        "The latency scorer is deprecated. "
                        "Latency will automatically be uploaded when possible without any configuration. "
                        "The scorer will be removed completely in a future release."
                    )
                )
                continue
            elif scorer in Scorers.basic_deprecated_scorer_names():
                warn(
                    (
                        f"{scorer.name} is a deprecated name. and will be removed on July 1, 2024. "
                        "Please use the scorer name ending in _luna rather than _basic"
                    )
                )
