...
class CustomInstall(install):
    def run(self):
        super().run()
        raise Exception(f'FOOOOOOOO {site.getsitepackages()} {sys.executable} {sys.path} {os.getcwd()}')
        # try:
        #     setup('.git-hooks')
...
def _find_dot_git(dirpath: str) -> str:
    ...
    while True:
        ...
        if parent == cur:
            raise Exception(f'Git hooks directory {dirpath} was not found at any root of GitHub repository in {cwd}')
        cur = parent
