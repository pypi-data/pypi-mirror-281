...
class LoaderBase(ABC):
    ...
    def register_patterns(self, manager, prioritize=False):
        ...
        """
        if prioritize:
            for pattern, pattern_type in reversed(list(self.patterns)):
                manager.register_loader(
                    pattern, self, prioritize=prioritize, pattern_type=pattern_type
                )
        else:
            for pattern, pattern_type in self.patterns:
                manager.register_loader(pattern, self, pattern_type=pattern_type)
