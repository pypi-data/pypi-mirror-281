...
def criar_tabela_contingencia_expandida_llm_OLD_V2(data_df, groups, categories, model):
...
def criar_tabela_contingencia_expandida_llm(data_df, grupos, categorias, modelo):
    ...
    def renomear_indices_e_colunas(df, categorias, grupos):
        novo_indice = []
        for idx in df.index:
            novo_idx = []
            for level, name in zip(idx, df.index.names):
                if name in categorias:
                    novo_idx.append(categorias[name][level])
                else:
                    novo_idx.append(level)
            novo_indice.append(tuple(novo_idx))
        df.index = pd.MultiIndex.from_tuples(novo_indice, names=df.index.names)

        if isinstance(df.columns, pd.MultiIndex):
            novas_colunas = []
            for col in df.columns:
                nova_col = []
                for level, name in zip(col, df.columns.names):
                    if name in categorias:
                        nova_col.append(categorias[name][level])
                    else:
                        nova_col.append(level)
                novas_colunas.append(tuple(nova_col))
            df.columns = pd.MultiIndex.from_tuples(novas_colunas, names=df.columns.names)
        else:
            df.columns = [categorias[grupos[-1]][col] if col in [0, 1] else col for col in df.columns]
    ...
    tabela_final = renomear_indices_e_colunas(tabela_final, categorias, grupos)

    tabela_final.columns = pd.MultiIndex.from_tuples([('observado' if col[0] == 'Freq' else col[0], col[1]) for col in tabela_final.columns])
