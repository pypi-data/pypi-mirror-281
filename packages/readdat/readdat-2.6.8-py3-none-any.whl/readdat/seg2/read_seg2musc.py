...
def _extract_source_location_from_seg2musc_header(trace: obspy.core.trace.Trace):
    try:
        # only one coordinate for the source
...
def _extract_receiver_location_from_seg2musc_header(trace):
    try:
...
def seg2musc_to_seg2obspy(stream: obspy.core.stream.Stream, verbose: bool=True) -> obspy.core.stream.Stream:
    ...
    for trace in stream:
        ...
        _extract_receiver_location_from_seg2musc_header(trace=trace)

        _extract_source_location_from_seg2musc_header(trace=trace)
