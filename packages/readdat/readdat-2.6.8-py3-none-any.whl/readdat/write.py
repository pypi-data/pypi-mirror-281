...
def write(
    ...
    if format.lower() in ['seg2', 'sg2']:
        ...
        if os.path.isfile(filename):
            """SECURITY TO AVOID OVERWRITING EXPERIMENTAL DATA"""
            new_filename = filename.split("." + ext)[0] + "_obs." + ext
            warnings.warn(f"{filename} exists already, => {new_filename}")
