...
from peegy.io.edf_bdf_writer import split_bdf_by_event_code
...
import numpy as np
from peegy.processing.events.event_tools import get_event_times
import os
...
split_bdf_by_event_code(input_file_name=_first_file,
                        event_code=16.0,
                        trash_original=True)

data_files_meta = get_files_and_meta_data(data_folder)
_first_file = data_files_meta.iloc[0].data_links.data_file
data4 = eeg_reader(file_name=_first_file)
data_ds, events_ds, _, _ = data4.get_data()
et_ds = get_event_times(event_channel=events_ds, fs=data4.fs)
et_ds.summary()
