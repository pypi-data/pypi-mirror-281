...
class MaitaiAsyncOpenAIClient:
    def __init__(
        ...
        *,
        ...
        if api_key:
            raise ValueError("The api_key parameter is not supported by Maitai. Provide openai_api_key or groq_api_key instead")
        if maitai_api_key:
            config.initialize(maitai_api_key)
        if openai_api_key:
            config.auth_info.openai_api_key = openai_api_key
        if groq_api_key:
            config.auth_info.groq_api_key = groq_api_key
