...
def get_legal_holidays(start_date: datetime.date, end_date: datetime.date) -> List[str]:
    """ 获取指定日期范围内的法定节假日 """
    res_dates = []
    for year in range(start_date.year, end_date.year + 1):
        # 获取中国法定节假日
        china_holidays = holidays.country_holidays('CN', subdiv=None, years=year)
        # 遍历每个日期,剔除掉周末，并且在范围内
        for each_date in china_holidays.keys():
            if each_date.weekday() >= 5 or each_date < start_date or each_date > end_date:
                continue
            res_dates.append(f"{each_date}".replace("-", ""))
        # 此处需要对2019年做一下修正:
        if year == 2019 and start_date <= datetime.date(2019, 5, 2) <= end_date:
            res_dates.extend(["20190502", "20190503"])
    # 按照先后顺序进行排序
    return sorted(res_dates)


def get_all_trading_days(start_date: datetime.date, end_date: datetime.date) -> List[datetime.date]:
    """ 获取所有交易日 """
    # 获取法定节假日，假设返回的也是 YYYYMMDD 格式的字符串集合
    holidays = get_legal_holidays(start_date, end_date)
    all_days = []
    current_date = start_date
    while current_date <= end_date:
        # 排除周末和法定节假日
