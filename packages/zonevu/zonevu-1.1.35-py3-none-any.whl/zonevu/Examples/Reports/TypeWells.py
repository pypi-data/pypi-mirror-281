...
def main(zonevu: Zonevu, output_path: str, exact_match: bool = True, name: Optional[str] = None, max_count: Optional[int] = None):
            ...
            try:
                for d in interp.curve_defs:
                    if d.wellbore_role == CurveDefWellboreRoleEnum.TypeWellbore:
                        type_well_entry = TypeWellEntry(d.well_id, d.well_name, well.id, well.full_name, interp.id,
                                                        interp.name)
    ...
    with open(output_path, 'w', newline='') as file:
        ...
        writer.writerow(['TypeWell', 'TargetWell', 'Interpretation'])
        for type_well_entries in type_well_dict.values():
            for entry in type_well_entries:
                writer.writerow([entry.well_name, entry.target_well_name, entry.interpretation_name])
