...
class Monitoring(object):
    ...
    def __init__(self, args):
        ...
        self.args = args
        self.default_output_file = "/tmp/output.json"
        self.report_full = {}
        self.kv = None
        self.success = False
        self.payloads = []
    ...
    def key_vault_report(self):
        ...
        if not self.args.vaults:
            logging.error("No vaults provided.")
            return

        vaults = self.args.vaults
    ...
    def write_report_full_json(self):
        ...
        # Ensure the path of the output file exists
        if not os.path.isdir(os.path.dirname(workflow_output_file)):
            os.makedirs(os.path.dirname(workflow_output_file))
    ...
    def sanitize_filename(self):
        """Sanitizes the output file of the json data"""

        # Ensure a valid filename is set. If that is not the case, then '/tmp/output.json' is used as default.
        if self.report_full:
            if not self.args.workflow_output_file == self.default_output_file:
                ...
                # Ensure if custom output file, that it can only be writen to local dir
                if not custom_workflow_output_file.startswith("./"):
                    return f"./{custom_workflow_output_file}"
...
def main():
    ...
    parser.add_argument("-w", "--workflow_output_file", type=str, default="/tmp/output.json",
