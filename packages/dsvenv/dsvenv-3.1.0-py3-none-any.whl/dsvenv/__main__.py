...
def list_available_to_download_python_versions() -> Dict[str, str]:
    """
    Return all Python versions (+ download link), available for the current platform, to
    create a new virtual environment.

    Note_1: Currently, only custom versions are supported on the Windows platform.
    Note_2: This function relies on a cache list of available Python versions. This list
            can be updatd by running `update_indygreg_python_versions_list.py` if needed.

    Returns:
        A dict with keys the Python versions in descending order, and values the direct
        download links (which point to archives in .zip, .tar.bz2 or .tar.zst format).
    ...
    if sys.platform != 'win32':
        raise EnvironmentError(
            ...
            f'({sys.platform}) at this moment. And this was by design!! You can only '
            f'use system Python {_THIS_PYTHON_VERSION}.'
        )

    # For the standalone Python builds, we use the excellent Indygreg Python builds
    # (https://github.com/indygreg/python-build-standalone/).
    # While it is perfectly possible to directly parse the content of
    # https://api.github.com/repos/indygreg/python-build-standalone/releases,
    # the implementation below relies on a parsing already done by the
    # `rye` project (https://github.com/astral-sh/rye).
    #
    # Note_1: Yes, it is probably more robust to directly rely on the Indygreg
    #         sources, but this is quicker.
    #
    # Note_2: You might see that Indygreg also provides standalone Python
    #         binaries for other platforms, such as Linux. For now, because of how
    #         our solutions are currently deployed in an Ubuntu-based VM, with the
    #         packages being installed in the system Python, there is no need for
    #         allowing custom Python versions on Linux as well. Even more, it was
    #         decided to NOT offer this functionality.
    #
    # Note_3: Instead of always pulling in an updated list from `rye` (which would
    #         break if the upstream format changes), we rely on a static list that
    #         is only updated when appropriate.
    #         Also see `update_indygreg_python_versions_list.py`.

    # The following relies on particular format and formatTING of the data
    # inside the `rye` project!!
    # Currently, the relevant lines look like the following:
    #   (PythonVersion { name: Cow::Borrowed("pypy"), arch: Cow::Borrowed("aarch64"), os: Cow::Borrowed("linux"), major: 3, minor: 10, patch: 14, suffix: None }, "https://downloads.python.org/pypy/pypy3.10-v7.3.16-aarch64.tar.bz2", Some("fc720999bc5050e1d3706b3b6445e695cf42bfc71ebc7c88ed6bb88828b1d385"))

    versions_file_raw = (THIS_DIR / 'python_downloads.inc').read_text().split('\n')

    # Get the raw data and immediately restrict to 64-bit cpython on Windows.
    versions_raw = [
        r
        for r in versions_file_raw
        if all(s in r for s in ['PythonVersion', 'cpython', 'windows', 'x86_64'])
    ]
    # Extract from this raw output exactly what we need.
    available_versions = {
        f'{ma}.{mi}.{p}': url
        for ma, mi, p, url in [
            re.match(
                '^.*major: ([^,]+).*minor: ([^,]+).*patch: ([^,]+).*"(https[^"]+)".*$',
                vr,
            ).groups()
    ...
    available_versions_sorted_keys = sorted(
        (Version(v) for v in available_versions.keys()), reverse=True
    )
    available_versions_sorted = {
        str(k): available_versions[str(k)] for k in available_versions_sorted_keys
    }

    return available_versions_sorted
...
def resolve_python_version_spec(python_version_spec: str) -> str:
    ...
    if compatible_version is not None:
        logging.info(
            f'A base environment with Python `{compatible_version}` was found, which '
            f'is compatible with the requested version specification '
            f'`{python_version_spec}`. It will be used to create a virtual '
            f'environment.'
        )
        return compatible_version

    available_versions = list_available_to_download_python_versions()
    compatible_version = get_compatible_version(available_versions, python_version_spec)
    if compatible_version is not None:
        logging.info(
            f'Python `{compatible_version}` was found online, which is compatible '
            f'with the version specification `{python_version_spec}`. It will be '
            f'installed locally and used as a base environment to create a virtual '
            f'environment.'
        )
        return compatible_version
    ...
    raise RuntimeError(
        ...
        f'neither locally (found: {list_base_venv_python_versions()}, see '
        f'{str(_BASE_ENVS_DIR)}) nor online (found: '
        f'{list(available_versions.keys())}. For the unparsed list, see '
        f'https://raw.githubusercontent.com/astral-sh/rye/main/rye/src/sources/generated/python_downloads.inc).'
    )
...
def create_base_venv(target_dir: Path, python_version: str):
    ...
    try:
        ...
        raise RuntimeError(
            f'Error retrieving a list of available Python versions. Exact error: {e}.'
        )

    if python_version not in available_versions.keys():
        raise RuntimeError(
            f'Unfortunately Python {python_version} is not available for installation.'
            'For the full list of versions run: '
            '`dsvenv --print-available-python-versions`.'
        )

    target_dir.mkdir(parents=True, exist_ok=True)

    # Download the required Python and put it in place as a base Python.
    archive_dir = target_dir / 'python_archive'
    archive_dir.mkdir(exist_ok=True)

    download_url = available_versions[python_version]
    archive = archive_dir / Path(urlparse(download_url).path).name
    ...
    # The received file can be a .zip, tar.bz2 or .tar.zst file.
    extract_dir = archive_dir / 'extracted'
    # Note: Only taking at most 2 of the suffixes, as it is possible that the filename
    #       contains dots, which would make part of the filename end up in `suffixes`.
    archive_type = ''.join(archive.suffixes[-2:]).lower()
    ...
    if archive_type == '.tar.zst':
        ...
        # We need the content of the 'python/install' folder.
        for to_move in (extract_dir / 'python' / 'install').iterdir():
            to_move.rename(target_dir / to_move.name)

    else:
        raise NotImplementedError(
            'Only .tar.zst archives are currently supported. Received archive: '
            f'{archive} (with file extension `{archive_type}`).'
        )
...
def parse_args() -> Namespace:
    ...
    parser.add_argument(
        ...
        action='store_true',
        help=(
            'Print all Python versions available on this platform for the creation of '
...
def main():
    ...
    if args.print_available_python_versions:
        try:
            available_python_versions = list(
                list_available_to_download_python_versions().keys()
            )
        except EnvironmentError:
            available_python_versions = [_THIS_PYTHON_VERSION]

        logging.info(f'Available Python versions: {available_python_versions}')
