...
def load_config() -> Config:
    ...
    auth = None
    if not os.getenv(f"{AUTH_PREFIX}DISABLED"):
        auth = AuthConfig(
            client_id=os.getenv(f"{AUTH_PREFIX}CLIENT_ID"),
            client_secret=os.getenv(f"{AUTH_PREFIX}CLIENT_SECRET"),
            token_endpoint=os.getenv(
                f"{AUTH_PREFIX}TOKEN_ENDPOINT", "https://login.stage.ciam.refinitiv.com/as/token.oauth2"
            ),
            scopes=os.getenv(f"{AUTH_PREFIX}SCOPES", "trapi").split(","),
        )

    return Config(
        base_url=os.getenv(f"{PREFIX}BASE_URL", "https://api.dev.azure-2-dev-test-aiid250621.public.inf0.net"),
