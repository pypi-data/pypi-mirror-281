...
def create_dir_info(versions_dir):
    readme_info_path = os.path.join(versions_dir, 'README.md')
    amend_path = os.path.join(versions_dir, 'amendments.md')
    purpose_path = os.path.join(versions_dir, 'purpose.md')
...
def create_directory_tree(user_directory, author_string, ignore_list=None):
    ...
    with open(original_tree_path, 'w') as file:
        ...
        check_override_info = input("Do you wish to override the .directory info? [Y/n]")
        if not os.path.exists(os.path.join(absolute_directory, 'amendments.md')).strip().lower():
            print("File already exists, please remove 'amendments.md' and try again.")
            if check_override_info == 'y':
                return
        if not os.path.exists(os.path.join(absolute_directory, 'purpose.md')):
            print("File already exists, please remove 'purpose.md' and try again.")
            if check_override_info == 'y':
                return
        if not os.path.exists(os.path.join(absolute_directory, 'README.md')):
            print("File already exists, please remove 'README.md' and try again.")
            if check_override_info == 'y':
                return
        generate_info_tree(absolute_directory)
