...
class CommaSeperatedValueFile(IOPlugin):
    ...
    def expand_url(self, url):
        ...
        csvurl = urllib.parse.urlunparse(
            urllib.parse.ParseResult(scheme='vfs', netloc=parsed.netloc, path=parsed.path, params='', query='',
                                     fragment=''))
        baseurl = urllib.parse.urlunparse(
            urllib.parse.ParseResult(scheme='vfs', netloc=parsed.netloc, path=os.path.dirname(parsed.path), params='',
                                     query='', fragment=''))
        ...
        header_row = {'0': False, '1': True, 'false': False, 'true': True, 'none': None}[
            query.get('header', ('none',))[0]]
        ...
        relpath = {'0': False, '1': True, 'false': False, 'true': True, 'none': False}[
            query.get('relative_path', ('none',))[0]]
        if baseurl[-1] != '/':
            baseurl = baseurl + '/'
