...
from ..models import PathSample, URLSample
...
class XNATStorage(IOPlugin):
    ...
    def fetch_url(self, inurlsample: URLSample, outpath: pathlike):
        ...
        """
        url, path_prefix, insecure, verify, use_regex, query = self.parse_uri(inurlsample.data_url)
    ...
    def upload(self, resource, in_path, location, retries=3, overwrite=False):
        # Initial upload
        success = False
        tries = retries
    ...
    def expand_url(self, urlsample: URLSample):
        ...
        if url.path == '/search':
            return self.expand_search(urlsample)
        if re.match('.*/projects/[^/]+$', url.path):
            return self.expand_project(urlsample)
        else:
            url, path_prefix, insecure, verify, use_regex, query = self.parse_uri(urlsample.data_url)
    ...
    def expand_project(self, urlsample: URLSample):
        ...
        return self.expand_search(new_sample)

    def expand_search(self, urlsample: URLSample):
        # Parse the query
        url = urlsample.data_url
        query = url.query
