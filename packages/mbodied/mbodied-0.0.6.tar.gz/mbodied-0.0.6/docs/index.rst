.. Mbodied Agents documentation master file, created by
   sphinx-quickstart on Fri Jun  7 12:09:33 2024.
   You can adapt this file completely to your liking, but it should at least
   contain the root `toctree` directive.

mbodied agents
=============================================

**mbodied agents** simplifies the integration of advanced AI models in robotics. It offers a consistent interface for controlling various robots using state-of-the-art transformers and multimodal data processing.

.. toctree::
   :maxdepth: 2

   overview/index
   installation/index
   getting_started/index
   glossary/index
   concepts/index
   building_blocks/index
   contributing/index
   system_components/index

.. raw:: html

   <div class="quickstart-guide">
     <div class="quickstart-text">
       <h2>Quickstart Guide</h2>
       <p>Start your path to understanding the mbodied agents package with our quickstart guide.</p>
       <a class="btn" href="getting_started/index.html">Get started</a>
     </div>
   </div>




.. Indices and tables
.. ==================

.. * :ref:`genindex`
.. * :ref:`modindex`
.. * :ref:`search`
