...
@task(positional=["pkg_name"])
def use_source(ctx, pkg_name):
    """
    Checkout a package to /workspaces and install it

    """
    repos = {
        "roxbot": "git@github.com:rox-automation/roxbot.git",
    }

    if pkg_name not in repos:
        print(f"Unknown package: {pkg_name}")
        print(f"Known packages: {list(repos.keys())}")
        return

    ctx.run(f"git clone {repos[pkg_name]} /workspaces/{pkg_name}")
    ctx.run(f"pip uninstall -y {pkg_name}")
    ctx.run(f"pip install -e /workspaces/{pkg_name}")
    ctx.run(f"pip show {pkg_name}")
