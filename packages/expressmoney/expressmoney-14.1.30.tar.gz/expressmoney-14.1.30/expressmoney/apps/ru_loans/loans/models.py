...
class Loan(models.Model):
    ...
    @property
    def overdue_period(self):
        """На сколько дней просрочена оплата по займу"""
        if self.status in (self.OPEN, self.OVERDUE, self.STOP_INTEREST):
            end_date = self.extended_end_date if self.extended_end_date else self.expiry_date
            overdue_period = (timezone.now().date() - end_date).days
            overdue_period = overdue_period if overdue_period >= 0 else 0
        else:
            overdue_period = 0
