...
# pylint: disable=too-many-lines, protected-access, too-many-instance-attributes
...
class AllStyleModifier:
    ...
    @error_handler
    def merge(self, theme_styles: ThemeStyles, targets: Optional[List[str]] = None) -> None:
...
class Theme:
    ...
    def apply_custom_theme(
        ...
        theme_colors: List[Tuple[str, Union[Tuple[int, int, int], Tuple[int, int, int, float]]]] = None,
...
def _check_color(color, error_message: str) -> None:
    """Validate the color format.

    Args:
        color (tuple): The color to validate.
        error_message (str): The error message to raise if the color is invalid.

    Raises:
        ValueError: If the color is not a tuple, if the length of the tuple is not 3 or 4,
                    if the RGB values are not integers between 0 and 255, or if the alpha
                    value is not a float or integer between 0 and 1.
    """
