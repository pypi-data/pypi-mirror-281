    ...
    for name, color in colors:
        for style_type in ["", "_flat", "_solid", "_dashed"]:
            for width_type, template in [("", default_template), ("_light", light_template), ("_bold", bold_template)]:
                ...
                if style_type == "":
                    named_styles.append(
                        (f"{name}{width_type}", _get_default_style(template, color, default_shapeline_color))
                    )
...
def _get_default_style(template: OfficialThemeTemplate, color, shape_line_color=None) -> ThemeStyles:
    t = template.copy()
    t.icon_color = color
