...
@cli.command(help="Optimize application workflow")
@click.argument("scenario_dir", required=True, type=click.Path(exists=True))
@click.argument("artifacts_dir", required=True, type=click.Path(exists=True))
@click.option(
    "--duration", default=0, type=int, help="Duration of each experiment in seconds."
...
def optimize(scenario_dir, artifacts_dir, duration, repeat):
    arg_scenario_dir = Path(scenario_dir).resolve()
    arg_artifacts_dir = Path(artifacts_dir).resolve()
    if utils.is_valid_setup(arg_scenario_dir, arg_artifacts_dir, COMMAND_DEPLOY):
        logger.warn(
            "[bold red blink]Deprecated[/] 'Optimization' command, contact support"
        )
        name = "UserDefinedOptimization"
        try:
            module = __import__(f"e2clab.optimizer.{name}", fromlist=[name])
            klass = getattr(module, name)
            optimization_klass = klass(
                arg_scenario_dir, arg_artifacts_dir, duration, repeat
            )
            optimization_klass.run()
        except ImportError as e:
            logger.exception(e)
            logger.error(
                r"Imports not found, use 'pip install e2clab\[opt]' "
                "to use optimization capabilities"
            )
            sys.exit(1)
    else:
