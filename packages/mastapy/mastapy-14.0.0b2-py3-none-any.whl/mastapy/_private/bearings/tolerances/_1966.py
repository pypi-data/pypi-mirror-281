"""RaceDetail"""
from __future__ import annotations

from typing import TYPE_CHECKING, ClassVar

from mastapy._private._internal.dataclasses import extended_dataclass
from mastapy._private.bearings.tolerances import _1960
from mastapy._private._internal.cast_exception import CastException
from mastapy._private._internal.python_net import python_net_import
from mastapy._private._internal import utility

_RACE_DETAIL = python_net_import("SMT.MastaAPI.Bearings.Tolerances", "RaceDetail")

if TYPE_CHECKING:
    from typing import Any, Type, TypeVar

    from mastapy._private.bearings.tolerances import _1953

    Self = TypeVar("Self", bound="RaceDetail")
    CastSelf = TypeVar("CastSelf", bound="RaceDetail._Cast_RaceDetail")


__docformat__ = "restructuredtext en"
__all__ = ("RaceDetail",)


@extended_dataclass(frozen=True, slots=True, weakref_slot=True)
class _Cast_RaceDetail:
    """Special nested class for casting RaceDetail to subclasses."""

    __parent__: "RaceDetail"

    @property
    def interference_detail(self: "CastSelf") -> "_1960.InterferenceDetail":
        return self.__parent__._cast(_1960.InterferenceDetail)

    @property
    def bearing_connection_component(
        self: "CastSelf",
    ) -> "_1953.BearingConnectionComponent":
        from mastapy._private.bearings.tolerances import _1953

        return self.__parent__._cast(_1953.BearingConnectionComponent)

    @property
    def race_detail(self: "CastSelf") -> "RaceDetail":
        return self.__parent__

    def __getattr__(self: "CastSelf", name: str) -> "Any":
        try:
            return self.__getattribute__(name)
        except AttributeError:
            class_name = utility.camel(name)
            raise CastException(
                f'Detected an invalid cast. Cannot cast to type "{class_name}"'
            ) from None


@extended_dataclass(frozen=True, slots=True, weakref_slot=True)
class RaceDetail(_1960.InterferenceDetail):
    """RaceDetail

    This is a mastapy class.
    """

    TYPE: ClassVar["Type"] = _RACE_DETAIL

    wrapped: "Any"

    def __post_init__(self: "Self") -> None:
        """Override of the post initialisation magic method."""
        if not hasattr(self.wrapped, "reference_count"):
            self.wrapped.reference_count = 0

        self.wrapped.reference_count += 1

    @property
    def cast_to(self: "Self") -> "_Cast_RaceDetail":
        """Cast to another type.

        Returns:
            _Cast_RaceDetail
        """
        return _Cast_RaceDetail(self)
