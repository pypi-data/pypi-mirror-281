...
class MerrorCls:
	...
	def set(self, rms: float or bool, peak: float or bool) -> None:
		...
		Snippet: driver.configure.nrSubMeas.multiEval.limit.qpsk.merror.set(rms = 1.0, peak = 1.0) \n
		...
		self._core.io.write(f'CONFigure:NRSub:MEASurement<Instance>:MEValuation:LIMit:QPSK:MERRor {param}'.rstrip())
	...
	def get(self) -> MerrorStruct:
		"""SCPI: CONFigure:NRSub:MEASurement<Instance>:MEValuation:LIMit:QPSK:MERRor \n
		Snippet: value: MerrorStruct = driver.configure.nrSubMeas.multiEval.limit.qpsk.merror.get() \n
		Defines upper limits for the RMS and peak values of the magnitude error for QPSK. \n
			:return: structure: for return value, see the help for MerrorStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:NRSub:MEASurement<Instance>:MEValuation:LIMit:QPSK:MERRor?', self.__class__.MerrorStruct())
