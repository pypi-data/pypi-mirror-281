...
class CbandwidthCls:
	...
	def set(self, relative_level: float or bool, absolute_level: float or bool, channelBwExt=repcap.ChannelBwExt.Default) -> None:
		"""SCPI: CONFigure:NRSub:MEASurement<Instance>:MEValuation:LIMit:ACLR:NR:CBANdwidth<bw> \n
		Snippet: driver.configure.nrSubMeas.multiEval.limit.aclr.nr.cbandwidth.set(relative_level = 1.0, absolute_level = 1.0, channelBwExt = repcap.ChannelBwExt.Default) \n
		...
		self._core.io.write(f'CONFigure:NRSub:MEASurement<Instance>:MEValuation:LIMit:ACLR:NR:CBANdwidth{channelBwExt_cmd_val} {param}'.rstrip())
	...
	def get(self, channelBwExt=repcap.ChannelBwExt.Default) -> CbandwidthStruct:
		"""SCPI: CONFigure:NRSub:MEASurement<Instance>:MEValuation:LIMit:ACLR:NR:CBANdwidth<bw> \n
		Snippet: value: CbandwidthStruct = driver.configure.nrSubMeas.multiEval.limit.aclr.nr.cbandwidth.get(channelBwExt = repcap.ChannelBwExt.Default) \n
		Defines relative and absolute limits for the ACLR measured in an adjacent NR channel (for NR SA without CA) .
		The settings are defined separately for each channel bandwidth. \n
			:param channelBwExt: optional repeated capability selector. Default value: Bw5 (settable in the interface 'Cbandwidth')
			:return: structure: for return value, see the help for CbandwidthStruct structure arguments."""
		channelBwExt_cmd_val = self._cmd_group.get_repcap_cmd_value(channelBwExt, repcap.ChannelBwExt)
		return self._core.io.query_struct(f'CONFigure:NRSub:MEASurement<Instance>:MEValuation:LIMit:ACLR:NR:CBANdwidth{channelBwExt_cmd_val}?', self.__class__.CbandwidthStruct())
