...
class CurrentCls:
	...
	def read(self, carrierComponent=repcap.CarrierComponent.Default) -> ResultData:
		"""SCPI: READ:NRSub:MEASurement<Instance>:MEValuation[:CC<no>]:PERRor:CURRent \n
		Snippet: value: ResultData = driver.nrSubMeas.multiEval.cc.perror.current.read(carrierComponent = repcap.CarrierComponent.Default) \n
		...
		return self._core.io.query_struct(f'READ:NRSub:MEASurement<Instance>:MEValuation:CC{carrierComponent_cmd_val}:PERRor:CURRent?', self.__class__.ResultData())

	def fetch(self, carrierComponent=repcap.CarrierComponent.Default) -> ResultData:
		"""SCPI: FETCh:NRSub:MEASurement<Instance>:MEValuation[:CC<no>]:PERRor:CURRent \n
		Snippet: value: ResultData = driver.nrSubMeas.multiEval.cc.perror.current.fetch(carrierComponent = repcap.CarrierComponent.Default) \n
		Returns the values of the phase error bar graphs for the OFDM symbols in the measured slot, for carrier <no>. The results
		of the current, average and maximum bar graphs can be retrieved. There is one pair of phase error values per OFDM symbol,
		returned in the following order: <Reliability>, {<Low>, <High>}symbol 0, {<Low>, <High>}symbol 1, ... See also 'Views
		Magnitude Error, Phase Error'. \n
			...
			:return: structure: for return value, see the help for ResultData structure arguments."""
		carrierComponent_cmd_val = self._cmd_group.get_repcap_cmd_value(carrierComponent, repcap.CarrierComponent)
		return self._core.io.query_struct(f'FETCh:NRSub:MEASurement<Instance>:MEValuation:CC{carrierComponent_cmd_val}:PERRor:CURRent?', self.__class__.ResultData())
	...
	def calculate(self, carrierComponent=repcap.CarrierComponent.Default) -> CalculateStruct:
		"""SCPI: CALCulate:NRSub:MEASurement<Instance>:MEValuation[:CC<no>]:PERRor:CURRent \n
		Snippet: value: CalculateStruct = driver.nrSubMeas.multiEval.cc.perror.current.calculate(carrierComponent = repcap.CarrierComponent.Default) \n
		No command help available \n
			:param carrierComponent: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Cc')
			:return: structure: for return value, see the help for CalculateStruct structure arguments."""
		carrierComponent_cmd_val = self._cmd_group.get_repcap_cmd_value(carrierComponent, repcap.CarrierComponent)
		return self._core.io.query_struct(f'CALCulate:NRSub:MEASurement<Instance>:MEValuation:CC{carrierComponent_cmd_val}:PERRor:CURRent?', self.__class__.CalculateStruct())
