...
class RbIndexCls:
	...
	def fetch(self, carrierComponent=repcap.CarrierComponent.Default) -> FetchStruct:
		...
		Snippet: value: FetchStruct = driver.nrSubMeas.multiEval.cc.iemission.margin.current.rbIndex.fetch(carrierComponent = repcap.CarrierComponent.Default) \n
		...
		return self._core.io.query_struct(f'FETCh:NRSub:MEASurement<Instance>:MEValuation:CC{carrierComponent_cmd_val}:IEMission:MARGin:CURRent:RBINdex?', self.__class__.FetchStruct())
