...
class CurrentCls:
	...
	def fetch(self, carrierComponent=repcap.CarrierComponent.Default) -> FetchStruct:
		"""SCPI: FETCh:NRSub:MEASurement<Instance>:MEValuation[:CC<no>]:IEMission:MARGin:CURRent \n
		Snippet: value: FetchStruct = driver.nrSubMeas.multiEval.cc.iemission.margin.current.fetch(carrierComponent = repcap.CarrierComponent.Default) \n
		...
		return self._core.io.query_struct(f'FETCh:NRSub:MEASurement<Instance>:MEValuation:CC{carrierComponent_cmd_val}:IEMission:MARGin:CURRent?', self.__class__.FetchStruct())
