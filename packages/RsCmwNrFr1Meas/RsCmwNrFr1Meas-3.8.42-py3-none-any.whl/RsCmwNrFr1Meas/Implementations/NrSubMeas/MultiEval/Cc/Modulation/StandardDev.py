...
class StandardDevCls:
	...
	def read(self, carrierComponent=repcap.CarrierComponent.Default) -> ResultData:
		"""SCPI: READ:NRSub:MEASurement<Instance>:MEValuation[:CC<no>]:MODulation:SDEViation \n
		Snippet: value: ResultData = driver.nrSubMeas.multiEval.cc.modulation.standardDev.read(carrierComponent = repcap.CarrierComponent.Default) \n
		...
		return self._core.io.query_struct(f'READ:NRSub:MEASurement<Instance>:MEValuation:CC{carrierComponent_cmd_val}:MODulation:SDEViation?', self.__class__.ResultData())

	def fetch(self, carrierComponent=repcap.CarrierComponent.Default) -> ResultData:
		"""SCPI: FETCh:NRSub:MEASurement<Instance>:MEValuation[:CC<no>]:MODulation:SDEViation \n
		Snippet: value: ResultData = driver.nrSubMeas.multiEval.cc.modulation.standardDev.fetch(carrierComponent = repcap.CarrierComponent.Default) \n
		Return the current, average and standard deviation single-value results for carrier <no>. The values described below are
		returned by FETCh and READ commands. CALCulate commands return limit check results instead, one value for each result
		listed below. \n
			:param carrierComponent: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Cc')
			:return: structure: for return value, see the help for ResultData structure arguments."""
		carrierComponent_cmd_val = self._cmd_group.get_repcap_cmd_value(carrierComponent, repcap.CarrierComponent)
		return self._core.io.query_struct(f'FETCh:NRSub:MEASurement<Instance>:MEValuation:CC{carrierComponent_cmd_val}:MODulation:SDEViation?', self.__class__.ResultData())
