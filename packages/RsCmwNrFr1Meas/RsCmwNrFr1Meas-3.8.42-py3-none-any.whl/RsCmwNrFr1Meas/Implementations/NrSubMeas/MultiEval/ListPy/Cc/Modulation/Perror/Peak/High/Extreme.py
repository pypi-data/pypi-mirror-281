...
class ExtremeCls:
	...
	def fetch(self, carrierComponent=repcap.CarrierComponent.Default) -> List[float]:
		...
		Snippet: value: List[float] = driver.nrSubMeas.multiEval.listPy.cc.modulation.perror.peak.high.extreme.fetch(carrierComponent = repcap.CarrierComponent.Default) \n
		...
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'FETCh:NRSub:MEASurement<Instance>:MEValuation:LIST:CC{carrierComponent_cmd_val}:MODulation:PERRor:PEAK:HIGH:EXTReme?', suppressed)
		return response
	...
	def calculate(self, carrierComponent=repcap.CarrierComponent.Default) -> List[float or bool]:
		...
		Snippet: value: List[float or bool] = driver.nrSubMeas.multiEval.listPy.cc.modulation.perror.peak.high.extreme.calculate(carrierComponent = repcap.CarrierComponent.Default) \n
		Return phase error peak values for low and high EVM window position, for all measured list mode segments, for carrier <c>.
		The values described below are returned by FETCh commands. CALCulate commands return limit check results instead, one
		value for each result listed below. \n
		Use RsCmwNrFr1Meas.reliability.last_value to read the updated reliability indicator. \n
		...
		carrierComponent_cmd_val = self._cmd_group.get_repcap_cmd_value(carrierComponent, repcap.CarrierComponent)
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_str_suppressed(f'CALCulate:NRSub:MEASurement<Instance>:MEValuation:LIST:CC{carrierComponent_cmd_val}:MODulation:PERRor:PEAK:HIGH:EXTReme?', suppressed)
