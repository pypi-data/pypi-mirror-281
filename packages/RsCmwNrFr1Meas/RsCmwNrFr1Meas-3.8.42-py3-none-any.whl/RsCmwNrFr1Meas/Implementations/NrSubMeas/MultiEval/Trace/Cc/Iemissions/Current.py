...
class CurrentCls:
	...
	def read(self, carrierComponent=repcap.CarrierComponent.Default) -> List[float]:
		...
		Snippet: value: List[float] = driver.nrSubMeas.multiEval.trace.cc.iemissions.current.read(carrierComponent = repcap.CarrierComponent.Default) \n
		...
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'READ:NRSub:MEASurement<Instance>:MEValuation:TRACe:CC{carrierComponent_cmd_val}:IEMissions:CURRent?', suppressed)
		return response
	...
	def fetch(self, carrierComponent=repcap.CarrierComponent.Default) -> List[float]:
		...
		Snippet: value: List[float] = driver.nrSubMeas.multiEval.trace.cc.iemissions.current.fetch(carrierComponent = repcap.CarrierComponent.Default) \n
		Returns the values of the inband emissions trace for carrier <no>. The current, average and maximum traces can be
		retrieved. See also 'View Inband Emissions'. \n
		Use RsCmwNrFr1Meas.reliability.last_value to read the updated reliability indicator. \n
			:param carrierComponent: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Cc')
		...
		carrierComponent_cmd_val = self._cmd_group.get_repcap_cmd_value(carrierComponent, repcap.CarrierComponent)
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'FETCh:NRSub:MEASurement<Instance>:MEValuation:TRACe:CC{carrierComponent_cmd_val}:IEMissions:CURRent?', suppressed)
		return response
