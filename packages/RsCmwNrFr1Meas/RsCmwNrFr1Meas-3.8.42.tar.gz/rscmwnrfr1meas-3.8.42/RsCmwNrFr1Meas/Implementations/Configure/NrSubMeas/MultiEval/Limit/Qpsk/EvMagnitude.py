...
class EvMagnitudeCls:
	...
	def set(self, rms: float or bool, peak: float or bool) -> None:
		...
		Snippet: driver.configure.nrSubMeas.multiEval.limit.qpsk.evMagnitude.set(rms = 1.0, peak = 1.0) \n
		...
		self._core.io.write(f'CONFigure:NRSub:MEASurement<Instance>:MEValuation:LIMit:QPSK:EVMagnitude {param}'.rstrip())
	...
	def get(self) -> EvMagnitudeStruct:
		"""SCPI: CONFigure:NRSub:MEASurement<Instance>:MEValuation:LIMit:QPSK:EVMagnitude \n
		Snippet: value: EvMagnitudeStruct = driver.configure.nrSubMeas.multiEval.limit.qpsk.evMagnitude.get() \n
		Defines upper limits for the RMS and peak values of the error vector magnitude (EVM) for QPSK. \n
			:return: structure: for return value, see the help for EvMagnitudeStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:NRSub:MEASurement<Instance>:MEValuation:LIMit:QPSK:EVMagnitude?', self.__class__.EvMagnitudeStruct())
