...
class EsFlatnessCls:
	...
	def read(self, carrierComponent=repcap.CarrierComponent.Default) -> List[float]:
		...
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'READ:NRSub:MEASurement<Instance>:MEValuation:TRACe:CC{carrierComponent_cmd_val}:ESFLatness?', suppressed)
		return response
	...
	def fetch(self, carrierComponent=repcap.CarrierComponent.Default) -> List[float]:
		...
		carrierComponent_cmd_val = self._cmd_group.get_repcap_cmd_value(carrierComponent, repcap.CarrierComponent)
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'FETCh:NRSub:MEASurement<Instance>:MEValuation:TRACe:CC{carrierComponent_cmd_val}:ESFLatness?', suppressed)
		return response
