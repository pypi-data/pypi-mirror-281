...
class MinimumCls:
	...
	def read(self) -> ResultData:
		...
		Snippet: value: ResultData = driver.nrSubMeas.multiEval.txPower.minimum.read() \n
		Return the total TX power results (current, average, minimum, maximum and standard deviation) . \n
			:return: structure: for return value, see the help for ResultData structure arguments."""
		return self._core.io.query_struct(f'READ:NRSub:MEASurement<Instance>:MEValuation:TXPower:MINimum?', self.__class__.ResultData())

	def fetch(self) -> ResultData:
		"""SCPI: FETCh:NRSub:MEASurement<Instance>:MEValuation:TXPower:MINimum \n
		Snippet: value: ResultData = driver.nrSubMeas.multiEval.txPower.minimum.fetch() \n
		Return the total TX power results (current, average, minimum, maximum and standard deviation) . \n
			:return: structure: for return value, see the help for ResultData structure arguments."""
		return self._core.io.query_struct(f'FETCh:NRSub:MEASurement<Instance>:MEValuation:TXPower:MINimum?', self.__class__.ResultData())
