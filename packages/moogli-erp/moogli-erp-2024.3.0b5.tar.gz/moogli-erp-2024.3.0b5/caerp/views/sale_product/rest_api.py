...
from caerp.forms.sale_product.category import (
...
class RestWorkItemView(BaseRestView):
    ...
    def post_load_from_catalog_view(self):
        ...
        sale_product_quantities = self.request.json_body.get(
            "base_sale_product_quantities", []
        )
        logger.debug("sale_product_quantities : %s", sale_product_quantities)

        items = []
        for id_, quantity in sale_product_quantities.items():
            ...
            work_item.quantity = float(quantity)
