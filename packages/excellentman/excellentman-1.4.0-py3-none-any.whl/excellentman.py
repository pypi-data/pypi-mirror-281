...
def process_excel(file_name, collection, environment, custom_formats={}, custom_template=None, xunit_file=None, worksheet_filter=match_all):
    ...
    for worksheet in worksheets:
        ...
        passed = run_newman(csv_title, collection, environment,custom_template,xunit_passthrough)
        if not passed:
            all_passed = False
...
def main():
    ...
    if args.custom_formats:
        ...
        passed = process_excel(excel_file, collection, environment, custom_formats= formats.custom_formats,custom_template=args.custom_template,xunit_file=args.xunit_file, worksheet_filter=matcher)
    else:
        passed = process_excel(excel_file, collection, environment,custom_template=args.custom_template, xunit_file=args.xunit_file, worksheet_filter=matcher)
    if not passed:
        sys.exit(1)
