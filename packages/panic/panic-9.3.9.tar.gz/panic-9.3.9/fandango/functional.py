...
def filtersmart(seq, filters,default_prefix='&',wildcards=' '):
    """
    ...
    behavior of filters depends on first filter character:
        ...
        '&' or '+' : required, requires all those filters to be marched
    ...
    raw = [toRegexp(f[1:]) for f in filters if f.startswith('&')]
    comp = [toRegexp(f[1:]) for f in filters if f.startswith('+')]
    neg = [toRegexp(f[1:]) for f in filters if f[0] in "!-"]
    if default == 'AND':
        raw.extend([f for f in filters if f not in (comp,neg,raw)])
    else:
        raw.extend([f for f in filters if f not in (comp,neg,raw)])
    ...
    return [
        ...
        and any(searchCl(r, s) for r in raw)
