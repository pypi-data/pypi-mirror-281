...
class HDBppReader(HDBppDB):
    ...
    def get_partitions_sorted_by_time(self, table, parts = None, tmax = None):
        ...
        parts = parts or super(HDBppReader,self).getTablePartitions(table)
        try:
            parts = sorted((self.get_partition_time_by_name(p),p) 
                           for p in parts)
    ...
    def getTablePartitions(self,table):
        return self.get_partitions_sorted_by_time(table)
    
    getTablePartitions = get_partitions_sorted_by_time
    ...
    def get_attribute_last_values(self,attribute,n=1,
            check_attribute=False,stop=None,period=90*86400,**kwargs):
        """
        ...
        vals = []
        try:
            attribute,name = self.get_attribute_name(attribute),attribute
            epoch = kwargs.get('epoch',stop) #backwards compatibility
            ...
            if attribute:
                ...
                if len(vals):
                    vals = vals[0] if abs(n)==1 else vals #@TODO: don't mess with return format!!           
            else:
    ...
    get_last_attribute_values = get_attribute_last_values #backwards compatibility
    
    def load_last_values(self,attributes=None,n=1,stop=None,period=90*86400,
            brief=False,**kwargs):
        """
        load_last_values provided to comply with Reader API
        get_last_attribute_values provided to comply with CommonAPI 
        ...
        period/tref: time from which start searching values (-90d by default)
        stop/epoch: end of window to search values (now by default)
        brief: if len(attributes)==1: get a list instead of a dict
        """       
        attributes = self.get_archived_attributes() if attributes is None else attributes
        epoch,tref = kwargs.get('epoch',stop),kwargs.get('tref',period)
        ...
        return fn.first(vals.values()) if (brief and len(vals)==1) else vals
    ...
    def get_attribute_values_query(self,attribute,
        ...
        if decimate:
            # decimate = Xs ; 1 value every X seconds
    ...
    def get_attribute_values(self,attribute,
        ...
        self.debug('HDBpp.get_attribute_values(%s,%s,%s,N=%s,decimate=%s,'
                   'int_time=%s,%s)'
                ...
                if decimate!=RAW and span and (density*(stop_time-start_time))>limit:
                    ...
                    for l in v:
                        ...
                        if ((ix not in lasts)
                            ...
                            # Decimation by time interval
                            # Only back-from-none and values out of interval are added
                            result.append(l)
                            lasts[ix] = l
                else:
                    result.extend(v)
        ...
        if is_array and (not index or not self.INDEX_IN_QUERY):
            ...
            for k,v in sorted(data.items()):
                ...
                # if any value in the array is None or NaN, it is set to None
                l = last_arrs[:]
    ...
    def get_attribute_stats(self,attribute,start_date=None,stop_date=None,values=None):
        t0 = fn.now()
        r = fn.Struct()
        r.attribute = attribute
        if not values and start_date and stop_date:
            values = self.get_attribute_values(attribute,start_date,stop_date,decimate=None)
        r.tquery = fn.now() - t0
        r.nvalues = len(values)
        r.first,r.last = None, None
        r.nones,r.vmax,r.vmin,r.vavg,r.tmax,r.tmin,r.tavg,r.tfreq = 0,0,0,0,0,0,0,0
        r.tdups, r.vdups = 0,0
        if r.nvalues:
            r.first = values and values[0]
            r.last = values and values[-1]
            dn = (abs(r.last[0]-r.first[0])/r.nvalues)
            r.tfreq = dn and 1./dn
            prev = values[0]
            
            for i,t in enumerate(values):
                
                tdiff = abs(t[0]-prev[0])
                if not tdiff: 
                    r.tdups += 1
                if not r.tmax or tdiff>r.tmax:
                    r.tmax = tdiff
                if not r.vmin or tdiff<r.tmin:
                    r.tmin = tdiff
                r.tavg = (r.tavg * i + tdiff)/(i+1)

                    
                if None in t[:2]:
                    r.nones += 1
                elif isinstance(t[1],(int,float,long)):
                    if t[1] == prev[1]: 
                        r.vdups += 1
                    if not r.vmax or t[1]>r.vmax:
                        r.vmax = t[1]
                    if not r.vmin or t[1]<r.vmin:
                        r.vmin = t[1]
                    r.vavg = (r.vavg * (i-r.nones) + t[1])/((i+1-r.nones) or 1)
    ...
    def check_attributes(self,attrs = '', load = False, t0 = 0):
        ...
        if fn.isDictionary(attrs):
            ...
            if isinstance(fn.first(vals.values()),dict):
                vals = dict((k,fn.first(v.values())) for k,v in vals.items())
        else:
