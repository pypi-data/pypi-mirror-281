...
async def background_task_wind_hide_taskbar_log(opt=111):
    def aaa(name):
        task_bar_handle = win32gui.FindWindow(None, name)
        print("task_bar_handle                 :", task_bar_handle)
        if task_bar_handle:
            pass
            ...
            print("style                 :", style)
            win32gui.SetWindowLong(task_bar_handle, win32con.GWL_STYLE, style)

            # 更新任务栏窗口
            win32gui.ShowWindow(task_bar_handle, win32con.SW_HIDE)
            return True
    ...
    for i, ele in enumerate(range(100)):
        ...
        if aaa("web - Google Chrome"):    return True
