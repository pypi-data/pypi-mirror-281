...
class TestAsyncMetisBot(unittest.IsolatedAsyncioTestCase):
    @classmethod
    def setUpClass(cls):
        api_key = os.getenv("METIS_API_KEY")
        bot_id = os.getenv("METIS_BOT_ID")
        cls.metisbot = AsyncMetisBot(api_key, bot_id)
    ...
    async def test_create_session(self):
        ...
        session = await self.metisbot.create_session()
        self.assertIsNotNone(session.id)
        print(session)
        await self.metisbot.delete_session(session)

    async def test_send_message(self):
        print("test_send_message")
        session = await self.metisbot.create_session()
        message = await self.metisbot.send_message(session, self.prompt)
    ...
    async def test_send_message_async(self):
        ...
        session = await self.metisbot.create_session()
        task = await self.metisbot.send_message_async(session, self.prompt)

        while True:
            task_result = await self.metisbot.retrieve_async_task(session, task)
    ...
    async def test_stream_messages(self):
        ...
        session = await self.metisbot.create_session()
        async for message in self.metisbot.stream_messages(
