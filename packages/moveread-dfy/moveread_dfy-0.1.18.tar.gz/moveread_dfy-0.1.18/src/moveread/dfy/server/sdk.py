...
from haskellian import either as E, Left, Either, promise as P
...
class SDK:
  ...
  def round_pgn(self, *, skip: int = 0, take: int | None = None, **roundId: Unpack[RoundId]) -> Iterable[str]:
    with Session(self._engine) as ses:
      order: Any = Game.index
      round = ses.exec(queries.select_round(**roundId)).first()
      stmt = select(Game).where(*queries.round_games(**roundId)).order_by(order)
      games = ses.exec(stmt).all()[skip:][:take]
      tnmt = ses.exec(queries.select_tnmt(roundId['tournId'])).first()
      yield from export_all(games, tnmt, round)

  def group_pgn(self, *, skip: int = 0, take: int | None = None, **groupId: Unpack[GroupId]) -> Iterable[str]:
    with Session(self._engine) as ses:
      order: Any = Game.index
  ...
  async def post_game(
    ...
    descaled_height: int = 768, **gameId: Unpack[GameId]
