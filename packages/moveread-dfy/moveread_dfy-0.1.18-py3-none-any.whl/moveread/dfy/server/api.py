...
from .util import Throttler, clientId, from_lichess
...
def fastapi(
  ...
  throttler = Throttler(max_entries=1000, ttl_secs=5*60)
  ...
  def group_pgn(tournId: str, group: str, req: Request, skip: int = 0, take: int | None = None, dirty: bool = False):
    if (not dirty and from_lichess(req) and (id := clientId(req))
      and throttler.throttle('-'.join([id, tournId, group, str(skip), str(take)]))):
        logger(f'Throttling request from {id} for {tournId}/{group}.pgn', level='DEBUG')
        return Response(content='')
    pgn = sdk.group_pgn(**groupId(tournId, group), skip=skip, take=take)
  ...
  def round_pgn(tournId: str, group: str, round: str, req: Request, skip: int = 0, take: int | None = None, dirty: bool = False):
    if (not dirty and from_lichess(req) and (id := clientId(req))
      and throttler.throttle('-'.join([id, tournId, group, round, str(skip), str(take)]))):
        logger(f'Throttling request from {id} for {tournId}/{group}/{round}.pgn', level='DEBUG')
        return Response(content='')
    pgn = sdk.round_pgn(**roundId(tournId, group, round), skip=skip, take=take)
    return StreamingResponse(content=pgn, media_type='application/x-chess-pgn', headers={
