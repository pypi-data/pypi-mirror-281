...
class Shellium:
    ...
    def make(self):
        if Path(self.options.user_data_dir).exists():
            raise UserDataExistsError(f'{self.options.user_data_dir} already exists.')
        ...
        if process.returncode != 0:
            raise UserDataMakeError('Failed to build a User Data Dir.')
