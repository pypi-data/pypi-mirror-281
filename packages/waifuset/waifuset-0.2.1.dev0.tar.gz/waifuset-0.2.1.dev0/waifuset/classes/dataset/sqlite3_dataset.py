...
class SQLite3Dataset(SQLite3Database, DiskIOMixin, Dataset):
    ...
    def __init__(self, source: StrPath = None, tbname=None, **kwargs):
        ...
        if tbname is None:
            ...
            if tbname not in self.get_all_tablenames() and primary_key is not None:
                self.logger.warning(f"Table name {tbname} not found when initializing {self.__class__.__name__}, creating new table with primary key {primary_key} and columns {col2type}")
    ...
    def add_columns(self, col2type):
        if isinstance(col2type, dict):
            for col, col_type in col2type.items():
                self.table.add_column(col, col_type)
        elif isinstance(col2type, list):
            for col in col2type:
                self.table.add_column(col, None)
    ...
    def rename_columns(self, column_mapping):
        for old_col, new_col in column_mapping.items():
            self.table.rename_column(old_col, new_col)
        self.register_to_config(col2type=self.types)
