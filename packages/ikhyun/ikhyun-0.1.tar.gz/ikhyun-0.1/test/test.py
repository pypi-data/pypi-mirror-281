...
class SHLoader1(LoaderInterface):
    file_path = None
...
class NeMoWriter(WriterInterface):
    ...
    def __call__(self, sents, logger):
        with open(self._file_path, 'w', newline="", encoding="utf-8") as fout:
            ...
            for sent in tqdm(sents, desc = f'Writing : {self._file_path}'):
                for words in sent:
...
def main():
    ...
    sils =[",","\'", "\"","!","?", "."]
    loader = SHLoader1("/data/utils/ikhyun/data/label.csv")
    writer = NeMoWriter("/data/utils/ikhyun/data/test_res.csv", sils)
    ...
    #builder._build()
    #unit = TaggingUnit.CHAR
    #tagged_res = builder._tag(unit)

    #aligned_res = builder._align(tagged_res.cls, tagged_res.org, tagged_res.trs, unit)

    #aligned_res = builder._split_by_sils(aligned_res.sents)
