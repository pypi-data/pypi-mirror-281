...
def infer_jsl_signature(spark_model):
    ...
    if output_type == 'table':
        output_data = [d.to_dict() for d in output_data]
...
Yesterday:
- Good synch with Sai, should have model ready next week, then I can integrate to DB
- Continue working on the DB signatures for the table model, have to make some custom json format I think sicne we return
an array of padnas 


Today/Tomorrow: 
- Continue with Table extractor integration for DB 
- Guide Faisal on NLU CI/CD, making good progress. we have a jenkins pipe now for this, finalizing trigger mechanisms on Git
