...
def append_one(P):
    """Append a 1 to a position vector if the current length is 3.  If the
    length is not currently 3, do nothing and return the unmodified input (so
    that if the length is already 4, calling this function is safe)."""
    if len(P) == 3:
        P = np.append(P,[1])
    return P
