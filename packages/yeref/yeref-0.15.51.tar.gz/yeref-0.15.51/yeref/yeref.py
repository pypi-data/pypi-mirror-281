        ...
        if POST_TYPE == 'text':
            ...
            if POST_TYPE == 'gif':
                ...
                if BOT_TOKEN:
                    ...
                    await extra_bot.session.close()
                    POSTB_FID = res.animation.file_id
                ...
                if BOT_TOKEN:
                    extra_bot = Bot(token=BOT_TOKEN)
                    res = await extra_bot.send_sticker(chat_id=chat_id, sticker=types.FSInputFile(file_name))
                    await extra_bot.delete_message(chat_id, res.message_id)
                    await extra_bot.session.close()
                    POSTB_FID = res.sticker.file_id
            ...
            if POST_TYPE == 'audio':
                ...
                if BOT_TOKEN:
                    extra_bot = Bot(token=BOT_TOKEN)
                    res = await extra_bot.send_audio(chat_id=chat_id, audio=types.FSInputFile(dst), thumbnail=thumbnail, title=ENT_TITLE, performer=ENT_USERNAME)
                    await extra_bot.delete_message(chat_id, res.message_id)
                    await extra_bot.session.close()
                    POSTB_FID = res.audio.file_id
            else:
                ...
                if BOT_TOKEN:
                    ...
                    await extra_bot.session.close()
                    if res_note2.audio: POSTB_FIDNOTE = res.audio.file_id
