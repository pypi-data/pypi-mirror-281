            ...
            for item in data_:
                try:
                    ...
                    if item['type'] == 'tgph':
                        ...
                        providers = await get_prodia_keys(file_path)
                        for provider_keys in providers['image']:
                            for api_key in provider_keys:
                                try:
                                    print(f"{provider_keys}: {api_key}")
                                            ...
                                            while attempts > 0:
                                                async with session.get(url, headers=headers) as response:
                                                    ...
                                                    if status.lower() == 'succeeded':
                                                        result.append({'type': item['type'], 'answer': result['imageUrl']})
                                                        return
                                                attempts -= 1
...
async def get_prodia_keys(file_path):
    result = None
    try:
