...
# ------------------------自定义------------------------
import Microsoft.Web.WebView2.Core as WebView2Core
import Microsoft.Web.WebView2.WinForms as WebView2WinForms
from abc import ABC, abstractmethod
from typing import List

DoSomething:List[filter] = []
...
class EdgeChrome:
    ...
    def __init__(self, form, window, cache_dir):
        ...
        # ------------------------自定义 author:clb------------------------
        self.CoreWebView2 = None # webview对象
        self.is_filter = True # 是否开启过滤器
        # ------------------------自定义 author:clb------------------------
        ...
        # ------------------------自定义 author:clb------------------------
        self.web_view.CoreWebView2InitializationCompleted += self.on_filter
        # ------------------------自定义 author:clb------------------------
    ...
    def on_filter(self, sender, args):
        ...
        self.CoreWebView2 = sender.CoreWebView2

        if self.is_filter:
            for d in DoSomething:
                try:
                    d.do(self, sender, args)
                except Exception as e:
                    # 异常处理
                    logging.error(str(e))
                    pass
