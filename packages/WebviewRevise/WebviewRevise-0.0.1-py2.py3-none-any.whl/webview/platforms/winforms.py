    ...
    class BrowserForm(WinForms.Form):
        def __init__(self, window, cache_dir):
            ...
            if is_cef:
                ...
                # ------------------------自定义 author:clb------------------------
                global BrowserMap
                # self.browser.is_filter = False if hasattr(self.pywebview_window,"is_filter") and (isinstance(self.pywebview_window.is_filter, bool) and not self.pywebview_window.is_filter) else True
                self.browser.is_filter = self.pywebview_window.is_filter
                BrowserMap[self.uid] = {
                                        "Browser":self.browser,
                                        "WebView_Handle":str(self.browser.web_view.Handle),
                                        "getCoreWebView2Func":self.browser.get_CoreWebView2,
                                        "WinForms":self,
                                        "WinForms_Handle":str(self.Handle)
                                    }
                # ------------------------自定义 author:clb------------------------
            else:
