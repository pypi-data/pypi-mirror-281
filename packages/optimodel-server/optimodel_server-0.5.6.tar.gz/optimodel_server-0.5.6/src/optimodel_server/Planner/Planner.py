...
def getAllAvailableProviders(body: QueryBody):
    ...
    if SAAS_MODE is not None:
        ...
        for provider in allAvailableProviders:
            ...
            logger.info(
                f"Checking provider {provider['provider']}... with creds {body.credentials}"
            )
            if provider["provider"] == "together":
                ...
                if credsForProvider is not None:
                    ...
                    logger.warn(
                        f"No credentials provided for together provider, skipping"
                    )
            if provider["provider"] == "aws-bedrock":
                ...
                if credsForProvider is not None:
                    ...
                    logger.warn(
                        f"No credentials provided for aws-bedrock provider, skipping"
                    )
            if provider["provider"] == "openai":
                ...
                if credsForProvider is not None:
                    ...
                    logger.warn(
                        f"No credentials provided for openai provider, skipping"
                    )
