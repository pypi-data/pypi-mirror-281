...
export default class WebC__Events_Viewer extends Web_Component {    
    ...
    constructor() {
        super();
        this.events_to_hook  = ['messageSent', 'new_input_message', 'streamStart', 'streamData','streamComplete','streamError']
        this.events_hooked   = []  
        this.events_captured = []
    ...
    set_data_table() {
        let columns    =  [{ width: "10px"}, { width: "20%"}, {width: "80%" }]
    ...
    render_html() {
        ...
        table.headers     = ['#', 'event_type', 'event_data']
...
`
    ...
    on_event(event) {
        this.events_captured.push(event)
        let event_type = event.type
        let event_data = JSON.stringify(event.detail)
        this.add_row([this.index.toString(), event_type, event_data])
        this.index++
    }
