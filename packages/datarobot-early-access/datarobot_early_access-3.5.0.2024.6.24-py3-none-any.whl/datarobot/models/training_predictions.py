...
class TrainingPredictions(APIObject):
    ...
    def get_all_as_dataframe(self, class_prefix=enums.PREDICTION_PREFIX.DEFAULT, serializer="csv"):
        """
        ...
        serializer : str, optional
            Serializer to use for the download. Options: ``csv`` (default).
        ...
        if serializer not in serializers:
            raise ValueError(f'Unknown serializer "{serializer}", use "csv"')

        if serializer == "json":
            deprecation_warning(
                subject="Parameter `serializer='json'` for get_all_as_dataframe is deprecated.",
                deprecated_since_version="3.4",
                will_remove_version="3.6",
                message="The parameter `serializer='json'` has been deprecated. Use `serializer='csv'` instead.",
            )
