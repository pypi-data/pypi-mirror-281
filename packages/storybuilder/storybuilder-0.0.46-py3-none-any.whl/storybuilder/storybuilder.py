                                ...
                                if isinstance(transcriptObject, str):
                                    transcriptStr = transcriptObject \
                                        if isinstance(alternativeObject, dict) \
                                            and (alternativeObject.get(DEFAULT_LANGUAGE, None) != None) \
                                    ...
                                    transcripts[i].languages = processedLanguages
                            except Exception as e:
...
class ExamPage(Page):
    ...
    def __init__(self, storyInstance, actor=None, postures=["smilesay"], audio=None, **kwargs):
        ...
        if all(key in kwargs for key in ("voices", "board", "objects", "interactions")):
            ...
            self.soundFile = f"voice-{uuid.uuid4()}.mp3" if audio == None else audio
...
class NotesPage(Page):
    ...
    def export(self, voiceOffset=0, pageId=0.0):
        ...
        outTranscripts = [noteScript.export()]
...
class ConcentrakPage(Page):
    ...
    def addNarration(self, text, narrator=None, alternativeText=None, audio=None, **kwargs):
        ...
        self.transcripts.append(Script(
            sound = f"voice-{uuid.uuid4()}.mp3" if audio == None else audio,
            transcript = MText(text),
            narrator = narrator,
    ...
    def updateNarration(self, pos, text=None, narrator=None, alternativeText=None):
        ...
        if pos < len(self.transcripts) and pos >= 0 and (transcript!=None or narrator!=None or alternative!=None):
            ...
            if transcript != None:
                self.transcripts[pos].transcript = update_object(self.transcripts[pos].transcript, transcript, self.locale)
                self.interactions[pos].text = update_object(self.interactions[pos].text, transcript, self.locale)
            if alternative != None:
                self.transcripts[pos].alternative = update_object(self.transcripts[pos].alternative, alternative, self.locale)
            if narrator != None:
...
class BoardPage(Page):
    ...
    def updateNarration(self, pos, text:Union[str, dict]=None, narrator=None, alternativeText:Union[str, dict]=None):
        if pos < len(self.narrations["transcripts"]) and pos >= 0 and (text!=None or narrator!=None or alternativeText!=None):
            self.narrations["transcripts"][pos].reset2basename()
            if text != None:
