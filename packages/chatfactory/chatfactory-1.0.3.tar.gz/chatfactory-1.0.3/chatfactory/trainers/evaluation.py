...
def generate_dialogue(dia_generator, agent, users, agent_prob=None, stat_type=None):
    ...
    if rand_num == 1:
        player_receive = agent
    else:
        player_receive = random.choice(users)

    player_send = random.choice(users)
    ...
    if stat_type == 'primitive':
        ...
        default_args_list = [{"player_receive": player_receive,
        ...
        default_args_list[0] = {"player_send": player_send}
        default_args_list[random_id] = {"player_receive": player_receive, "player_send": player_send}
