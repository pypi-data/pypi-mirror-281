...
def compute_policy_steps(policy, item,  can_not_action_res,
    ...
    if policy.dialogue.dia_generator.knowledge_state.check(loc_is_rev):
        go_steps = []
    ...
    if action_res != shelpers.reduce_sentences([orig_res[0]])[0]:
        ...
        steps_checked, steps_not_checked = compute_say_steps(flattened_res,
                                                             orig_res,
                                                             policy.player,
                                                             policy.dialogue.dia_generator.knowledge_state)
    ...
    if action_res != shelpers.reduce_sentences([orig_res[0]])[0]:
        ...
        while True:
            if "open" not in curr_loc.attributes:
                ...
                is_not_openable = tsentences.be((curr_loc, None),
                                                ...
                                                ("not", None),
                                                ("openable", None))
                if (policy.dialogue.dia_generator.knowledge_state.check(is_locked) is True or
                        policy.dialogue.dia_generator.knowledge_state.check(is_not_openable) is True):
                    flag_open = False
        ...
        if flag_open:
            for item_loc in candidates_open:
                ...
                if (prec_steps is None and policy.dialogue.dia_generator.knowledge_state.check(res3) is not False and
                        policy.dialogue.dia_generator.knowledge_state.check(res1) and
                        policy.dialogue.dia_generator.knowledge_state.check(res2)):
                        ...
                        for step in steps:
                            ...
                            for sent in inner_sentences:
                                if sent == desc_mappers.be(sent.describers) and sent.describers[0].get_arg('Arg-PRD') in ['open', 'openable', 'locked']:
                                    ...
                                    curr_loc = check_loc(policy.dialogue.dia_generator.knowledge_state,
                                                         item,
...
def compute_say_steps(flattened_env_res, orig_env_res, player, knowledge_state):
    """
    ...
    knowledge_state : KnowledgeState
        The knowledge state is used to check whether the agent has seen the environment responses in the past.
    ...
    for idx, res in enumerate(flattened_env_res):
        ...
        step = tsentences.say(player,
                              ...
                              orig_env_res[idx],
                              speaker=player)
        if knowledge_state.multi_check(res):
...
def check_path(know_state, iloc, directions, dir_idx, tloc):
    """
    ...
    Parameters
    ----------
    know_state : KnowledgeState
        The knowledge state is used to check whether the agent knows that
    ...
    is_seen = kn_checkers.property_check_alt(know_state, iloc, pkey, pval, None)

    if is_seen:
        further_check = check_path(know_state, pval, directions, dir_idx+1, tloc)
...
def check_loc(know_state, item, item_loc):
    """ Checks whether the agent has seen where the item is located.
        ...
        ----------
        know_state : KnowledgeState
            The knowledge state stores the information that the agent has seen.
        item : Entity
    ...
    is_seen = kn_checkers.property_check_alt(know_state, item, 'location', item_loc, None)
    if item_loc[1] == item and is_seen:
        return None

    if is_seen:
        further_check = check_loc(know_state, item_loc[1], item_loc[1].properties["location"])
        if further_check is None:
            return None
        else:
            return item_loc[1]
...
def path_revealed(dialogue, player, sloc, tloc, neg_res):
    ...
    know_state = dialogue.dia_generator.knowledge_state

    if combination in know_state.world.all_paths:
        dirs = know_state.world.all_paths[combination]
        step = check_path(know_state, sloc, dirs, 0, tloc)

    else:
        path_not_exist = True
        for direction in dialogue.dia_generator.world.directions:
            for place in dialogue.dia_generator.world.places:
                if kn_checkers.check_elem_exists_alt(know_state, place, direction, None) is None:
...
def prec_action_item(dialogue,
    ...
    for item in items:
        item_loc_not_rev = check_loc(know_state, item, item.properties["location"])
