...
def find_empty_entities(dia_generator):
    ...
    for ent in dia_generator.world.obj_list:
        for lp in dia_generator.world.location_positions:
            if len(ent.objects) == 0 and dia_generator.knowledge_state.check(tsentences.have((ent, None),
                                                                                             ('has', None),
                                                                                             ('not', None),
                                                                                             ('items', None),
                                                                                             ([lp, ent], None))):
...
def update_seen_location(dia_generator, hard_world, empty_entities):
    ...
    for ent in hard_world.obj_list:
        for empty_ent in empty_entities:
            for lp in dia_generator.world.location_positions:
                kn_updaters.property_update_alt(dia_generator.knowledge_state, ent, "location", [lp, empty_ent], "not")
...
def build_world():
    ...
    cat = builders.build_player(world, 'cat', color="brown", _type="cat", location=['in', home_office])
