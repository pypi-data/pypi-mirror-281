...
def reduce_say(say_sent, player_send, player_receive, speaker):
    """
    ...
    for sent in reduced_sent:
        new_sent.append(say(player_send,  None, 'says', sent, player_receive, speaker=speaker))
...
def say(player_send=(None, None), neg=(None, None), rel=(None, None),
        statement=(None, None), player_receive=(None, None), speaker=None):
    """
    Creates a Sentence for the verb say in the following form:

        <player_send> (neg) <say> <player_receive>: <statement>
    ...
    sent = lc.Sentence([player_send[1],
                       neg[1],
                       rel[1],
                       player_receive[1],
                       lc.Word(':'),
                       statement[1]], speaker=speaker)
    describer = tdescribers.say(player_send, neg, rel, statement, player_receive)
...
def get(player=(None, None), mod=(None, None), neg=(None, None), rel=(None, None),
    ...
    """ Creates a Sentence for the verb get in the following form:
            ...
            <player_send> <mod> <neg> <rel> <entity> <prepos_location>
