...
# -*- coding: utf-8 -*-
"""
This module contains the functions that update the KnowledgeState.
"""
...
def basic_update(kn_state, sent):
    """
    ...
    kn_state : KnowledgeState
        The knowledge state to be potentially updated.
    sent : Sentence
    ...
    if sent not in kn_state.sent_db:
        kn_state.sent_db.add(sent)

        def undo_add(know_state=kn_state, sentence=sent):
            know_state.sent_db.discard(sentence)

        kn_state.undo_changes.append(undo_add)

    opposite_sent = kn_helpers.create_oppos_sent(sent)
    if opposite_sent in kn_state.sent_db:
        kn_state.sent_db.remove(opposite_sent)

        def undo_remove(know_state=kn_state, sentence=opposite_sent):
            know_state.sent_db.add(sentence)

        kn_state.undo_changes.append(undo_remove)


def property_update(kn_state, sent):
    """
    ...
    it updates the knowledge state with further information, too.
    For example, if a door is locked, it means that the door is not open as well.

    Parameters
    ----------
    kn_state : KnowledgeState
        The knowledge state to be updated.
    sent : Sentence
    ...
    if mem is not None:
        ...
        property_update_alt(kn_state, ent, pkey, pval, pneg)

        ent = getattr(kn_state.world, ent.get_property("var_name"), None)
        if ent is not None:
            if pkey is None:
                if pval == 'locked' and pneg is None and 'locked' in ent.attributes:
                    property_update_alt(kn_state, ent, None, "open", "not")
            else:
                if pkey == 'type' and pneg is None and ent.properties['type'] == pval:
                    if 'place' in ent.attributes:
                        if 'location' in ent.properties:
                            property_update_alt(kn_state, ent, "location", ent.properties['location'], None)
    return None


def property_update_alt(kn_state, ent, pkey, pval, pneg):
    """
    ...
    where each of the entities is taken from kn_state.world in case the entity is a copy.
    ...
    ----------
    kn_state : KnowledgeState
        The knowledge state is used to fetch the world.
    ent : Entity
    ...
    if isinstance(ent, em.Entity):
        world_ent = getattr(kn_state.world, ent.get_property("var_name"), None)
    else:
        world_ent = None
    ...
    if pkey is not None:
        ...
        if pneg is None:
            kn_helpers.add_prop_seen(kn_state, world_ent, pkey, pval)
            kn_helpers.remove_prop_seen_neg(kn_state, world_ent, pkey, pval)
        else:
            kn_helpers.add_prop_seen_neg(kn_state, world_ent, pkey, pval)
            if pkey in world_ent.prop_seen and world_ent.prop_seen[pkey] == pval:
                kn_helpers.remove_prop_seen(kn_state, world_ent, pkey)
        ...
        if pneg is None:
            kn_helpers.add_attr_seen(kn_state, world_ent, pval, None)
            kn_helpers.remove_attr_seen(kn_state, world_ent, pval, "not")

        else:
            kn_helpers.add_attr_seen(kn_state, world_ent, pval, pneg)
            kn_helpers.remove_attr_seen(kn_state, world_ent, pval, None)
    return None


def update_elem_exists(kn_state, sent):
    """
    ...
    if parsed is not None:
        ...
        update_elem_exists_alt(kn_state, ent, elem, pneg)


def update_elem_exists_alt(kn_state, ent, elem, pneg):
    """
    Updates one of the following dictionaries:
    ...
    if cond:
        ...
        if pneg is None:
            if elem not in world_ent.elem_exists:
                ...
                kn_state.undo_changes.append(undo_exists)
            ...
            if elem in world_ent.elem_not_exists:
                ...
                kn_state.undo_changes.append(undo_not_exists)
            ...
            if elem in world_ent.elem_exists:
                ...
                kn_state.undo_changes.append(undo_elem_exists)

    return None


def have_update(kn_state, sent):
    """
    ...
    if possession == "items" and neg is not None:
        all_objects = kn_state.world.obj_list
        for obj in all_objects:
            property_update_alt(kn_state, obj, "location", loc, "not")
    else:
        ...
        for obj in possession:
            property_update_alt(kn_state, obj, "location", loc, neg)
                ...
                if lc.verb_inf(rel_look.lower()) == "look" and lc.verb_tense(rel_look) in lc.PRESENT_TENSES:
                    ...
                    if (look_desc == sent.describers[1] and player == player_look and isinstance(direction, str)
                        ...
                        if direction is not None and player is not None:
                            property_update_alt(kn_state, start_point[-1], direction,
                                                start_point[1].properties[direction], None)

                        property_update_alt(kn_state, player, "location", thing_looked, None)
    return None


def get_updater(kn_state, sent):
    """
    ...
    if len(sent.describers) > 0:
        ...
        if mapped_sent == sent:
            if all(map(isinstance, [player, thing_gotten], [em.Entity, em.Entity])):
                property_update_alt(kn_state, thing_gotten, "location", ["in", player], None)


def drop_updater(kn_state, sent):
    """
    ...
    if len(sent.describers) > 0:
        ...
        if mapped_sent == sent:
            if (all(map(isinstance, [player, thing_dropped, location], [em.Entity, em.Entity, list]))
                    ...
                    and location[-2] in kn_state.world.location_positions):
                for ent in [player, thing_dropped]:
                    property_update_alt(kn_state, ent, "location", location, None)


def see_updater(kn_state, sent):
...
def look_updater(kn_state, sent):
    """
    ...
    if len(sent.describers) > 0:
        ...
        if describer == mapped_desc:
            ...
            if (isinstance(thing_looked, list)
                    and thing_looked[0] in kn_state.world.location_positions
                    and all(map(isinstance, [thing_looked[1], player], [em.Entity, em.Entity]))):
                for ent in [player, thing_looked[1]]:
                    property_update_alt(kn_state, ent, "location", thing_looked, None)
            if len(sent.describers) == 2:
                ...
                see_updater(kn_state, see_sent)

                if added:
                    del sent.describers[1].args["AM-LOC"]


def opens_updater(kn_state, sent):
    ...
    if len(sent.describers) > 0:
        ...
        if mapped_sent == sent:
            if all(map(isinstance, [opener, thing_opened], [em.Entity, em.Entity])):
                property_update_alt(kn_state, thing_opened, None, "open", None)
                property_update_alt(kn_state, thing_opened, None, "openable", None)


def close_updater(kn_state, sent):
    """ If the sentence is in the format:
    ...
    if len(sent.describers) > 0:
        ...
        if mapped_sent == sent:
            if all(map(isinstance, [closer, thing_closed], [em.Entity, em.Entity])):
                property_update_alt(kn_state, thing_closed, None, "open", "not")
                property_update_alt(kn_state, thing_closed, None, "openable", None)


def change_updater(kn_state, sent):
    """
    ...
    if len(sent.describers) > 0:
        ...
        if arg_ppt is None:
            return None
        changable_properties = kn_state.world.change_action_properties
        if isinstance(arg_ppt, lc.Sentence):
            ...
            if tsentences.permit(tsentences.change(rel="changing", thing_changing=element_key), "not", "permitted") == sent and element_key not in changable_properties:
                basic_update(kn_state, sent)


def val_is_key_updater(kn_state, sent):
    ...
    if len(sent.describers) > 0:
        ...
        if arg_prd not in kn_state.world.all_properties+['direction']+additional_prds:
            return None
        kn_checked = kn_state.world.check_val_is_key(arg_prd, arg_ppt)
...
def permit_updater(kn_state, sent):
    """ Adds one of the following sentences in the database:
