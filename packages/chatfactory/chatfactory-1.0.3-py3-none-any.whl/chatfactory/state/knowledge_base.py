...
class KnowledgeState:
    """
    The knowledge state is used to efficiently store information and quickly fetch information that
    ...
    On the contrary, the knowledge state only passes the meta context only once and stores the information in
    ...
    ----------
    context : ListDb
        The meta-context is used to update the knowledge state.
    world : World
    ...
    updaters : list
        A list of functions that update the knowledge state.
    checkers : list
    ...
    def __init__(self, meta_context, world, undo_changes=None,
        ...
        self.updaters = [kn_updaters.property_update,
                         ...
                         kn_updaters.val_is_key_updater] if updaters is None else updaters
    ...
    def context_update(self):
        """ Updates the knowledge state with the unseen context sentences. """
    ...
    def multi_update(self, sents):
        """ Updates the knowledge state with a list of sentences (each class of components.Sentence) """
        for sent in sents:
            try:
    ...
    def single_update(self, sent):
        """ Updates the knowledge state with a single sentence. """
        for updater in self.updaters:
            updater(self, sent)

    def multi_check(self, sents):
        """ Checks the sentences against the knowledge state for their validity.
    ...
    def check(self, sent, update_context=True):
        """ Checks the validity of a single sentence against the knowledge state.
            A sentence is valid if it's factual, and it is observed by the agent.
            If update_context is true, it will update the knowledge state first
