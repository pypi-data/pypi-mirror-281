...
def go_direction(dia_generator, player_send, player_receive, direction, entities_descriptions=None):
    """ Creates a dialogue between a user (player_send), an agent (player_receive) and the environment.
    ...
    """
    user_policy = helpers.find_policy(dia_generator.user_policy_database[player_send],
                                      user_policies.GoDirectionPolicy
                                      )
    user_policy.agent = player_receive
    user_policy.direction = direction

    agent_policy = helpers.find_policy(dia_generator.agent_policy_database[player_receive],
...
def action_item(dia_generator, player_send, player_receive, user_pol_class, agent_pol_class,
...
def change_prop(dia_generator, player_send, player_receive, item,
...
def and_(dia_generator, player_send, any_template1, any_template2, entities_descriptions=None):
    """
    Creates a dialogue between a user (player_send) and multiple agents in the environment.
    ...
    player_send_pol = helpers.find_policy(dialogue.dia_generator.user_policy_database[player_send],
                                          user_policies.AndPolicy
                                          )

    player_send_pol.user_policies = [user_pol1, user_pol2]
    dialogue.add_policies([player_send_pol] + participants_policies)
