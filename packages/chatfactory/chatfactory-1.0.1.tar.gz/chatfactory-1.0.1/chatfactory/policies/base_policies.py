...
class ActionPolicy(BasePolicy, ABC):
        ...
        **task_params : dict
        ...
        for sitem in similar_items:
            ...
            for elem in item.description.elements:
                if elem in sitem.properties and not kn_checkers.property_check_alt(know_state, sitem, elem, sitem.properties[elem], None):
                    result = False
                if elem != 'abstract' and elem in sitem.attributes and not kn_checkers.property_check_alt(know_state, sitem, None, elem, None):
                    result = False
            if result is True:
    ...
    def save_state(self):
        ...
        if self.dialogue is not None:
            kn_state = self.dialogue.dia_generator.knowledge_state.save_state()
        else:
            kn_state = None
        if self.go_location_policy is not None:
            go_location_state = self.go_location_policy.save_state()

        return super().save_state(), go_location_state, self.item, self.prev_user_command, kn_state
    ...
    def recover_state(self, state):
        ...
        if self.dialogue is not None:
            self.dialogue.dia_generator.knowledge_state.recover_state(state[4])
