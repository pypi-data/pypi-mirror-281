...
class GoLocationPolicy(bpolicies.ActionPolicy):
    ...
    def task(self, item, prepos_location=None, preconditions=True):
        ...
        if dirs is not None and len(dirs) == 0:
            ...
            if prec_steps is not None and self.dialogue.dia_generator.knowledge_state.check(step) is not True:
                steps = prec_steps
            else:
                steps = [say_step]
            ...
            if dirs is not None and len(dirs) > 0:
                ...
                for direction in dirs:
                    ...
                    if (direction, 'obstacle') in player_loc.properties:
                        ...
                        check_obstacle = kn_checkers.property_check_alt(self.dialogue.dia_generator.knowledge_state,
                                                                        player_loc,
                                                                        (direction, "obstacle"),
                                                                        obs,
                                                                        None
                                                                        )
                        x_is_door = kn_checkers.property_check_alt(self.dialogue.dia_generator.knowledge_state,
                                                                   obs,
                                                                   "type",
                                                                   "door",
                                                                   None)
                ...
                if len(neg_responses) > 0:
                    ...
                    steps_checked, steps_unchecked = phelpers.compute_say_steps(reasons,
                                                                                neg_responses,
                                                                                self.player,
                                                                                self.dialogue.dia_generator.knowledge_state)
    ...
    def task(self, direction, last_user_command=None):
        ...
        if go_step != lhelpers.reduce_sentences([orig_res[0]])[0]:
            ...
            if ((direction, "obstacle") in initial_loc.properties and
                self.dialogue.dia_generator.knowledge_state.multi_check([player_loc, obstacle_present, door_closed, x_is_door])
                    and self.dialogue.dia_generator.knowledge_state.check(door_not_locked) is not True):
                ...
                if len(steps_checked) > 0:
                    player_loc = tsentences.be(([self.player, "'s", "location"], None),
                                               ...
                                               (None, None),
                                               (['in', initial_loc], None))
                    player_loc_checked = self.dialogue.dia_generator.knowledge_state.check(player_loc)
                    steps = []
...
class IsItemPolicy(bpolicies.IsItem):
    ...
    def task(self, item, property_key, property_val):
        ...
        know_state = self.dialogue.dia_generator.knowledge_state
        is_seen_pos = kn_checkers.property_check_alt(know_state,
                                                     item,
                                                     property_key,
                                                     property_val,
                                                     None)
...
class IsItemAttributePolicy(bpolicies.IsItem):
    ...
    def task(self, item, attribute):
        ...
        kn_state = dialogue.dia_generator.knowledge_state
        is_seen_pos = kn_checkers.property_check_alt(kn_state, item, None, attribute, None)
...
class ChangePolicy(bpolicies.ActionPolicy):
    ...
    def task(self, item, element_key, end_state):
        ...
        get_neg = False
        if element_key in ['color', 'size'] and self.dialogue.dia_generator.knowledge_state.check(sent2) and not self.dialogue.dia_generator.knowledge_state.check(loc_is_rev):
