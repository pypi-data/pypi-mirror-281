...
def say(describers, no_parts=True):
    ...
    if lc.verb_inf(rel.lower()) != "say":
        return None
    player_send = describers[0].get_arg("Arg-PAG")
    neg = describers[0].get_arg("AM-NEG")
    statement = describers[0].get_arg("Arg-PPT")
    player_receive = describers[0].get_arg("Arg-GOL")

    if no_parts:
        res = lc.Sentence()
        describer = tdescribers.say((player_send, None), (neg, None), (rel, None),
                                    (statement, None), (player_receive, None))
        res.describers.append(describer)
    else:
        res = tsentences.say(player_send, neg, rel, statement, player_receive)
    return res
