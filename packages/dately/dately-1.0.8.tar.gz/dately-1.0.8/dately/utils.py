...
def check_version_sufficiency(current_version, minimum_required_version='3.9'):
    """
    Checks if the current version is at least the minimum required version.

    Args:
    current_version (str): The version of the software currently running.
    ...
    """
    # Convert version strings into integer lists
    ...
    # Compare version numbers element-wise
    for current, minimum in zip(parts_current, parts_minimum):
        if current > minimum:
            return True
