...
namespace PyGroup
{
    ...
    template <typename T> T* get_pointer(PyGroup::auto_ptr<T> const& p) {
...
{
  template <class T> struct pointee<PyGroup::auto_ptr<T>>
  {
     typedef T type;
  };
}}
...
//    ;
    class_<Tango::Group, PyGroup::auto_ptr<Tango::Group>, boost::noncopyable >
        Group("__Group",
              init<const std::string&>())
    ;
