...
void is_method_defined(bopy::object &obj, const std::string &method_name,
...
void ensure_default_telemetry_interface_initialized()
{
    if (!telemetry_interface)
    {
        Tango::telemetry::Configuration cfg{
            .enabled=true,
            .kernel_traces_enabled=false,
            .name_space="tango",
            .details=Tango::telemetry::Configuration::Client{"pytango.client"},
             // don't set .collector_traces_endpoint, so env or or default will be used
        };
...
public:
    TraceContextScope(const std::string &new_span_name_, const std::string &trace_parent_, const std::string &trace_state_)
     :
    ...
    void acquire()
    {
        if (scope == nullptr)
        {
            ...
            scope = Tango::telemetry::Interface::set_trace_context(
                ...
                trace_parent,
                trace_state,
                Tango::telemetry::Span::Kind::kClient
            );
        }
    }
...
{
public:
    NoOpTraceContextScope(const std::string &new_span_name_, const std::string &trace_parent_, const std::string &trace_state_) {}
    void acquire() {}
    void release() {}
