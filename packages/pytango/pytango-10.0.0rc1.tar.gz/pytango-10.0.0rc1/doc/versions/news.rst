...
You can find the full changelog in :ref:`revisions history <pytango-version-history>` with the links to related MR,
where the detailed descriptions of every change/new feature can be found

Here we provide short highlights of changes:
...
- When using the asyncio green mode, a :class:`DeprecationWarning` will be emitted during the
  ...
  Note, that most probably the support of sync methods in Asyncio servers will be removed in the further.
- Error messages, arose during device start-up, are now redirected to the stderr instead of stdout
- PyTango now must be complied with C++17 standard
- PyTango now compiled with support of OpenTelemetry (in Linux and MacOs)
...
+------------------------+--------------+--------------+--------------+
| tango-idl              |    6.0       |   6.0        |   6.0        |
+------------------------+--------------+--------------+--------------+
| boost                  |  1.85.0      |  1.85.0      |  1.82.0      |
+------------------------+--------------+--------------+--------------+
| abseil                 |  20240116.2  |      -       | 20240116.2   |
+------------------------+--------------+--------------+--------------+
| protobuf               |  v27.1       |      -       |  v25.3       |
+------------------------+--------------+--------------+--------------+
| c-ares                 | 1.29.0       |      -       |   1.28.1     |
+------------------------+--------------+--------------+--------------+
| re2                    | 2024-06-01   |      -       | 2023.09.01   |
+------------------------+--------------+--------------+--------------+
| OpenSSL                |    3.3.1     |      -       |   3.3.1      |
+------------------------+--------------+--------------+--------------+
| curl                   | curl-8_8_0   |      -       |   8.8.0      |
+------------------------+--------------+--------------+--------------+
| gRPC                   |   v1.64.2    |      -       |   1.62.2     |
+------------------------+--------------+--------------+--------------+
| opentelemetry-cpp      |   v1.15.0    |      -       |   1.15.0     |
+------------------------+--------------+--------------+--------------+
...
- Numpy 2.0 support
- PyTango now supports OpenTelemetry support for distributed tracing
- New Alarm event was added
- Added DevInfo6 implementation (IDL 6)
...
- PyTango provides stub file to better autocompletion
- Events now can be pushed with python`s exceptions directly, no need to convert to DevFailed
- Device description, status, state can be set at the device start-up

Fixed
=====

- Fixed deadlock when pushing event from attribute read method, introduced in 9.5.1
- Memory leak when writing DevString attribute with DeviceProxy is fixed
- Segfault in push_archive_event(self, attr_name) with attr_name != state or status was fixed
- Various fixes of Asyncio DeviceProxy methods
- Fixes of \*_asynch methods on DeviceProxy
- class_property is now inherited in child classes

Removed
=======

- Fix, resolving crash of Asyncio devices, when an attribute is read
  at the same time as an event is being pushed was removed (original bug is not fixed)
