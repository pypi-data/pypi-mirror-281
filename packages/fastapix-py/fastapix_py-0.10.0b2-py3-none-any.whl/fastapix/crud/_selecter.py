...
class Foreign:
    ...
    def __call__(self):
        ...
        def _foreign(foreign: List[Enum("enum", foreign_names)] = Query(
            None, title='外连接', description='`join`'
        )):
            if foreign is None:
                return []
            foreign = [f.name for f in foreign]
            return [fm for fm in self.foreign_models if fm.__tablename__ in foreign]
