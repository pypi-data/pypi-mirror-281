...
class Resources:
    ...
    def get_res_path(self, relative_path: str) -> str:
        path = os.path.join(self.base.resdir, relative_path)
        if os.path.exists(path):
            return path

        fallback_path = os.path.join(self.base.fallback_resdir, relative_path)
        if os.path.exists(fallback_path):
            return fallback_path

        return os.path.join(self.base.second_fallback_resdir, relative_path)

    def get_res_path_platform(self, relative_path: str) -> str:
        if self.base.system.is_windows:
            return self.get_res_path(os.path.join("win32", relative_path))

        return self.get_res_path(os.path.join("linux", relative_path))
