...
class MorphConfig:
    ...
    def get_filepath(self, alias: str) -> Optional[str]:
        ...
        return os.path.join(self.config_path, os.path.relpath(resource["path"])) if resource and "path" in resource else None
    ...
    def get_alias(self, abs_path: str) -> str:
        for alias, resource in self.config["resources"].items():
            if os.path.join(self.config_path, os.path.relpath(resource["path"])) == abs_path:
                return alias
        return None
    ...
    def get_code(path_to_config_dir: str, filepath: str) -> str:
        path = os.path.join(path_to_config_dir, filepath) if not filepath.startswith(path_to_config_dir) else filepath
