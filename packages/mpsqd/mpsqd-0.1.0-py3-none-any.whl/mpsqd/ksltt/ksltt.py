...
def ksltt(rin,pall,dt,update_type='krylov',mmax=30):
...
#def ksltt2(rin,pall,dt,update_type='krylov',mmax=30):
def ksltt2(rin,pall,dt,mmax=30,small=1e-13):
...
###### the first part of KSL, from left to right
  for i in range(nlen-1):
    ...
    if (i != nlen-2):
      phi1 = ttf.phia_next(phi1,pall.nodes[i],q,q,0)
      phia[i+1] = phi1
#      r = ttf.update_k(r,-dt2,pall.nodes[i+1],phi1,phi2)
