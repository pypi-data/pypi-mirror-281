...
class Prompt:
    ...
    def truncate(self, token_limit: int = None, truncation_step: int = None):
        ...
        if truncation_step is not None:
            self.logger.info(f"Overriding {self._truncation_step=} with {truncation_step=}")
        else:
        ...
        self._truncate(truncation_blocks=truncation_blocks, num_tokens_to_truncate=num_tokens_to_truncate)
    ...
    def _truncate(self, truncation_blocks: list[TruncationBlock], num_tokens_to_truncate: int):
        ...
        for block in truncation_blocks:
            ...
            if tokens_removed + block.total_tokens <= num_tokens_to_truncate:
                to_remove[block.left:block.right+1] = [True] * (block.right - block.left + 1)
    ...
    def _build_truncation_blocks(self) -> list[TruncationBlock]:
        ...
        start = None
        current_truncation_priority, total_block_tokens = 0, 0
        for idx, current_part in enumerate(self._parts):
            # Never truncate part when `truncation_priority` == 0.
            if current_part.truncation_priority == 0:
                continue
            if current_part.tokens is None:
                raise ValueError(f"Part has not been tokenized; failing truncation loudly: {current_part=}")
        ...
        if start is None:
            self.logger.warning("No parts with truncation_priority set; no truncation blocks created; skipping truncation.")
            return []

        # Append the last block if necessary.
        if current_truncation_priority:
        ...
        truncation_blocks.sort(key=lambda marker: marker.truncation_priority, reverse=True)
        return truncation_blocks

    def _calculate_num_tokens_to_truncate(self, token_limit: int, truncation_step: int) -> int:
        """Calculates the number of messages to truncate."""
        num_surplus_tokens = max(0, self.num_total_tokens - token_limit)
        return (
            math.ceil(num_surplus_tokens / truncation_step) * truncation_step
        )
    ...
    def _tokenize_part(self, part: PromptPart, force_retokenize: bool = False):
        ...
        if self._tokenizer is None:
            try:
                ...
                raise ImportError(f"Error loading default tokenizer; consider providing your own tokenizer to Prompt(..., tokenizer=your_tokenizer). {ex=}.")

        # Avoid retokenizing the part if it has already been tokenized.
        if part.tokens and not force_retokenize:
            return
