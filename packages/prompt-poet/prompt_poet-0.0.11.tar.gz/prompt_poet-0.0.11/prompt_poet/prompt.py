...
class Prompt:
    ...
    def pretruncation_tokens(self) -> str: 
        ...
        try:
            ...
            raise TypeError(f"Error concatenating tokens: {ex=}. You may need to .tokenize() first.")
    ...
    def _build_truncation_blocks(self) -> list[TruncationBlock]:
        ...
        truncation_blocks = [block for block in truncation_blocks if block.truncation_priority > 0]
        truncation_blocks.sort(key=lambda marker: marker.truncation_priority, reverse=True)
        if not truncation_blocks:
            self.logger.warning("No parts with non-zero and positive truncation_priority set; no truncation blocks created; skipping truncation.")
