...
class APILoggerMiddleware:
    def __init__(self, get_response):
        ...
        if hasattr(settings, 'API_LOGGER_PATH_TYPE'):
            if settings.API_LOGGER_PATH_TYPE in ['ABSOLUTE', 'RAW_URI', 'FULL_PATH']:
                self.API_LOGGER_PATH_TYPE = settings.API_LOGGER_PATH_TYPE

        self.API_LOGGER_SKIP_URL_NAME = ['']
        if hasattr(settings, 'API_LOGGER_SKIP_URL_NAME'):
            if isinstance(settings.API_LOGGER_SKIP_URL_NAME, (tuple, list)):
        ...
        if hasattr(settings, 'API_LOGGER_SKIP_NAMESPACE'):
            if isinstance(settings.API_LOGGER_SKIP_NAMESPACE, (tuple, list)):
