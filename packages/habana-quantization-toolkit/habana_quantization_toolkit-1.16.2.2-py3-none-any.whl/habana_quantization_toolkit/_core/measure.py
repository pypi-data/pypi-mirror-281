...
from .._quant_common.quant_config import QuantMode, ScaleMethod, MeasureExclude, get_hqt_config
from .common import *
...
def init_measure_object(mod, name, observer_class, mod_type, skip_measure_output, d_shape=None, params=None):
  ...
  if (not mod_type.required_output) and skip_measure_output:
    ...
    output_observer = [observer_class('output%d'%(i), mod, None if d_shape is None else d_shape.outputs[i], params=None if params is None else params.outputs[i]) for i in range(mod_type.outputs)]
    ...
    with torch.no_grad():
        for name, mod in model.named_modules():
            if (name in mod_list) or (mod_list is None):
                ...
                mod_config = init_measure_object(mod, name, observer_class, mod_types[mod_type],
...
def measure_control_to_state_dict(mcd):
  ...
  for mname in mcd:
    ...
    if mcd[mname].outputs:
      sd[mname]['outputs'] = [mcd[mname].outputs[i].state.detach().cpu().float().numpy() for i in range(len(mcd[mname].outputs)) if mcd[mname].outputs[i].state is not None]
      sdl[mname]['outputs'] = [mcd[mname].outputs[i].state.detach().cpu().float().numpy().tolist() for i in range(len(mcd[mname].outputs)) if mcd[mname].outputs[i].state is not None]
