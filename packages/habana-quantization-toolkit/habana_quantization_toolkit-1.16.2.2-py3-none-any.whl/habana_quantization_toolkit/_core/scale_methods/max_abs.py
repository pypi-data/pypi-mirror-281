...
def fsdpa_act_maxabs_pts_weight_maxabs_pts_pow2_hw_scales(mod, measurement, params):
  ...
  input_scale.append(calc_maxabs_scale(torch.tensor(measurement.outputs[1], dtype=hp_dtype, device=device).max(), fullscale, input_backoff))
  input_scale = [scale_to_pow2_hw(x, device_type=config['device_type']) for x in input_scale]
  output_scale = calc_maxabs_scale(torch.tensor(measurement.outputs[0], dtype=hp_dtype, device=device).max(), fullscale, input_backoff)
