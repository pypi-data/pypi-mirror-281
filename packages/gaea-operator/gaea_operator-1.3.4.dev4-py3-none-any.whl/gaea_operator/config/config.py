...
class Config(metaclass=ABCMeta):
    ...
    def write_model_config(self, transform_model_uri: str):
        ...
        """
        model_type = self.metadata["algorithmParameters"].get(KEY_NETWORK_ARCHITECTURE, "")
        bcelogger.info('network architecture: {}'.format(model_type))
    ...
    def write_relate_config(self,
        ...
        modify_sub_extra_models = {"subModels": modify_sub_models, "extraModels": modify_extra_models}
        if is_new_ensemble_model:
            sub_extra_models = {"subModels": sub_models, "extraModels": extra_models}
            for key, models in sub_extra_models.items():
                for name in models:
                    if name not in modify_sub_extra_models[key]:
                        modify_sub_extra_models[key][name] = models[name]

        return modify_sub_extra_models["subModels"], modify_sub_extra_models["extraModels"]
