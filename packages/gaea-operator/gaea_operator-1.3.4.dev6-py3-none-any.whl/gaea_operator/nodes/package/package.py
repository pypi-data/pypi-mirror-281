...
def package(args):
    ...
    sub_extra_models.update(transform_model_name)
    new_sub_models = copy.deepcopy(origin_sub_models)
    new_extra_models = copy.deepcopy(origin_extra_models)
    for name in sub_extra_models.items():
        artifact_name = parse_artifact_name(name=name)
        model_name = parse_model_name(name=artifact_name.object_name)
        if model_name.local_name in new_sub_models:
            new_sub_models[model_name.local_name] = artifact_name.version
        if model_name.local_name in new_extra_models:
            new_extra_models[model_name.local_name] = artifact_name.version

    # 7. 创建需要上传的模型
    modify_models = {model_names["template_transform_model_name"].local_name: (transform_model_name, -1)}
    ...
    for name, version in modify_sub_models.items():
        ...
        if local_name in new_sub_models:
            new_sub_models[local_name] = version
        if local_name in new_extra_models:
            new_extra_models[local_name] = version
    ...
    config.metadata = {"subModels": new_sub_models, "extraModels": new_extra_models},
