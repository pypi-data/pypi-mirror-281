...
def chatbot_backend(
    ...
    Args:
        model_path: Hugging Face model name, e.g. "NousResearch/Llama-2-7b-chat-hf".
        device_map: PyTorch device map for model sub-modules. Defaults to "auto".
        torch_dtype: PyTorch data type (as string) for model parameters. Defaults to "float16".
        use_saved_model: Load the saved model from the cloud volume, if available. Defaults to True.
        save_model_to_volume: Save the pretrained model to the cloud volume, overwriting if a copy already exists. Defaults to True.
    """
    ...
    model = AutoModelForCausalLM.from_pretrained(
        model_name, device_map=device_map, torch_dtype=getattr(
            torch, torch_dtype),
    )
...
def generate(
    ...
    *,
    ...
    Kwargs:
        ...
        max_new_tokens: Maximum number of new tokens to generate. Defaults to 200.
        truncation: Whether to truncate the generated text. Defaults to True.
