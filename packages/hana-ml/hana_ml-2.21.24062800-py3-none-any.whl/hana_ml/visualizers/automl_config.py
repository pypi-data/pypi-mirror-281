...
CONFIG_TEMPLATE_DICT = {
    ...
    'Time Series': get_default_config_dict(identifier='timeseries')
}
...
def get_tempdir_path():
    template_name = 'automl_config.html'
...
def save_code_by_python(frame_id, config_str):
    config_file = open(get_tempdir_path() + os.sep + frame_id + '_config.json', 'w')
...
class AutoMLConfig(object):
    """
    Renders the AutoML Config UI as a notebook iframe.

    1.Call the get_config_dict() method on the VSCode or BAS platform requires manually importing the Visual Studio Code Extension(VSIX).

    .. image:: image/import_vscode_extension_1.png
       :align: center
       :width: 600px

    2.Follow the image below to install a .vsix file on VSCode or BAS.
    ...
    3.You'll be prompted with a successful installation message on the bottom-right corner.
    ...
    config_dict : dict, optional
        Manually set the custom config_dict. If this parameter is not specified, the classification config template is used.
    ...
    iframe_height : int, optional
        ...
        Defaults to 500.
    """
    __TEMPLATE = TemplateUtil.get_template('automl_config.html')

    def __init__(self, config_dict=None, iframe_height: int = 500) -> None:

        if config_dict is None:
            config_dict = CONFIG_TEMPLATE_DICT['Classification']
        elif isinstance(config_dict, str):
            config_dict = get_default_config_dict(config_dict)
        elif type(config_dict) != dict:
            raise TypeError("The value of parameter config_dict must be dict type or {'classification', 'regression', 'timeseries'}!")

        self.frame_id = '{}'.format(uuid.uuid4()).replace('-', '_').upper()
        self.html_str = AutoMLConfig.__TEMPLATE.render(frameId=self.frame_id, operator_metadata=OPERATOR_METADATA, parameter_metadata=PARAMETER_METADATA, input_config=json.dumps(config_dict), config_template_dict=json.dumps(CONFIG_TEMPLATE_DICT))

        display(HTML(build_frame_html(self.frame_id, HTMLFrameUtils.build_frame_src(self.html_str), iframe_height)))
        time.sleep(2)

    def generate_html(self, filename):
        """
        Save the AutoML Config UI as a html file.
        ...
        filename : str
            Html file name.
        """
        TemplateUtil.generate_html_file('{}_automl_config.html'.format(filename), self.html_str)

    def get_config_dict(self):
        """
        Get the latest config code from UI.
        """
        ...
        print('AutoML Config Temp Flag: {}: {}'.format(get_tempdir_path() + os.sep, self.frame_id))
        time.sleep(1)

        # 3. get code from temp file
        code = None
        file_path = get_tempdir_path() + os.sep + self.frame_id + '_config.json'
        retry_count = 1
        while retry_count <= 3:
        ...
        if code:
            ...
            print('\nThis get_config_dict method cannot be called on current running platform! Please click the button on the UI to get the config code!')
            print('\nIf the current code is running on the VSCode or BAS platform, please refer to the following content.')
            print('In order to successfully call get_config_dict on the VSCode or BAS platform, you must import the VSCode extension package manually.')
            extension_path = get_tempdir_path().replace('templates' + os.sep + 'temp', 'templates' + os.sep + 'hanamlapi-monitor-1.1.0.vsix')
