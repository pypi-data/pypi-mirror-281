...
class HANAScheduler(object):
    ...
    def _check_client_server_utctime(self):
        ...
        if abs((client_time - server_time).total_seconds()) > 60:
            logger.warning("The client's timezone does not align with the server's timezone. The server utctime is: %s. The client utctime is: %s", server_time, client_time)
