...
from ytb2audiobot.utils import delete_files_by_movie_id
...
async def processing_download(sender_id, message_id, command_context: dict, post_status_message_id = None):
    ...
    if result.get('subtitles'):
        if len(result.get('subtitles')) >= TELEGRAM_MAX_MESSAGE_TEXT_SIZE:
            await bot.send_document(
                chat_id=sender_id,
                reply_to_message_id=message_id,
                document=BufferedInputFile(
                    file=result.get('subtitles').encode('utf-8'),
                    filename='subtitles-' + command_context['id'] + '.txt'))
        else:
            await bot.send_message(
                chat_id=sender_id,
                reply_to_message_id=message_id,
                text=result.get('subtitles'), parse_mode='HTML')
    ...
    if not keep_data_files:
        print('🗑❌ Empty Files')
        await delete_files_by_movie_id(data_dir, command_context['id'])
...
async def message_parser_handler(message: Message) -> None | Message | bool:
    ...
    if not command_context.get('id'):
        return await bot.send_message(chat_id=sender_id, text='🟥️ Command context. Not a valid movie id.')
