...
@form_router.callback_query(lambda c: c.data.startswith('download:'))
async def process_callback_button1(callback_query: types.CallbackQuery, state: FSMContext):
    await state.set_state(Form.pushed_callback)
...
async def message_parser_handler(message: Message, state: FSMContext) -> None:
    ...
    if sender_type != 'user' and not command_context.get('name'):
        ...
        post = await bot.edit_message_reply_markup(
            chat_id=command_context.get('sender_id'),
