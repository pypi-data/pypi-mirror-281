...
class Tanuki:
    ...
    def prepare_task_queue(self):
        ...
        self.task_channel.queue_declare(queue=self.task_queue_name, durable=True) # To be modified to check the existence of the queue and to raise an error if not.
...
class TanukiWorker:
    ...
    def __init__(self, name: str, auth: str = "", host=None, port=None):
        ...
        @self.command("hello")
        def hello():
            return {"greeting": f"Greeting from {self.name}!"}
...
class LocalTanukiWorker(TanukiWorker):
