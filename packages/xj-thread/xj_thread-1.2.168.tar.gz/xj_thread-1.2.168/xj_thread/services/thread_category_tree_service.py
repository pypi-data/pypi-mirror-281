...
class ThreadCategoryTreeServices:
    ...
    def get_category_tree(category_id: int = None, category_value: str = None):
        ...
        category_set = ThreadCategory.objects.filter(
            Q(is_deleted__isnull=True) | Q(is_deleted=0)).annotate(category_value=F('value')).order_by('sort')
    ...
    def get_child_ids(category_id=None, category_value=None):
        ...
        if not category_id and not category_value:
            return None, "参数错误"
        category_set = ThreadCategory.objects.filter(
            Q(is_deleted=0) | Q(is_deleted__isnull=True)).annotate(category_value=F('value'))
        ...
        all_category_set = ThreadCategory.objects.filter(Q(is_deleted=0) | Q(is_deleted__isnull=True))
