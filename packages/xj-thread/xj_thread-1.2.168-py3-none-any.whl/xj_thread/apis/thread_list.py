...
class ThreadListAPIView(APIView):
    ...
    def get(self, *args, user_info=None, request_params, **kwargs):
        ...
        # print('> thread_list.py::get() request_params:', request_params)

        category_id = request_params.get("category_value")
        category_value = request_params.get("category_value")

        # ============== section 是否检查子树的的信息,如果category_id或者category_value都没传则查询全部 start ==============
        need_child = request_params.pop('need_child', None)
        if need_child:
            # print('> thread_list.py::get() category_id / value:', request_params.get("category_id"), category_value)
            ...
            # print('> thread_list.py::get() category_ids:', category_ids)
