...
def get_igs_for_invert(work_rid: str, work_path: str, comment_re: str = None, comment: str = None) -> ([str], bool):
    """
    return a list of comma separated tokens (representing image group names)
    from an optional dip_log comment with an optional regular expression
    :param work_rid: Work name
    :param work_path: source
    :param comment_re: Regular expression to match the comment
    :param comment: content of comment, or none
    :return: Tuple of:
        - The list of image groups in the BUDA catalog, or, if the comment is present, the list of image groups 
        in the `comment_re`
        - Boolean if the comment the source of the image groups
        
    if the comment_re is specified, and the comment matches it, all the image groups contained in the comment must exist.
    Otherwise, throw a FileNotFoundError
    """
    import re
    import archive_ops.api as AoApi

    rc_igs: [] = []
    comment_was_source: bool = True

    # Parse the comment first
    try:
        if not comment or not comment_re:
            raise IWE.NoCommentError()
        m = re.match(comment_re, comment)
        if m:
            rc_igs = m.groups()[0].split(',')
            # This is a special case. If image groups were given in the comment, they all must exist.
            # This inhibits the re-upload of the entire work when the comment is wrong
            test_rc_igs(rc_igs, work_path)
        else:
            # Just raise something to trigger call to BUDA
            e_str: str = f"Comment doesn't contain {comment_re}"
            raise IWE.NoCommentError(e_str)
    except IWE.NoCommentError as e:
        ...
        comment_was_source = False

    return rc_igs, comment_was_source
