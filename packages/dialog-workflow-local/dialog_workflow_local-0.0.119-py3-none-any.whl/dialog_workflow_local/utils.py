...
# TODO Change the order of the parameters (alphabet order): campaign, channel, message, profile
...
def get_curr_state(profile_id: int, channel_id: int = None, campaign_id: int = None, message_id: int = None) -> int:
    ...
    logger.info(f"Current state for profile_id: {profile_id} is {curr_state}", object=locals())
    return curr_state
...
def generic_menu(*, options: list, got_response: bool, chosen_numbers: str, choose_one_option: bool,
    ...
    if not got_response:
        outgoing_message += f"Please choose EXACTLY ONE option between 1-{len(options)}:~" if choose_one_option else \
            f"Please select your desired choices, You may select any of the numbers between 1-{len(options)} with a comma seperator between each choice:~"
    ...
    generic_menu_result = [int(x) for x in chosen_numbers]
