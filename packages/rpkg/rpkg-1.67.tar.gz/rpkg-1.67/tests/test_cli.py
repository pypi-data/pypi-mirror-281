...
class TestCoprBuild(CliTestCase):
    ...
    def assert_copr_build(self, cli_cmd, expected_copr_cli, cfg=None):
        with patch('sys.argv', new=cli_cmd):
            cli = self.new_cli(cfg)
            cli.copr_build()
    ...
    def test_copr_build(self):
        ...
        self.assert_copr_build(cli_cmd, [
            ...
            '{0}/{1}.src.rpm'.format(self.cloned_repo_path, self.mock_nvr.return_value)
        ])
    ...
    def test_copr_build_with_results_dir(self):
        cli_cmd = ['rpkg', '--path', self.cloned_repo_path,
                   'copr-build', 'user/project']
        conf_file = self.get_absolute_conf_filename('rpkg-results_dir-subdir.conf')

        self.assert_copr_build(cli_cmd, [
            'copr-cli', 'build', 'user/project',
            '{0}/results/{1}.src.rpm'.format(self.cloned_repo_path, self.mock_nvr.return_value)
        ], conf_file)
