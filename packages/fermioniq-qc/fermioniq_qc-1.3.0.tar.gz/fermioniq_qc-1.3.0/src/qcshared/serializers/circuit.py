...
class SerializedCircuit(BaseConfig):
    ...
    qubits: Optional[tuple[str, ...]] = None

    @validator("circuit")
    def validate_circuit(cls, circuit, values):
    ...
    def validate_qubits(cls, qubits, values):
        ...
        """
        if values.get("type") == "dict_list":
            return get_qubits_from_circuit_dict_list(values.get("circuit"))
