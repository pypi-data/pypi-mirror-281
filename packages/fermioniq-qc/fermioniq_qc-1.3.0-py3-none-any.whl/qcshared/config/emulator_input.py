...
class EmulatorInput(BaseConfigWithWarning):
    ...
    @validator("emulator_config", pre=False)
    def validate_config_circuit(cls, em_config, values, **kwargs):
        """Post-validation on circuit + config"""
        try:
            s_c = values["serialized_circuit"]
        except KeyError:
            return em_config

        circuit_qubits = s_c.qubits
        config_qubits = em_config.qubits
        ...
        if em_config.mode == "statevector":
            ...
            if em_config.optimizer.enabled:
                raise ValueError(
            ...
            if em_config.mode == "tebd" and long_range_gates(
                s_c.circuit, em_config.grouping
            ):
                raise ValueError(
            ...
            if em_config.optimizer.enabled and em_config.optimizer.initial_param_values:
                ...
                params_config = set(em_config.optimizer.initial_param_values)
    ...
    def validate_noise_model(cls, values):
        ...
        if not ("serialized_circuit" in values and "emulator_config" in values):
            return values
        ...
        if noise_model is None:
            ...
            if values["emulator_config"].mode == "statevector":
                raise ValueError(
            ...
            if values["emulator_config"].noise.validate_model:
                validate_noise_model_against_qubits(
                    noise_model=noise_model, qubits=values["emulator_config"].qubits
                )
                validate_noise_model_against_serialized_circuit(
                    noise_model=noise_model,
                    serialized_circuit=values["serialized_circuit"],
                )
        return values
